...
void exo_write_results(mesh_data* mesh, const char *infile, const char *outfile, double **values) {
  ...
  ex_put_time(outexoid, 1, &time_value);

  ex_put_nodal_var(outexoid, 1, 1, mesh->num_nodes, values[0]);
  ex_put_nodal_var(outexoid, 1, 2, mesh->num_nodes, values[1]);
  ex_put_nodal_var(outexoid, 1, 3, mesh->num_nodes, values[2]);

...
int bin_search_max(const int list[], const int num, const int value)
...
void init_nodes(Exo_DB *exo, Dpi *dpi)
 ...
 *****************************************************************/
{
  ...
  if (total_nodes != number_proc_nodes) {
    fprintf(stderr, "Proc %d : init_nodes ERROR total nodes, %d, differs from num_proc_nodes, %d\n",
            ProcID, total_nodes, number_proc_nodes);
    GOMA_EH(GOMA_ERROR, "init_nodes: Shouldn't be here");
  }
...
void make_node_to_elem_matrl_map(Exo_DB *exo)
...
int index_umi_list(const UMI_LIST_STRUCT *umi_ptr, const int value)
 ...
 *********************************************************************/
{
  ...
  if (list_loc < umi_ptr->Length) {
    if (umi_ptr->List[list_loc] == value)
      return list_loc;
  }
  return -1;
}
...
int node_matrl_index(const int nodeNum, const int mn)
...
int first_matID_at_node(const int nodeNum)
 ...
 *********************************************************************/
{
  ...
  if (umi_ptr == NULL || umi_ptr->Length <= 0) {
    fprintf(stderr, "first_matID_at_node ERROR P_%d: mat list is empty at node %d\n", ProcID,
            nodeNum);
    GOMA_EH(GOMA_ERROR, "first_matID_at_node ERROR");
    return -1;
  }
...
void free_umi_list(UMI_LIST_STRUCT *umi_ptr)
...
NODAL_RESID_WKSP_STRUCT *nodal_resid_wksp_alloc(void)
...
void nodal_resid_wksp_destroy(NODAL_RESID_WKSP_STRUCT **residWkspHdl)
...
void nodal_resid_wksp_free(NODAL_RESID_WKSP_STRUCT *residWksp)
...
void vcrr_add(VCRR_STRUCT ***vcrr_hdl, const int eqn, const int mn_curr, const int mn_apply)
      ...
      if (vcrr_ptr->Var_Type == eqn) {
        if (vcrr_ptr->MatID_Volume == mn_curr) {
          if (vcrr_ptr->MatID_Row_Redirect == mn_apply) {
            ...
            GOMA_EH(GOMA_ERROR, "incompatibility");
          }
        }
      }
    }
...
int vcrr_lookup(VCRR_STRUCT **vcrr, const int eqn, const int mn_curr)
...
void nullify_dirichlet_bcs(void)

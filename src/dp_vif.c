...
void noahs_ark(void) {
  ...
  */
  ...
  for (i = 0; i < upd->Num_Mat; i++) {
    ...
    ddd_add_member(n, &mp_glob[i]->turb_k_wt_func, 1, MPI_DOUBLE);
    ddd_add_member(n, &mp_glob[i]->turb_omega_wt_func, 1, MPI_DOUBLE);
    ...
    ddd_add_member(n, &mp_glob[i]->turb_k_wt_funcModel, 1, MPI_INT);
    ddd_add_member(n, &mp_glob[i]->turb_omega_wt_funcModel, 1, MPI_INT);

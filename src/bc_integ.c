...
int apply_integrated_bc(double x[],            /* Solution vector for the current processor    */
 ...
 ****************************************************************************/
{
  ...
  for (ip = 0; ip < ip_total; ip++) {
    ...
    for (ibc = 0; (bc_input_id = (int)elem_side_bc->BC_input_id[ibc]) != -1; ibc++) {
       ...
       */
      if (bc->BC_Name == VL_EQUIL_PRXN_BC || bc->BC_Name == IS_EQUIL_PRXN_BC ||
          bc->BC_Name == YFLUX_DISC_RXN_BC || bc->BC_Name == SDC_STEFANFLOW_BC ||
      ...
      if (bc_desc->method == bc_application) {
         ...
         */
        if (bc->BC_Name > NEW_WAY_LOW_BC && bc->BC_Name < NEW_WAY_HIGH_BC) {
          new_way = TRUE;
        }
        ...
        case VELO_NORMAL_LS_PETROV_BC: {
          ...
          */
          if ((SS_Internal_Boundary[ss_index] == -1 && pd->gv[VELOCITY1]) ||
              ...
              (SS_Internal_Boundary[ss_index] != -1 && bc->BC_Data_Int[0] == -1 && iapply &&
               pd->gv[VELOCITY1])) {
            fvelo_normal_bc(func, d_func, bc->BC_Data_Float[0], contact_flag, x_dot, theta, delta_t,
                            (int)bc->BC_Name, bc->BC_Data_Float[1], bc->BC_Data_Float[2],
                            bc->BC_Data_Float[3]);
          }
        }
        //        }
          ...
          if (ls != NULL) {
            if ((SS_Internal_Boundary[ss_index] == -1 && pd->v[pg->imtrx][VELOCITY1]) ||
                (SS_Internal_Boundary[ss_index] != -1 &&
                 bc->BC_Data_Int[0] == ei[pg->imtrx]->elem_blk_id) ||
                (SS_Internal_Boundary[ss_index] != -1 && bc->BC_Data_Int[0] == -1 && iapply &&
                 pd->v[pg->imtrx][VELOCITY1])) {
              fvelo_tangential_ls_bc(func, d_func, bc->BC_Data_Float[0], x_dot, theta, delta_t,
        ...
        case KIN_DISPLACEMENT_BC:
          f_kinematic_displacement_bc(func, d_func, bc->BC_Data_Int[0], bc->BC_ID, bc->u_BC,
                                      bc->len_u_BC);
          break;

        case KIN_DISPLACEMENT_RS_BC:
          f_kinematic_displacement_rs_bc(func, d_func, bc->BC_Data_Int[0], elem_side_bc->id_side,
                                         xi, exo);
          break;

        case KINEMATIC_DISC_BC:
        case VELO_NORMAL_DISC_BC:
          fvelo_normal_disc_bc(func, d_func, bc->BC_Data_Float[0], x_dot, theta, delta_t);
          break;

        case KINEMATIC_SPECIES_BC:
          kinematic_species_bc(func, d_func, bc->BC_Data_Int[0], bc->BC_Data_Float[0], x_dot, theta,
                               delta_t);
          break;
        ...
        case DISCONTINUOUS_VELO_BC:
          discontinuous_velocity(func, d_func, x_dot, bc->BC_Data_Int[0], bc->BC_Data_Int[1],
                                 bc->BC_Data_Int[2], theta, delta_t);
          break;
        ...
        case DARCY_LUB_BC:
          ...
          if (time_intermediate >= bc->BC_Data_Float[1]) {
            ...
            sat_darcy_continuous_bc(func, d_func, theta, delta_t, time_value, bc->BC_Data_Int[0],
                                    bc->BC_Data_Int[1], bc->BC_Data_Float[0], v_attach);
            if (neg_elem_volume)
              return (status);
          }
        ...
        case T_MELT_BC:
          if (iapply) {
            ftmelt_bc(func, d_func, bc->BC_Data_Float[0], x_dot, theta, delta_t);
          }
          break;
        ...
        case KIN_CHEM_BC:
          if (iapply) {
            kin_bc_leak(func, d_func, x_dot, theta, delta_t, bc_input_id, BC_Types);
          }
          break;
        ...
        case SHELL_DIFF_KINEMATIC_BC: /* Applies only to shell elements */
          shell_diff_kinematic_bc(func, d_func, x_dot, theta, delta_t, elem_side_bc->id_side, wt,
                                  xi, exo, 0);
          break;
        ...
        case LUBP_SH_FP_MATCH_BC:
          match_lubrication_film_pressure(func, d_func, bc->BC_Data_Int[0], bc->BC_Data_Int[1]);
          break;

        case KIN_ELECTRODEPOSITION_BC: /*  RSL 5/28/02  */
          if (iapply) {
            kin_bc_electrodeposition(func, d_func, time_value, theta, delta_t, bc_input_id,
                                     BC_Types);
          }
          break;
        ...
        case VNORM_ELECTRODEPOSITION_BC:
          if (iapply) {
            vnorm_bc_electrodeposition(func, d_func, theta, time_value, delta_t, bc_input_id,
                                       BC_Types);
          }
          break;
        ...
        case VELO_STREAMING_BC:
          fvelo_tangential_bc(func, d_func, x, bc->BC_Data_Int[0], bc->BC_Data_Float[0],
                              bc->BC_Data_Float[1], bc->BC_Data_Float[2], bc->BC_Data_Float[3],
                              xsurf, x_dot, theta, delta_t, (int)bc->BC_Name, elem_side_bc->id_side,
        ...
        case VELO_TANGENT_3D_BC:
          fvelo_tangent_3d(func, d_func, x_dot, bc->BC_Data_Float[0], bc->BC_Data_Float[1],
                           bc->BC_Data_Float[2], bc->BC_Data_Float[3], theta, delta_t);
          if (neg_elem_volume)
            return (status);
          break;
        ...
        case VELO_SLIP_SOLID_BC:
          fvelo_tangential_solid_bc(func, d_func, x, x_dot, x_rs_dot, (int)bc->BC_Name,
                                    bc->BC_Data_Int[0], bc->BC_Data_Int[1], bc->BC_Data_Float[0],
                                    bc->BC_Data_Int[2], bc->BC_Data_Float[1], xsurf, theta,
                                    delta_t);
          break;

        case VELO_NORMAL_SOLID_BC:

          fvelo_normal_solid_bc(func, d_func, x, x_dot, x_rs_dot, (int)bc->BC_Name,
                                bc->BC_Data_Int[0], bc->BC_Data_Int[1], theta, delta_t);
          break;
        ...
        case VELO_SLIP_ROT_FLUID_BC:
          fvelo_slip_bc(func, d_func, x, (int)bc->BC_Name, (int)bc->max_DFlt, bc->BC_Data_Float,
                        (int)bc->BC_Data_Int[0], xsurf, theta, delta_t);
          break;
        case VELO_SLIP_POWER_CARD_BC:
        case VELO_SLIP_POWER_BC:
          fvelo_slip_power_bc(func, d_func, (int)bc->BC_Name, (int)bc->max_DFlt, bc->BC_Data_Float,
                              theta, delta_t);
          break;

        case AIR_FILM_BC:
        case AIR_FILM_ROT_BC:
          fvelo_airfilm_bc(func, d_func, x, (int)bc->BC_Name, bc->BC_Data_Float,
                           (int)bc->BC_Data_Int[0], xsurf, theta, delta_t);
          break;

        case VELO_SLIP_EK_BC:
          fvelo_slip_electrokinetic_bc(func, d_func, bc->BC_Data_Float[0], bc->BC_Data_Float[1]);
          break;

        case VELO_EK_3D_BC:
          fvelo_electrokinetic_3d(func, d_func, x_dot, bc->BC_Data_Float[0], bc->BC_Data_Float[1],
        ...
        case VELO_SLIP_LS_ROT_BC:
          fvelo_slip_level(func, d_func, (int)bc->BC_Name, bc->BC_Data_Float[0],
                           bc->BC_Data_Float[1], bc->BC_Data_Float[2], bc->BC_Data_Float[3],
                           bc->BC_Data_Float[4], bc->BC_Data_Float[5], bc->BC_Data_Float[6],
                           bc->BC_Data_Float[7], bc->BC_Data_Float[8], theta, delta_t);
          break;
        ...
        case VELO_SLIP_LS_ORIENTED_BC:
          fvelo_slip_ls_oriented(func, d_func, bc->BC_Data_Float[0], bc->BC_Data_Float[1],
                                 bc->BC_Data_Float[2], bc->BC_Data_Float[3], bc->BC_Data_Float[4],
                                 bc->BC_Data_Float[5], bc->BC_Data_Float[6], bc->BC_Data_Float[7]);
          break;

        case Q_VELO_SLIP_BC:
          q_velo_slip_bc(func, d_func, (int)BC_Types[bc_input_id].BC_Data_Int[0], x, xsurf, theta,
                         delta_t);
          break;
        ...
        case NO_SLIP_RS_BC: /*for backward compatibility even though
          ...
          no_slip(func, d_func, x_dot, x_rs_dot, theta, delta_t, bc_input_id, BC_Types,
                  (int)bc->BC_Data_Int[0], (int)bc->BC_Data_Int[1]);
          break;

        case VN_POROUS_BC:
          porous_normal_velocity_bc(func, d_func, x_dot, theta, delta_t, bc_input_id, BC_Types,
                                    (int)bc->BC_Data_Int[0], (int)bc->BC_Data_Int[1],
                                    (int)bc->BC_Data_Int[2], bc->BC_Data_Float[0]);
          break;
        ...
        case SLOPEZ_BC:
        case SLOPE_BC:
          slope_n_dot_n0_bc(func, d_func, bc->BC_Data_Float[0], bc->BC_Data_Float[1],
                            bc->BC_Data_Float[2]);
          break;

        case FORCE_BC:
        case FORCE_RS_BC:
          force_n_dot_f_bc(func, d_func, bc->BC_Data_Float[0], bc->BC_Data_Float[1],
                           bc->BC_Data_Float[2], 0, delta_t, theta, ip, ip_total,
                           time_intermediate);
          break;
        case FORCE_SIC_BC:
          force_n_dot_f_bc(func, d_func, bc->BC_Data_Float[0], bc->BC_Data_Float[1],
                           bc->BC_Data_Float[2], 1, delta_t, theta, ip, ip_total,
                           time_intermediate);
          break;

        case FORCE_USER_BC:
        case FORCE_USER_RS_BC:
          force_user_surf(func, d_func, bc->u_BC, time_intermediate);
          break;

        case FORCE_USER_SIC_BC:
          force_user_surf(func, d_func, bc->u_BC, time_intermediate);
          force_n_dot_f_bc(func, d_func, 0, 0, 0, 2, delta_t, theta, ip, ip_total,
                           time_intermediate);
          break;
        ...
        case ATTR_FORCE_RS_BC:
          rep_force_n_dot_f_bc(func, d_func, bc->BC_Data_Float[0], bc->BC_Data_Float[1],
                               bc->BC_Data_Float[2], bc->BC_Data_Float[3], bc->BC_Data_Float[4],
                               bc->BC_Data_Float[5], bc->BC_Data_Float[6], (int)bc->BC_Name);
          break;
        case REP_FORCE_ROLL_BC:
        case REP_FORCE_ROLL_RS_BC:
          rep_force_roll_n_dot_f_bc(func, d_func, bc->BC_Data_Float[0], &(bc->BC_Data_Float[1]),
                                    &(bc->BC_Data_Float[4]), bc->BC_Data_Float[7],
                                    bc->BC_Data_Float[8], bc->BC_Data_Float[9], (int)bc->BC_Name);
          break;

        case NORM_FORCE_BC:
        case NORM_FORCE_RS_BC:
          norm_force_n_dot_f_bc(func, d_func, bc->BC_Data_Float[0], bc->BC_Data_Float[1],
                                bc->BC_Data_Float[2]);
          break;
        ...
        case FRICTION_ACOUSTIC_RS_BC:
          friction_n_dot_f_bc(func, d_func, bc->BC_Data_Float[0], bc->BC_Data_Int[0], delta_t,
                              theta, ip, ip_total, (int)bc->BC_Name, time_intermediate, bc->u_BC,
                              bc->len_u_BC);
          break;

        case REP_FORCE_SHU_BC: /* Applies only to shell elements */
          rep_force_shell_n_dot_f_bc(func, d_func, x_dot, theta, delta_t, ip, ip_total,
        ...
        case ELEC_TRACTION_SOLID_BC:
          ...
          if (bc->BC_Data_Int[0] == ei[pg->imtrx]->elem_blk_id) {
            elec_surf_stress(cfunc, d_cfunc, elem_side_bc->id_side, elem_side_bc, iconnect_ptr,
                             mp->permittivity, bc->BC_Data_Float[0], (int)bc->BC_Name);
        ...
        case SHEAR_TO_SHELL_BC:
          ...
          shear_to_shell(cfunc, d_cfunc, elem_side_bc->id_side, bc->BC_Data_Float[0], elem_side_bc,
                         iconnect_ptr, xi, exo);
          break;
        ...
        case TENSION_SHEET_BC:
          memset(cfunc, 0, MDE * DIM * sizeof(double));
          memset(d_cfunc, 0, MDE * DIM * (MAX_VARIABLE_TYPES + MAX_CONC) * MDE * sizeof(double));
          sheet_tension(cfunc, d_cfunc, elem_side_bc->id_side, bc->BC_Data_Float[0], elem_side_bc,
                        iconnect_ptr, xi, exo);
          break;
        ...
        case CAPILLARY_TABLE_BC:
          ...
          if (bc->BC_Data_Int[0] == ei[pg->imtrx]->elem_blk_id ||
            ...
            fn_dot_T(cfunc, d_cfunc, elem_side_bc->id_side, bc->BC_Data_Float[0], pb, elem_side_bc,
                     iconnect_ptr, dsigma_dx);

            if (bc->BC_Name == CAP_REPULSE_BC) {
              apply_repulsion(cfunc, d_cfunc, bc->BC_Data_Float[3], bc->BC_Data_Float[4],
                              bc->BC_Data_Float[5], bc->BC_Data_Float[6], bc->BC_Data_Float[7],
                              elem_side_bc, iconnect_ptr);
            }

            if (bc->BC_Name == CAP_REPULSE_ROLL_BC) {
              apply_repulsion_roll(
                  cfunc, d_cfunc, x, bc->BC_Data_Float[3], &(bc->BC_Data_Float[4]),
                  &(bc->BC_Data_Float[7]), bc->BC_Data_Float[10], bc->BC_Data_Float[11],
                  bc->BC_Data_Float[12], bc->BC_Data_Float[13], bc->BC_Data_Float[14],
                  bc->BC_Data_Float[15], bc->BC_Data_Int[2], elem_side_bc, iconnect_ptr);
            }
            if (bc->BC_Name == CAP_REPULSE_USER_BC) {
              apply_repulsion_user(cfunc, d_cfunc, bc->BC_Data_Float[3], &(bc->BC_Data_Float[4]),
                                   &(bc->BC_Data_Float[7]), bc->BC_Data_Float[10],
                                   bc->BC_Data_Float[11], bc->BC_Data_Float[12],
                                   bc->BC_Data_Float[13], bc->BC_Data_Float[14], elem_side_bc,
                                   iconnect_ptr);
            }
            if (bc->BC_Name == CAP_REPULSE_TABLE_BC) {
              apply_repulsion_table(cfunc, d_cfunc, x, bc->BC_Data_Float[3], bc->BC_Data_Float[4],
                                    bc->BC_Data_Float[5], bc->BC_Data_Float[6],
                                    bc->BC_Data_Float[7], &(bc->BC_Data_Float[8]),
                                    bc->BC_Data_Int[2], elem_side_bc, iconnect_ptr);
            }
            if (BC_Types[bc_input_id].BC_Name == CAP_RECOIL_PRESS_BC) {
              apply_vapor_recoil(
                  cfunc, d_cfunc, BC_Types[bc_input_id].BC_Data_Float[3],
                  BC_Types[bc_input_id].BC_Data_Float[4], BC_Types[bc_input_id].BC_Data_Float[5],
                  BC_Types[bc_input_id].BC_Data_Float[6], BC_Types[bc_input_id].BC_Data_Float[7],
                  elem_side_bc, iconnect_ptr);
            }
        ...
        case PRESSURE_USER_BC:
          fn_dot_T_user(func, d_func, bc->u_BC, time_intermediate);
          break;

        case FLOW_PRESSURE_BC:
          flow_n_dot_T_hydro(func, d_func, 0., 0., 0., bc->BC_Data_Float[0]);
          break;

        case LGR_FLOWRATE_BC: {
          int iAC = bc->BC_Data_Int[0]; /* need this index so we can retrieve the current LM value
                                           from augc */

          flow_n_dot_T_hydro(func, d_func, 0., 0., 0., -augc[iAC].DataFlt[0]);
        } break;

        case FLOW_HYDROSTATIC_BC:
          flow_n_dot_T_hydro(func, d_func, bc->BC_Data_Float[0], bc->BC_Data_Float[1],
                             bc->BC_Data_Float[2], bc->BC_Data_Float[3]);
          break;

        case FLOW_PRESSURE_VAR_BC:
          flow_n_dot_T_var_density(func, d_func, bc->BC_Data_Float[0], time_value);
          break;

        case FLOW_STRESSNOBC_BC:
          flow_n_dot_T_nobc(func, d_func, bc->BC_Data_Float[0], bc->BC_Data_Int[0], time_value);
          break;
        ...
        case GRAD_LUB_PRESS_BC:
          shell_n_dot_flow_bc_confined(func, d_func, bc->BC_Data_Float[0], time_value, delta_t, xi,
                                       exo);
        ...
        case SHELL_GRAD_FP_BC:
          shell_n_dot_flow_bc_film(func, d_func, bc->BC_Data_Float[0], time_value, delta_t, xi,
                                   exo);
          surface_determinant_and_normal(
              ielem, iconnect_ptr, num_local_nodes, ielem_dim - 1, (int)elem_side_bc->id_side,
              (int)elem_side_bc->num_nodes_on_side, (elem_side_bc->local_elem_node_id));
        ...
        case SHELL_FLOW_DEVELOPED_BC:
          shell_n_dot_gradp_bc(func, d_func, time_value, delta_t, xi, exo);
          surface_determinant_and_normal(
              ielem, iconnect_ptr, num_local_nodes, ielem_dim - 1, (int)elem_side_bc->id_side,
              (int)elem_side_bc->num_nodes_on_side, (elem_side_bc->local_elem_node_id));
          break;
        ...
        case SHELL_GRAD_FP_NOBC_BC:
          shell_n_dot_flow_bc_film(func, d_func, 0.0, time_value, delta_t, xi, exo);
          surface_determinant_and_normal(
              ielem, iconnect_ptr, num_local_nodes, ielem_dim - 1, (int)elem_side_bc->id_side,
              (int)elem_side_bc->num_nodes_on_side, (elem_side_bc->local_elem_node_id));
          break;
        ...
        case SHELL_GRAD_FH_BC:
          shell_n_dot_gradh_bc(func, d_func, bc->BC_Data_Float[0]);
          surface_determinant_and_normal(
              ielem, iconnect_ptr, num_local_nodes, ielem_dim - 1, (int)elem_side_bc->id_side,
              (int)elem_side_bc->num_nodes_on_side, (elem_side_bc->local_elem_node_id));
          break;
        ...
        case SHELL_GRAD_PC_BC:
          shell_n_dot_pflux_bc(func, d_func, bc->BC_Data_Float[0], time_value, delta_t);
          surface_determinant_and_normal(
              ielem, iconnect_ptr, num_local_nodes, ielem_dim - 1, (int)elem_side_bc->id_side,
              (int)elem_side_bc->num_nodes_on_side, (elem_side_bc->local_elem_node_id));
          break;
        ...
        case SHELL_GRAD_PC_NOBC_BC:
          shell_n_dot_pflux_bc(func, d_func, 0.0, time_value, delta_t);
          surface_determinant_and_normal(
              ielem, iconnect_ptr, num_local_nodes, ielem_dim - 1, (int)elem_side_bc->id_side,
              (int)elem_side_bc->num_nodes_on_side, (elem_side_bc->local_elem_node_id));
          break;
        ...
        case SHELL_TFMP_FREE_LIQ_BC:
          if (pd->e[pg->imtrx][R_TFMP_MASS]) {
            shell_n_dot_liq_velo_bc_tfmp(func, d_func, 0.0, time_value, delta_t, xi, exo);
        ...
        case SHELL_TFMP_NUM_DIFF_BC:
          shell_num_diff_bc_tfmp(func, d_func, time_value, delta_t, xi, exo);
          surface_determinant_and_normal(
              ielem, iconnect_ptr, num_local_nodes, ielem_dim - 1, (int)elem_side_bc->id_side,
              (int)elem_side_bc->num_nodes_on_side, (elem_side_bc->local_elem_node_id));
          break;
        case SHELL_LUBRICATION_OUTFLOW_BC:
          shell_lubrication_outflow(func, d_func, time_value, delta_t, xi, exo);
          surface_determinant_and_normal(
              ielem, iconnect_ptr, num_local_nodes, ielem_dim - 1, (int)elem_side_bc->id_side,
              (int)elem_side_bc->num_nodes_on_side, (elem_side_bc->local_elem_node_id));
          break;
        ...
        case SH_S11_WEAK_BC:
        case SH_S22_WEAK_BC:
          apply_shell_traction_bc(func, d_func, bc->BC_Name, bc->BC_Data_Float[0],
                                  bc->BC_Data_Float[1], bc->BC_Data_Float[2]);

          break;
        case SHELL_TFMP_AVG_PLATE_VELO_BC:
          shell_tfmp_avg_plate_velo_liq(func, d_func, time_value, delta_t, xi, exo);
          surface_determinant_and_normal(
              ielem, iconnect_ptr, num_local_nodes, ielem_dim - 1, (int)elem_side_bc->id_side,
              (int)elem_side_bc->num_nodes_on_side, (elem_side_bc->local_elem_node_id));
          break;
        ...
        case SHELL_TFMP_FREE_GAS_BC:
          shell_n_dot_gas_velo_bc_tfmp(func, d_func, 0.0, time_value, delta_t, xi, exo);
          surface_determinant_and_normal(
              ielem, iconnect_ptr, num_local_nodes, ielem_dim - 1, (int)elem_side_bc->id_side,
              (int)elem_side_bc->num_nodes_on_side, (elem_side_bc->local_elem_node_id));
        ...
        case SHELL_TFMP_GRAD_S_BC:
          shell_tfmp_n_dot_grad_s(func, d_func, time_value, delta_t, xi, exo);
          surface_determinant_and_normal(
              ielem, iconnect_ptr, num_local_nodes, ielem_dim - 1, (int)elem_side_bc->id_side,
              (int)elem_side_bc->num_nodes_on_side, (elem_side_bc->local_elem_node_id));
          break;
        ...
        case VL_EQUIL_BC:
          ...
          raoults_law(func, d_func, bc->BC_Data_Int[0], bc->BC_Data_Int[1], bc->BC_Data_Int[2],
                      bc->BC_Data_Float[0], bc->BC_Data_Float[1], bc->BC_Data_Float[2],
                      bc->BC_Data_Float[3], bc->BC_Data_Float[4]);
          break;

        case YFLUX_DISC_RXN_BC:

          yflux_disc_rxn_bc(func, d_func, bc->BC_Data_Int[0], bc->BC_Data_Int[1],
                            bc->BC_Data_Int[2], bc->BC_Data_Float[0], bc->BC_Data_Float[1], delta_t,
                            theta);
        ...
        case VL_EQUIL_PRXN_BC:
          new_way = TRUE;
          func[0] = raoults_law_prxn(&jacCol, bc, ip, elem_side_bc, x_dot, time_value, theta,
                                     delta_t, interface_id);
          break;

        case IS_EQUIL_PRXN_BC:
          new_way = TRUE;
          func[0] = is_equil_prxn(&jacCol, bc, ip, elem_side_bc, x_dot, time_value, theta, delta_t,
                                  interface_id);
          break;
        ...
        case SDC_STEFANFLOW_BC:
          ...
          if (iapply) {
            func[0] = sdc_stefan_flow(&jacCol, bc, ip, elem_side_bc, x_dot, time_value, theta,
                                      delta_t, interface_id);
          } else {
            skip_other_side = TRUE;
          }
        ...
        case SDC_KIN_SFV_BC:
          ...
          if (iapply) {
            func[0] = sdc_stefan_volume_flow(&jacCol, bc, ip, elem_side_bc, x_dot, theta, delta_t);
          } else {
            skip_other_side = TRUE;
          }
          new_way = TRUE;
        ...
        case VL_POLY_BC:
          flory_huggins(func, d_func, bc->BC_Data_Int[0], bc->BC_Data_Int[1], bc->BC_Data_Int[2],
                        bc->BC_Data_Int[3], bc->BC_Data_Float[0]);
          break;

        case YTOTALFLUX_CONST_BC: /*  RSL 6/7/00  */
          if (iapply) {
            const_mass_flux_surf_bc(func, d_func, bc->BC_Data_Int[0], bc->BC_Data_Float[0],
        ...
        case YFLUX_BC:
          if (iapply) {
            mass_flux_surf_bc(func, d_func, bc->BC_Data_Int[0], bc->BC_Data_Float[0],
                              bc->BC_Data_Float[1], delta_t, theta);
          }
          break;

        case YFLUX_ETCH_BC:
          if (iapply) {
            mass_flux_surf_etch(func, d_func, bc->BC_Data_Int[0], bc->BC_Data_Int[1], time_value,
                                delta_t, theta);
          }
          break;
        ...
        case YFLUX_BV_BC:
          if (iapply) {
            mass_flux_BV_surf_bc(
                func, d_func, BC_Types[bc_input_id].BC_Data_Int[0],
                BC_Types[bc_input_id].BC_Data_Float[0], BC_Types[bc_input_id].BC_Data_Float[1],
                BC_Types[bc_input_id].BC_Data_Float[2], BC_Types[bc_input_id].BC_Data_Float[3],
                BC_Types[bc_input_id].BC_Data_Float[4], BC_Types[bc_input_id].BC_Data_Float[5],
                BC_Types[bc_input_id].BC_Data_Float[6], delta_t, theta);
          }
          break;
        ...
        case YFLUX_HOR_BC:
          if (iapply) {
            mass_flux_HOR_surf_bc(
                ...
                BC_Types[bc_input_id].BC_Data_Float[4], BC_Types[bc_input_id].BC_Data_Float[5],
                BC_Types[bc_input_id].BC_Data_Float[6], BC_Types[bc_input_id].BC_Data_Float[7],
                BC_Types[bc_input_id].BC_Data_Float[8], BC_Types[bc_input_id].BC_Data_Float[9],
                delta_t, theta);
          }
          break;
        ...
        case YFLUX_ORR_BC:
          if (iapply) {
            mass_flux_ORR_surf_bc(
                ...
                BC_Types[bc_input_id].BC_Data_Float[4], BC_Types[bc_input_id].BC_Data_Float[5],
                BC_Types[bc_input_id].BC_Data_Float[6], BC_Types[bc_input_id].BC_Data_Float[7],
                BC_Types[bc_input_id].BC_Data_Float[8], delta_t, theta);
          }
          break;
        ...
        case YFLUX_SULFIDATION_BC:
          if (iapply) {
            mass_flux_SULFIDATION_surf_bc(
                func, d_func, BC_Types[bc_input_id].BC_Data_Int[2],
                BC_Types[bc_input_id].BC_Data_Int[0], BC_Types[bc_input_id].BC_Data_Float[0],
                ...
                BC_Types[bc_input_id].BC_Data_Float[5], BC_Types[bc_input_id].BC_Data_Float[6],
                BC_Types[bc_input_id].BC_Data_Float[7], BC_Types[bc_input_id].BC_Data_Float[8],
                BC_Types[bc_input_id].BC_Data_Float[9], delta_t, theta);
          }
          break;

        case YFLUX_BV2_BC: /* RSL 1/15/01 */
          if (iapply) {
            mass_flux_BV2_surf_bc(func, d_func, bc->BC_Data_Int[0], bc->BC_Data_Float[0],
                                  bc->BC_Data_Float[1], bc->BC_Data_Float[2], bc->BC_Data_Float[3],
                                  bc->BC_Data_Float[4], bc->BC_Data_Float[5], time_value, delta_t,
                                  theta);
          }
          break;
        ...
        case CURRENT_BV2_BC: /* RSL 1/22/01 */
          if (iapply) {
            current_BV2_surf_bc(func, d_func, bc->BC_Data_Int[0], bc->BC_Data_Float[0],
                                bc->BC_Data_Float[1], bc->BC_Data_Float[2], bc->BC_Data_Float[3],
                                bc->BC_Data_Float[4], bc->BC_Data_Float[5], time_value, delta_t,
                                theta);
          }
        ...
        case YFLUX_NI_BC: /* RSL 3/9/01 */
          if (iapply) {
            mass_flux_NI_surf_bc(func, d_func, bc->BC_Data_Int[0], bc->BC_Data_Float[0],
                                 bc->BC_Data_Float[1], time_value, delta_t, theta);
          }
          break;

        case CURRENT_NI_BC: /* RSL 3/9/01 */
          if (iapply) {
            current_NI_surf_bc(func, d_func, bc->BC_Data_Int[0], bc->BC_Data_Float[0],
                               bc->BC_Data_Float[1], time_value, delta_t, theta);
          }
          break;

        case YFLUX_SUS_BC:
          sus_mass_flux_surf_bc(func, d_func, bc->BC_Data_Int[0], time_value, delta_t, theta,
                                pg_data->h);
          break;
        ...
        case YFLUX_USER_BC:
          mass_flux_surf_user_bc(func, d_func, bc->BC_Data_Int[0], bc->u_BC, time_intermediate);
          break;

        case YFLUX_EQUIL_BC:
          get_equil_surf_bc(func, d_func, bc->BC_Data_Int[2], bc->BC_Data_Int[0],
                            bc->BC_Data_Float[0], bc->BC_Data_Float[1], bc->BC_Data_Float[2],
                            bc->BC_Data_Float[3], bc->BC_Data_Float[4], delta_t, theta);
          break;

        case YUSER_BC:
          yuser_surf(func, d_func, BC_Types[bc_input_id].BC_Data_Int[0], BC_Types[bc_input_id].u_BC,
                     time_intermediate);
          break;
        ...
        case POROUS_FLUX_BC:
          porous_mass_flux_surf_bc(func, d_func, bc->BC_Data_Int[0], bc->BC_Data_Float[0],
                                   bc->BC_Data_Float[1], bc->BC_Data_Float[2], bc->BC_Data_Float[3],
                                   delta_t, theta);
          if (neg_elem_volume)
            return (status);
        ...
        case POROUS_CONV_BC:
          porous_convection_bc(func, d_func, bc->BC_Data_Int[0], delta_t, theta);
          if (neg_elem_volume)
            return (status);
          break;

        case VP_EQUIL_BC:
          porous_vapor_equil_bc(func, d_func, x_dot, theta, delta_t, bc_input_id, BC_Types,
                                (int)bc->BC_Data_Int[0], (int)bc->BC_Data_Int[1],
                                (int)bc->BC_Data_Int[2], bc->BC_Data_Float[0]);
          break;

        case POROUS_GAS_BC:
          put_gas_flux_in_pores(func, d_func, x_dot, theta, delta_t, bc_input_id, BC_Types,
                                (int)bc->BC_Data_Int[0], (int)bc->BC_Data_Int[1],
                                (int)bc->BC_Data_Int[2], bc->BC_Data_Float[0],
                                bc->BC_Data_Float[1]);
          break;
        case POR_LIQ_FLUX_FILL_BC:
          por_liq_flux_fill(func, d_func, theta, delta_t, (int)bc->BC_Data_Int[0],
                            (int)bc->BC_Data_Int[1], (double)bc->BC_Data_Float[0],
                            (double)bc->BC_Data_Float[1], (double)bc->BC_Data_Float[2],
        ...
        case QRAD_REPULSE_ROLL_BC:
          qrad_surf_repulse(
              func, d_func, BC_Types[bc_input_id].BC_Data_Float[0],
              BC_Types[bc_input_id].BC_Data_Float[1], BC_Types[bc_input_id].BC_Data_Float[2],
              BC_Types[bc_input_id].BC_Data_Float[3], BC_Types[bc_input_id].BC_Data_Float[4],
              &(BC_Types[bc_input_id].BC_Data_Float[5]), &(BC_Types[bc_input_id].BC_Data_Float[8]),
              BC_Types[bc_input_id].BC_Data_Float[11], BC_Types[bc_input_id].BC_Data_Float[12],
              BC_Types[bc_input_id].BC_Data_Float[13], BC_Types[bc_input_id].BC_Data_Float[14]);
          break;

        case QUSER_BC:
          quser_surf(func, d_func, BC_Types[bc_input_id].u_BC, time_intermediate);
          break;
        case Q_LASER_WELD_BC:
          qlaser_surf(func, d_func, BC_Types[bc_input_id].u_BC, x, time_intermediate);
          break;

        case Q_VAPOR_BC:
          q_vapor(func, d_func, BC_Types[bc_input_id].u_BC, x, time_intermediate);
          break;
        ...
        case QSIDE_DIR_BC:
          qside_directional(func, d_func, bc->BC_Data_Float[0], bc->BC_Data_Float[1],
                            bc->BC_Data_Float[2]);
          break;

        case QSIDE_LS_BC:

          qside_ls(func, d_func, bc->BC_Data_Int[0], bc->BC_Data_Int[1], bc->BC_Data_Float[0],
                   bc->BC_Data_Float[1]);
        ...
        case T_CONTACT_RESIS_BC:
          qside_contact_resis(func, d_func, bc->BC_Data_Int[0], bc->BC_Data_Int[1],
                              bc->BC_Data_Float[0]);
          break;
        ...
        case API_VELOCITY_BC:
          acoustic_plane_transmission(
              func, d_func, time_intermediate, (int)bc->BC_Name, bc->BC_Data_Float[0],
              bc->BC_Data_Float[1], bc->BC_Data_Float[2], bc->BC_Data_Float[3], bc->BC_Data_Int[0]);
          break;
        case EM_ER_FARFIELD_DIRECT_BC:
        case EM_EI_FARFIELD_DIRECT_BC:
        case EM_HR_FARFIELD_DIRECT_BC:
        case EM_HI_FARFIELD_DIRECT_BC:
          apply_em_farfield_direct_vec(func, d_func, xi, (int)bc->BC_Name, bc->BC_Data_Float);
          break;
        case EM_ER_SOMMERFELD_BC:
        case EM_EI_SOMMERFELD_BC:
        case EM_HR_SOMMERFELD_BC:
        case EM_HI_SOMMERFELD_BC:
          apply_em_sommerfeld_vec(func, d_func, xi, (int)bc->BC_Name, bc->BC_Data_Float);
          break;
        case EM_ER_FREE_BC:
        case EM_EI_FREE_BC:
        case EM_HR_FREE_BC:
        case EM_HI_FREE_BC:
          apply_em_free_vec(func, d_func, xi, (int)bc->BC_Name);
          break;
        case E_ER_PLANEWAVE_BC:
        case E_EI_PLANEWAVE_BC:
          apply_ewave_planewave_vec(func, d_func, xi, (int)bc->BC_Name, bc->BC_Data_Float);
          break;
        ...
        case E_ER_2D_BC:
        case E_EI_2D_BC:
          apply_ewave_2D(func, d_func, xi, (int)bc->BC_Name);
          break;
        case LIGHTP_TRANS_BC:
        case LIGHTM_TRANS_BC:
        case LIGHTD_TRANS_BC:
          light_transmission(func, d_func, time_intermediate, (int)bc->BC_Name,
                             bc->BC_Data_Float[0], bc->BC_Data_Float[1], bc->BC_Data_Float[2],
                             bc->BC_Data_Int[0]);
          break;
        case LIGHTP_JUMP_BC:
        case LIGHTM_JUMP_BC:
          ...
          if (iapply) {
            qside_light_jump(func, d_func, time_intermediate, (int)bc->BC_Name, bc->BC_Data_Int[0],
                             bc->BC_Data_Int[1]);
          } else {
            skip_other_side = FALSE;
          }
        ...
        case LATENT_HEAT_BC:
          if (iapply) {
            lat_heat_bc(func, d_func, x_dot, theta, delta_t, bc_input_id, BC_Types);
            if (neg_elem_volume)
              return (status);
          }
          break;
        ...
        case LATENT_HEAT_INTERNAL_BC:
          if (iapply) {
            lat_heat_internal_bc(func, d_func, theta, delta_t, bc_input_id, BC_Types);
          }
          break;
        ...
        case PSPG_BC:
          ...
          PSPG_consistency_bc(func, d_func, x_dot, time_value, delta_t, theta, pg_data);
          break;

        case FILL_CA_BC:
          apply_CA_FILL(func, d_func, BC_Types[bc_input_id].BC_Data_Float[0]);
          break;

        case WETTING_TENSION_BC:
          apply_wetting_tension(func, d_func, BC_Types[bc_input_id].BC_Data_Float[0]);
          break;

        case WETTING_SPEED_LIN_BC:
          apply_wetting_velocity(
              func, d_func, elem_side_bc->id_side, ielem_type,
              BC_Types[bc_input_id].BC_Data_Float[3], &(BC_Types[bc_input_id].BC_Data_Float[4]),
              BC_Types[bc_input_id].BC_Data_Float[2], BC_Types[bc_input_id].BC_Data_Float[0],
              BC_Types[bc_input_id].BC_Data_Float[1]);
          break;
        case LINEAR_WETTING_SIC_BC:
          apply_linear_wetting_sic(
              func, d_func, delta_t, theta, elem_side_bc->id_side, ielem_type,
              BC_Types[bc_input_id].BC_Data_Float[3], &(BC_Types[bc_input_id].BC_Data_Float[4]),
              BC_Types[bc_input_id].BC_Data_Float[2], BC_Types[bc_input_id].BC_Data_Float[0],
              BC_Types[bc_input_id].BC_Data_Float[1], BC_Types[bc_input_id].BC_Data_Float[7]);
          break;
        case WETTING_SPEED_BLAKE_BC:
        case WETTING_SPEED_HOFFMAN_BC:
        case WETTING_SPEED_COX_BC:
        case WETTING_SPEED_SHIK_BC:
          apply_blake_wetting_velocity(
              func, d_func, (int)bc->BC_Name, elem_side_bc->id_side, ielem_type,
              BC_Types[bc_input_id].BC_Data_Float[4], BC_Types[bc_input_id].BC_Data_Float[3],
              BC_Types[bc_input_id].BC_Data_Float[0], BC_Types[bc_input_id].BC_Data_Float[1],
        ...
        case COX_DIRICHLET_BC: { /* trying to find wall velocity, assume velo_normal=0 */
          ...
          for (i1 = 0; i1 < Num_BC; i1++) {
            if (BC_Types[i1].BC_ID == bc->BC_ID) {
              ...
              case AIR_FILM_ROT_BC:
                wall_velo[0] =
                    BC_Types[i1].BC_Data_Float[1] * (fv->x[1] - BC_Types[i1].BC_Data_Float[3]);
                wall_velo[1] =
                    BC_Types[i1].BC_Data_Float[1] * (fv->x[0] - BC_Types[i1].BC_Data_Float[2]);
                break;
              case VELO_TANGENT_USER_BC:
                wall_velo[0] = velo_vary_fnc(UVARY_BC, fv->x[0], fv->x[1], fv->x[2],
                                             BC_Types[i1].u_BC, time_value);
                wall_velo[1] = velo_vary_fnc(VVARY_BC, fv->x[0], fv->x[1], fv->x[2],
                                             BC_Types[i1].u_BC, time_value);
                wall_velo[2] = velo_vary_fnc(WVARY_BC, fv->x[0], fv->x[1], fv->x[2],
                                             BC_Types[i1].u_BC, time_value);
                break;
              default:
                if (Debug_Flag > 1)
          ...
          apply_blake_wetting_velocity_sic(
              ...
              BC_Types[bc_input_id].BC_Data_Float[1], BC_Types[bc_input_id].BC_Data_Float[2],
              BC_Types[bc_input_id].BC_Data_Float[4], wall_velo, theta_max,
              BC_Types[bc_input_id].BC_Data_Int[3], BC_Types[bc_input_id].BC_Data_Float[9],
              BC_Types[bc_input_id].BC_Data_Float[10], BC_Types[bc_input_id].BC_Data_Float[11]);
        } break;
        ...
        case SHIK_DIRICHLET_BC:
          apply_blake_wetting_velocity_sic(
              ...
              BC_Types[bc_input_id].BC_Data_Float[1], BC_Types[bc_input_id].BC_Data_Float[2],
              BC_Types[bc_input_id].BC_Data_Float[4], &(BC_Types[bc_input_id].BC_Data_Float[5]),
              BC_Types[bc_input_id].BC_Data_Float[8], BC_Types[bc_input_id].BC_Data_Int[3],
              BC_Types[bc_input_id].BC_Data_Float[9], BC_Types[bc_input_id].BC_Data_Float[10],
              BC_Types[bc_input_id].BC_Data_Float[11]);
          break;

        case HYSTERESIS_WETTING_BC:
          apply_hysteresis_wetting_sic(func, d_func, delta_t, theta,
        ...
        case SH_MESH2_WEAK_BC:
          apply_sh_weak(func, d_func, xi, exo, BC_Types[bc_input_id].BC_Data_Float[0]);
        ...
        for (i = 0; i < (int)elem_side_bc->num_nodes_on_side; i++) {
          ...
          if ((BC_Types[bc_input_id].BC_Name == CAPILLARY_BC ||
              ...
              (Dolphin[pg->imtrx][I][VELOCITY1] >
               0)) { /* DRN: getting segfault for Q1P0 on Q2 mesh, does this fix it? */
            for (p = 0; p < bc_desc->vector; p++) {
              ...
              for (k = 0; k < MAX_VARIABLE_TYPES + MAX_CONC; k++) {
                for (j = 0; j < MDE; j++) {
                  d_func[p][k][j] =
                      d_cfunc[ei[pg->imtrx]->ln_to_first_dof[R_MOMENTUM1][id]][p][k][j];
                }
              }
            }
          }
          ...
          if ((BC_Types[bc_input_id].BC_Name == TENSION_SHEET_BC) &&
              (Dolphin[pg->imtrx][I][MESH_DISPLACEMENT1] > 0)) {
          ...
          if ((BC_Types[bc_input_id].BC_Name == SHEAR_TO_SHELL_BC) &&
              (Dolphin[pg->imtrx][I][R_SHELL_TENSION] > 0)) {
            func[0] = cfunc[ei[pg->imtrx]->ln_to_first_dof[R_SHELL_TENSION][id]][0];
            for (k = 0; k < MAX_VARIABLE_TYPES + MAX_CONC; k++) {
              for (j = 0; j < MDE; j++) {
                d_func[0][k][j] =
                    d_cfunc[ei[pg->imtrx]->ln_to_first_dof[R_SHELL_TENSION][id]][0][k][j];
              }
            }
          }
          ...
          if (stress_bc == 0) {
            ...
            for (p = 0; p < bc_desc->vector; p++) {
              ...
              if (index_eq >= 0) {
                ...
                if (BC_Types[bc_input_id].BC_Name != CAPILLARY_BC &&
                  ...
                  if (bc_desc->i_apply == CROSS_PHASE_DISCONTINUOUS) {
                    ...
                    if ((BC_Types[bc_input_id].BC_Data_Int[1] &&
                         Current_EB_ptr->Elem_Blk_Id == BC_Types[bc_input_id].BC_Data_Int[1]) ||
                     ...
                     */
                    if ((bf[eqn]->interpolation == I_Q2_D || bf[eqn]->interpolation == I_Q1_D ||
                         bf[eqn]->interpolation == I_Q2_D_LSA) &&
                        (bc->BC_Name == CONT_TANG_VEL_BC || bc->BC_Name == CONT_NORM_VEL_BC ||
                         bc->BC_Name == DISCONTINUOUS_VELO_BC || bc->BC_Name == VL_EQUIL_BC ||
                         bc->BC_Name == VL_POLY_BC || bc->BC_Name == SDC_STEFANFLOW_BC ||
                         bc->BC_Name == SDC_KIN_SF_BC || bc->BC_Name == LIGHTP_JUMP_BC ||
                         bc->BC_Name == LIGHTM_JUMP_BC || bc->BC_Name == T_CONTACT_RESIS_2_BC)) {
                      ldof_eqn += 1;
                    }
                 ...
                 *   cases
                 */
                if (bc_desc->method == WEAK_INT_SURF && bc->BC_Name != PSPG_BC &&
                    bc->BC_Name != VELO_SLIP_SOLID_BC && bc->BC_Name != ELEC_TRACTION_BC &&
                    bc->BC_Name != QSIDE_LS_BC && bc->BC_Name != LS_ADC_BC &&
                    bc->BC_Name != SHEAR_TO_SHELL_BC && bc->BC_Name != POR_LIQ_FLUX_FILL_BC &&
                    bc->BC_Name != DARCY_LUB_BC && bc->BC_Name != SHELL_TFMP_FREE_LIQ_BC &&
                    bc->BC_Name != SHELL_TFMP_NUM_DIFF_BC && bc->BC_Name != SH_SDET_BC &&
                    ...
                    if (new_way) {
                      ...
                      if (!af->Assemble_LSA_Mass_Matrix) {
                        ...
                        for (w = 0; w < jacCol.Num_lvdof; w++) {
                          ...
                          lec->J[LEC_J_INDEX(ieqn, pvar, ldof_eqn, j)] += tmp * jacCol.Jac_lvdof[w];
                        }
          ...
          if (stress_bc == 1) {
            ...
            for (imode = 0; imode < vn->modes; imode++) {
              for (p = 0; p < bc_desc->vector; p++) {
                 ...
                 */
                index_eq = bc_eqn_index_stress(id, I, bc_input_id, ei[pg->imtrx]->mn, p, imode,
                                               &eqn, &matID_apply, &vd);
                        ...
                        /* Case for variable type that is MASS_FRACTION */
                          ...
                          for (w = 0; w < pd->Num_Species_Eqn; w++) {
                            for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
                              lec->J[LEC_J_INDEX(ieqn, MAX_PROB_VAR + w, ldof_eqn, j)] +=
                                  weight * fv->sdet *
                                  d_func_stress[imode][p][MAX_VARIABLE_TYPES + w][j];
                            }
                          }
                        }
                      }
    ...
    for (ibc = 0; (bc_input_id = (int)elem_side_bc->BC_input_id[ibc]) != -1; ibc++) {
      ...
      if (bc_desc->method == bc_application) {
        ...
        case EM_ABSORBING_REAL_BC: {
          ...
          //cross_really_simple_vectors(fv->snormal, curl_wave_real, nxcurl);

          //dbl Ei_imag[DIM] = {-omega*cimag(wave[0]), -omega*cimag(wave[1]), -omega*cimag(wave[2])};
          //double nxnxE[DIM];
          //double nxE[DIM];
          //cross_really_simple_vectors(fv->snormal, Ei_imag, nxE);
          //cross_really_simple_vectors(fv->snormal, nxE, nxnxE);

          //func[0] = nxnxE[0];
          //func[1] = nxnxE[1];
          //func[2] = nxnxE[2];

          func[0] += omega*fv->em_ei[0];
          func[1] += omega*fv->em_ei[1];
          func[2] += omega*fv->em_ei[2];
          //func[0] += -omega*fv->em_ei[0];
          //func[1] += -omega*fv->em_ei[1];
          //func[2] += -omega*fv->em_ei[2];
          for (int j = 0; j < ei[pg->imtrx]->dof[EM_E1_IMAG]; j++) {
            d_func[0][EM_E1_IMAG][j] += omega * bf[EM_E1_IMAG]->phi_e[j][0];
            d_func[1][EM_E1_IMAG][j] += omega * bf[EM_E1_IMAG]->phi_e[j][1];
            d_func[2][EM_E1_IMAG][j] += omega * bf[EM_E1_IMAG]->phi_e[j][2];
          }
          //dbl omega = upd->Acoustic_Frequency;
          //dbl x = fv->x[0];
          //dbl y = fv->x[1];
        ...
        case EM_ABSORBING_IMAG_BC: {
          ...
          //dbl Ei_real[DIM] = {-omega*creal(wave[0]), -omega*creal(wave[1]), -omega*creal(wave[2])};
          //double nxnxE[DIM];
          //double nxE[DIM];
          //cross_really_simple_vectors(fv->snormal, Ei_real, nxE);
          //cross_really_simple_vectors(fv->snormal, nxE, nxnxE);
          ...
          func[0] -= omega*fv->em_er[0];
          func[1] -= omega*fv->em_er[1];
          func[2] -= omega*fv->em_er[2];
          for (int j = 0; j < ei[pg->imtrx]->dof[EM_E1_REAL]; j++) {
            d_func[0][EM_E1_REAL][j] -= omega * bf[EM_E1_REAL]->phi_e[j][0];
            d_func[1][EM_E1_REAL][j] -= omega * bf[EM_E1_REAL]->phi_e[j][1];
            d_func[2][EM_E1_REAL][j] -= omega * bf[EM_E1_REAL]->phi_e[j][2];
          }
          //dbl omega = upd->Acoustic_Frequency;
          //dbl x = fv->x[0];
          //dbl y = fv->x[1];
...
int equation_index_auto_rotate(const ELEM_SIDE_BC_STRUCT *elem_side_bc,
  ...
  for (int i = 0; i < 3; i++) {
    double dot = 0;
    for (int j = 0; j < 3; j++) {
      dot += goma_automatic_rotations.rotation_nodes[I].rotated_coord[i]->normal->data[j] *
             fv->snormal[j];
    }
    if (fabs(dot) > max_dot) {
      rot_dir = i;
...
void apply_table_wic_bc(double func[],

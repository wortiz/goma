...
int brk_exo_file(int num_pieces, char *Brk_File, char *Exo_File) {
  ...
  if (num_pieces < 1) {
    EH(-1, "For number of pieces, specify one positive nonzero integer.");
  }
  ...
  if (!user_params_file_exists) {
    ...
    if (fs_up == NULL) {
      sr = sprintf(err_msg, "Problem opening \"%s\" [Chaco input] for create/append.",
  ...
  rd_exo(
      mono, in_exodus_file_name, 0,
      (EXODB_ACTION_RD_INIT + EXODB_ACTION_RD_MESH + EXODB_ACTION_RD_RES0 + EXODB_ACTION_RD_RESG));
  ...
  pnd = (Node_Description **)smalloc(MAX_NODE_KINDS * sizeof(Node_Description *));
  ...
  for (i = 0; i < length_node_node; i++) {
    ...
    fprintf(stderr, "%3d-%3d interaction contributes: %d %d -> %d <-\n", eqn_node_names[i] + 1,
            var_node_names[i] + 1, nnz_contribute[i], nat_contribute[i], ccs_contribute[i]);
#endif
  }
  ...
  if (write_monolith_graph) {
    wr_graph_file(out_graph_file_name, in_file_name, in_exodus_file_name, ne, nn, total_dofs, tnnz,
                  tnat, one_neighbor_per_line, pnn, dragon, var_node_names, nat_contribute,
                  ccs_contribute);
  }
  ...
  for (n = 0; n < nn; n++) {
    ...
    for (j = pnn[n]; j < pnn[n + 1]; j++) {
      ...
      if (ccs_contribute[j] != 0 && var_node_names[j] != n) {
...
#endif
    err = interface(nvtxs, start, adjacency, vwgts, ewgts, x, y, z, outassignname, outfilename,
                    assignment, architecture, ndims_tot, mesh_dims, goal, global_method,
    ...
    if (is_shell_element(mono, e)) {
      ...
      if (num_friends == 1) {
        ...
        for (n = mono->elem_node_pntr[fe]; n < mono->elem_node_pntr[fe + 1]; n++) {
          ...
          if (!match)
            BULL(node, unique_nodes, num_unique_nodes);
          if (match)
            BULL(node, shared_nodes, num_shared_nodes);
        }
        ...
        for (n = 0; n < num_shared_nodes; n++) {
          ...
          for (i = 0; i < 3; i++) {
            ...
            if (!is_node_in_element(mono, gnn, e))
              break;
          }
   ...
   *             membership begins
   *
   * if ( node_set_membership[node+1] - node_set_membership[node] ) > 1, then this is more than an
   *internal node, it's part of a boundary/external interaction.
   *
  ...
  for (s = 0; s < num_pieces; s++) {
    ...
    D->global_node_description = (int **)smalloc(num_kinds_nodes * SZPINT);

    D->global_node_description[0] = (int *)smalloc(num_kinds_nodes * LEN_NODE_DESCRIPTION * SZ_INT);
    for (i = 1; i < num_kinds_nodes; i++) {
      D->global_node_description[i] = (D->global_node_description[i - 1] + LEN_NODE_DESCRIPTION);
    }
    ...
    for (k = 0; k < num_kinds_nodes; k++) {
      ...
      for (i = 0; i < MAX_PROB_VAR; i++) {
        D->global_node_description[k][1 + MAX_PROB_VAR + 3 * i] = pnd[k]->eqnvar_wts[i][0];
        D->global_node_description[k][1 + MAX_PROB_VAR + 3 * i + 1] = pnd[k]->eqnvar_wts[i][1];
        D->global_node_description[k][1 + MAX_PROB_VAR + 3 * i + 2] = pnd[k]->eqnvar_wts[i][2];
      }
    }
    ...
    num_universe_nodes = (num_internal_nodes + num_boundary_nodes + num_external_nodes);
    ...
    if (index_internal_node != num_internal_nodes) {
      fprintf(stderr, "Inconsistent count of internal nodes, set %d (%d != %d)\n", s,
              index_internal_node, num_internal_nodes);
      exit(0);
    }

    if (index_boundary_node != num_boundary_nodes) {
      fprintf(stderr, "Inconsistent count of boundary nodes, set %d (%d != %d)\n", s,
              index_boundary_node, num_boundary_nodes);
      exit(0);
    }

    if (index_external_node != num_external_nodes) {
      fprintf(stderr, "Inconsistent count of external nodes, set %d (%d != %d)\n", s,
              index_external_node, num_external_nodes);
      exit(0);
    }
    ...
    for (i = 0; i < num_internal_nodes; i++)
      printf("%4d", internal_nodes[i] + 1);
    printf("\n   Boundary nodes:");
    for (i = 0; i < num_boundary_nodes; i++)
      printf("%4d", boundary_nodes[i] + 1);
    printf("\n   External nodes:");
    for (i = 0; i < num_external_nodes; i++)
      printf("%4d", external_nodes[i] + 1);
    printf("\n");
#endif
    ...
    for (i = 0; i < num_internal_nodes; i++) {
      ...
      internal_dofweight += (node_dof0[global_node_index + 1] - node_dof0[global_node_index]);
    }

    for (i = 0; i < num_boundary_nodes; i++) {
      global_node_index = boundary_nodes[i];
      boundary_dofweight += (node_dof0[global_node_index + 1] - node_dof0[global_node_index]);
    }

    for (i = 0; i < num_external_nodes; i++) {
      global_node_index = external_nodes[i];
      external_dofweight += (node_dof0[global_node_index + 1] - node_dof0[global_node_index]);
    }
    ...
    */
    fprintf(stderr, "num_external_nodes = %d\n", num_external_nodes);
#endif
    proc_sort(external_nodes, num_external_nodes, nvtxs, assignment);
#ifdef DEBUG
    ...
    */
    ...
    for (i = 1; i < num_internal_nodes; i++) {
      if (internal_nodes[i] < internal_nodes[i - 1]) {
        sr = sprintf(err_msg, "internal_nodes[] nonmonotone in Set %d", s);
        EH(-1, err_msg);
      }
    }
    ...
    for (i = 1; i < num_boundary_nodes; i++) {
      if (boundary_nodes[i] < boundary_nodes[i - 1]) {
        sr = sprintf(err_msg, "boundary_nodes[] nonmonotone in Set %d", s);
        EH(-1, err_msg);
      }
    }
    ...
    for (i = 0; i < num_external_nodes; i++) {
      proc_nodes[num_internal_nodes + num_boundary_nodes + i] = external_nodes[i];
    }

#ifdef DEBUG
    for (i = 0; i < num_universe_nodes; i++) {
      fprintf(stderr, "proc_nodes[%d] = %d (%d)\n", i, proc_nodes[i], proc_nodes[i] + 1);
    }
#endif
...
#ifdef DEBUG
    fprintf(stderr, "Proc/Set %d has %d b nodes to send to:", s, num_boundary_nodes);
...
#ifdef DEBUG
    fprintf(stderr, "Proc/Set %d has %d e nodes living on:", s, num_external_nodes);
    ...
    if (proc_ne < 1 || proc_ne > ne) {
      sr = sprintf(err_msg, "Bad element count of %d in Set %d", proc_ne, s);
      EH(-1, err_msg);
    }
    ...
    for (ieb = 0; ieb < proc_neb; ieb++) {
      ...
      fprintf(stderr, "In Setproc %d, proc_eb_ptr[%d] for (ebid=%d) = %d\n", s, ieb,
              proc_eb_id[ieb], proc_eb_ptr[ieb]);
    }
    ...
    for (e = 0; e < proc_ne; e++) {
      ...
      fprintf(stderr, "Set %d elem[%d](=(%d)) is %s in local EB[%d]=%d\n", s, e, elem + 1,
              (private_elem ? ("priv") : ("shar")), proc_eb_index, eb_id);
#endif
    ...
    for (ieb = 0; ieb < proc_neb; ieb++) {
      ...
      for (e = proc_eb_ptr[ieb]; e < proc_eb_ptr[ieb + 1]; e++) {
        ...
        for (l = ep[elem]; l < ep[elem + 1]; l++) {
          ...
          private_elem &= !(IUL(node, external_nodes, num_external_nodes));
        }
        ...
        fprintf(stderr, "proc_elems[%d] = (%d) is %s.\n", e, elem + 1,
                (private_elem ? "private" : "shared"));
#endif
      }

      if (index_priv != private_elem_count[ieb]) {
        sr = sprintf(err_msg, "index_priv=%d != private_elem_count[%d]=%d EBID %d", index_priv, ieb,
                     private_elem_count[ieb], eb_id);
        EH(-1, err_msg);
      }

      if (index_priv + index_shar != proc_eb_ptr[ieb + 1] - proc_eb_ptr[ieb]) {
        sr = sprintf(err_msg, "#priv elems(%d)+#shar elems(%d) != total(%d)", index_priv,
                     index_shar, proc_eb_ptr[ieb + 1] - proc_eb_ptr[ieb]);
        EH(-1, err_msg);
      }
    ...
    for (ieb = 0; ieb < proc_neb; ieb++) {
      fprintf(stderr, "EB[%d] = %d has %d/%d (private/total) elems.\n", ieb, proc_eb_id[ieb],
              private_elem_count[ieb], proc_eb_ptr[ieb + 1] - proc_eb_ptr[ieb]);
    }
    fprintf(stderr, "\n");
#endif
    ...
    for (ieb = 0; ieb < proc_neb; ieb++) {
      ...
      if (index_max_private_elems != ieb) {
        ISWAP(private_elem_count[ieb], private_elem_count[index_max_private_elems]);

        ISWAP(proc_eb_id[ieb], proc_eb_id[index_max_private_elems]);

        ISWAP(ebi[ieb], ebi[index_max_private_elems]);
      }
    }
    ...
    for (ieb = 0; ieb < proc_neb; ieb++) {
      ...
      for (i = 0; i < len; i++) {
        ...
        fprintf(stderr, "new_proc_elems[%d+%d] = %d (proc_elem[%d+%d])\n", new_start, i,
                new_proc_elems[new_start + i], old_start, i);
#endif
      }
    ...
    proc_ns_node_list_index_global = (int *)smalloc(INIT_PROC_NS_NODE_LIST_LENGTH * SZ_INT);

    size_proc_ns_node_list = INIT_PROC_NS_NODE_LIST_LENGTH;

    INIT_IVEC(proc_ns_node_list, -1, INIT_PROC_NS_NODE_LIST_LENGTH);

    proc_ns_distfact_list = (dbl *)smalloc(INIT_PROC_NS_DISTFACT_LIST_LENGTH * SZ_DBL);

    proc_ns_distfact_list_index_global = (int *)smalloc(INIT_PROC_NS_DISTFACT_LIST_LENGTH * SZ_INT);
    ...
    for (index_ns = 0; index_ns < mono->num_node_sets; index_ns++) {
      ...
      if (gnnns < 1) {
        sr = sprintf(err_msg, "nodeset %d has %d nodes - punting!", mono->ns_id[index_ns],
...
#ifdef DEBUG
        fprintf(stderr, "NS[%d]=%d, Looking for gn[%d]=%d\n", index_ns, mono->ns_id[index_ns], j,
                node + 1);
#endif
        n = get_internal_boundary_index(node, proc_nodes, num_internal_nodes, num_boundary_nodes);
        if (n != -1) {
          ...
          if (proc_ns_node_len > size_proc_ns_node_list - 1) {
            size_proc_ns_node_list += INIT_PROC_NS_NODE_LIST_LENGTH;
            proc_ns_node_list = (int *)realloc(proc_ns_node_list, SZ_INT * size_proc_ns_node_list);
            proc_ns_node_list_index_global =
                (int *)realloc(proc_ns_node_list_index_global, SZ_INT * size_proc_ns_node_list);
          }
          ...
          if (gndfpn > 0) {
            ...
            for (m = 0; m < gndfpn; m++) {
              proc_ns_distfact_list[proc_ns_distfact_len] = mono->ns_distfact_list[where + m];
              ...
              proc_ns_distfact_list_index_global[proc_ns_distfact_len] = where + m;

              proc_ns_distfact_len++;

              if (proc_ns_distfact_len > size_proc_ns_distfact_list - 1) {
                size_proc_ns_distfact_list += INIT_PROC_NS_DISTFACT_LIST_LENGTH;
                proc_ns_distfact_list =
                    (dbl *)realloc(proc_ns_distfact_list, SZ_DBL * size_proc_ns_distfact_list);
                proc_ns_distfact_list_index_global = (int *)realloc(
                    proc_ns_distfact_list_index_global, SZ_INT * size_proc_ns_distfact_list);
              }
            }
          }
        }
      ...
      if (ns_touch) {
        ...
        if (proc_num_node_sets > size_proc_num_ns - 1) {
          ...
          proc_ns_id = ((int *)realloc(proc_ns_id, SZ_INT * size_proc_num_ns));

          proc_ns_num_nodes = ((int *)realloc(proc_ns_num_nodes, SZ_INT * size_proc_num_ns));

          proc_ns_num_distfacts =
              ((int *)realloc(proc_ns_num_distfacts, size_proc_num_ns * SZ_INT));

          proc_ns_node_index = ((int *)realloc(proc_ns_node_index, SZ_INT * size_proc_num_ns));

          proc_ns_distfact_index =
              ((int *)realloc(proc_ns_distfact_index, SZ_INT * size_proc_num_ns));
        }
      }
    }
    ...
    proc_ss_elem_list = (int *)smalloc(INIT_PROC_SS_SIDE_LIST_LENGTH * SZ_INT);
    proc_ss_side_list = (int *)smalloc(INIT_PROC_SS_SIDE_LIST_LENGTH * SZ_INT);
    proc_ss_elem_list_index_global = (int *)smalloc(INIT_PROC_SS_SIDE_LIST_LENGTH * SZ_INT);

    size_proc_ss_elem_list = INIT_PROC_SS_SIDE_LIST_LENGTH;

    proc_ss_distfact_list = (dbl *)smalloc(INIT_PROC_SS_DISTFACT_LIST_LENGTH * SZ_DBL);

    proc_ss_distfact_list_index_global = (int *)smalloc(INIT_PROC_SS_DISTFACT_LIST_LENGTH * SZ_INT);
    ...
    for (index_ss = 0; index_ss < mono->num_side_sets; index_ss++) {
      ...
      at_least_one = FALSE; /* assume this ssid is not here
                             ...
                             */
#ifdef DEBUG
      fprintf(stderr, "Checking %d elem/sides for global sideset SS[%d] = %d\n\n",
              mono->ss_num_sides[index_ss], index_ss, mono->ss_id[index_ss]);
#endif
      /*
      ...
      for (j = 0; j < mono->ss_num_sides[index_ss]; j++) {
...
#ifdef DEBUG
        fprintf(stderr, "Checking: elem=(%d) side=%d j=%d ", elem + 1, side, j);
#endif
        ...
        if (local_elem > -1) {
#ifdef DEBUG
          fprintf(stderr, "Adding. global SS[%d]=%d, elem=(%d), sd=%d !\n", index_ss,
                  mono->ss_id[index_ss], elem + 1, mono->ss_side_list[begin + j]);
          fprintf(stderr, "Local element index = %d\n", local_elem);
#endif
          at_least_one = TRUE;
          ...
          proc_ss_elem_list_index_global[proc_ss_elem_len] = begin + j;
          ...
          if (proc_ss_elem_len > size_proc_ss_elem_list - 1) {
            ...
            proc_ss_elem_list = (int *)realloc(proc_ss_elem_list, SZ_INT * size_proc_ss_elem_list);

            proc_ss_elem_list_index_global =
                (int *)realloc(proc_ss_elem_list_index_global, SZ_INT * size_proc_ss_elem_list);

            proc_ss_side_list = (int *)realloc(proc_ss_side_list, SZ_INT * size_proc_ss_elem_list);
          }
          ...
          if (mono->ss_num_distfacts[index_ss] > 0) {
            for (l = mono->ss_node_side_index[index_ss][j];
              ...
              proc_ss_distfact_list[proc_ss_distfact_len] = mono->ss_distfact_list[begin_df + l];

              /*
               * And now record where we got it, too!
               */

              proc_ss_distfact_list_index_global[proc_ss_distfact_len] = begin_df + l;

              proc_ss_distfact_len++;

              if (proc_ss_distfact_len > size_proc_ss_distfact_list - 1) {
                size_proc_ss_distfact_list += INIT_PROC_SS_DISTFACT_LIST_LENGTH;

                proc_ss_distfact_list =
                    (dbl *)realloc(proc_ss_distfact_list, SZ_DBL * size_proc_ss_distfact_list);

                proc_ss_distfact_list_index_global = (int *)realloc(
                    proc_ss_distfact_list_index_global, SZ_INT * size_proc_ss_distfact_list);
              }
            }
          }
        }
      ...
      if (at_least_one) /* mark this ssid  */
      {
        ...
        if (proc_num_side_sets > size_proc_num_ss - 1) {
          ...
          proc_ss_id = ((int *)realloc(proc_ss_id, SZ_INT * size_proc_num_ss));

          proc_ss_num_sides = ((int *)realloc(proc_ss_num_sides, SZ_INT * size_proc_num_ss));

          proc_ss_num_distfacts =
              ((int *)realloc(proc_ss_num_distfacts, SZ_INT * size_proc_num_ss));

          proc_ss_elem_index = ((int *)realloc(proc_ss_elem_index, SZ_INT * size_proc_num_ss));

          proc_ss_distfact_index =
              ((int *)realloc(proc_ss_distfact_index, SZ_INT * size_proc_num_ss));
        }
      }
    }
    ...
    if (E->num_info > 0) {
      E->info = (INFO_Record *)smalloc(E->num_info * sizeof(INFO_Record));
    ...
    for (i = 0; i < proc_neb; i++) {
      if ((E->eb_num_elems[i] * E->eb_num_nodes_per_elem[i]) > 0) {
        E->eb_conn[i] = (int *)smalloc(E->eb_num_elems[i] * E->eb_num_nodes_per_elem[i] * SZ_INT);
      }

      if ((E->eb_num_elems[i] * E->eb_num_attr[i]) > 0) {
        E->eb_attr[i] = (dbl *)smalloc(E->eb_num_elems[i] * E->eb_num_attr[i] * SZ_DBL);
      }
    }
    ...
    for (ieb = 0; ieb < proc_neb; ieb++) {
      ...
      for (e = 0; e < length; e++) {
        ...
        for (l = ep[elem]; l < ep[elem + 1]; l++) {
          ...
          n = get_node_index(node, proc_nodes, num_internal_nodes, num_boundary_nodes,
                             num_external_nodes);
#ifdef DEBUG
          fprintf(stderr, "gieb=%d, gelem=%d, (eb:%d), gnode=(%d)\n, n=[%d]\n", eb_index, elem,
                  e_name, node + 1, n);
#endif
        ...
        for (a = 0; a < lnattr; a++) {
          E->eb_attr[ieb][e * lnattr + a] = mono->eb_attr[eb_index][e_name * gnattr + a];
        }
      }
    }
    ...
    if (E->ns_num_props > 1 && E->num_node_sets > 0 &&
      ...
      for (ins = 0; ins < E->num_node_sets; ins++) {
        g_ns_index = in_list(E->ns_id[ins], 0, mono->num_node_sets, mono->ns_id);
    ...
    if (E->ss_num_props > 1 && E->num_side_sets > 0 &&
      ...
      for (iss = 0; iss < E->num_side_sets; iss++) {
        g_ss_index = in_list(E->ss_id[iss], 0, mono->num_side_sets, mono->ss_id);
    ...
    if (E->eb_num_props > 1 && E->num_elem_blocks > 0 &&
      ...
      for (ieb = 0; ieb < E->num_elem_blocks; ieb++) {
        g_eb_index = in_list(E->eb_id[ieb], 0, mono->num_elem_blocks, mono->eb_id);
    ...
    if (num_send_procs != num_recv_procs) {
      sr = sprintf(err_msg, "! num_send_procs = %d, num_recv_procs = %d.", num_send_procs,
                   num_recv_procs);
      EH(-1, err_msg);
    }
    ...
    D->ptr_set_membership = (int *)smalloc(D->len_ptr_set_membership * sizeof(int));
    ...
    D->eb_elem_type_global = (char **)smalloc(D->num_elem_blocks_global * sizeof(char *));

    D->eb_num_attr_global = (int *)smalloc(D->num_elem_blocks_global * sizeof(char *));

    D->eb_num_nodes_per_elem_global = (int *)smalloc(D->num_elem_blocks_global * sizeof(char *));
    ...
    D->eb_elem_type_global[0] =
        (char *)smalloc((D->num_elem_blocks_global) * (D->len_string) * sizeof(char));
    ...
    for (i = 1; i < D->num_elem_blocks_global; i++) {
      D->eb_elem_type_global[i] = D->eb_elem_type_global[i - 1] + D->len_string;
    }
    ...
    if (mono->eb_num_props > 1) {
      D->eb_prop_global = (int **)smalloc(mono->eb_num_props * sizeof(int *));

      D->eb_prop_global[0] =
          (int *)smalloc(mono->eb_num_props * mono->num_elem_blocks * sizeof(int));

      for (i = 1; i < mono->eb_num_props; i++) {
        D->eb_prop_global[i] = (D->eb_prop_global[i - 1] + mono->num_elem_blocks);
      }
    ...
    if (D->len_elem_var_tab_global > 0) {
      D->elem_var_tab_global = (int *)smalloc(D->len_elem_var_tab_global * sizeof(int));
    }
    ...
    D->ns_node_index_global = (int *)smalloc(mono->num_node_sets * sizeof(int));
    for (i = 0; i < mono->num_node_sets; i++) {
      D->ns_node_index_global[i] = mono->ns_node_index[i];
    }

    D->ns_distfact_index_global = (int *)smalloc(mono->num_node_sets * sizeof(int));
    for (i = 0; i < mono->num_node_sets; i++) {
      D->ns_distfact_index_global[i] = mono->ns_distfact_index[i];
    }

    if (mono->ns_num_props > 1) {
      D->ns_prop_global = (int **)smalloc(mono->ns_num_props * sizeof(int *));

      D->ns_prop_global[0] = (int *)smalloc(mono->ns_num_props * mono->num_node_sets * sizeof(int));
      for (i = 1; i < mono->ns_num_props; i++) {
        D->ns_prop_global[i] = (D->ns_prop_global[i - 1] + mono->num_node_sets);
      }
    ...
    D->ns_node_list_index_global = (int *)smalloc(E->ns_node_len * sizeof(int));
    ...
    D->ns_distfact_list_index_global = (int *)smalloc(E->ns_distfact_len * sizeof(int));
    for (i = 0; i < E->ns_distfact_len; i++) {
      D->ns_distfact_list_index_global[i] = proc_ns_distfact_list_index_global[i];
    }
    ...
    D->ss_distfact_index_global = (int *)smalloc(mono->num_side_sets * sizeof(int));
    D->ss_elem_index_global = (int *)smalloc(mono->num_side_sets * sizeof(int));
    ...
    D->ss_elem_list_index_global = (int *)smalloc(E->ss_elem_len * sizeof(int));
    D->ss_distfact_list_index_global = (int *)smalloc(E->ss_distfact_len * sizeof(int));

    for (i = 0; i < E->ss_distfact_len; i++) {
      D->ss_distfact_list_index_global[i] = proc_ss_distfact_list_index_global[i];
    }

    for (i = 0; i < E->ss_elem_len; i++) {
      D->ss_elem_list_index_global[i] = proc_ss_elem_list_index_global[i];
    }

    if (mono->ss_num_props > 1) {
      D->ss_prop_global = (int **)smalloc(mono->ss_num_props * sizeof(int *));

      D->ss_prop_global[0] = (int *)smalloc(mono->ss_num_props * mono->num_side_sets * sizeof(int));
      for (i = 1; i < mono->ss_num_props; i++) {
        D->ss_prop_global[i] = (D->ss_prop_global[i - 1] + mono->num_side_sets);
      }
    ...
    if (mono->num_side_sets > 0) {
      ...
      for (int ss_id = 0; ss_id < mono->num_side_sets; ss_id++) {
        ...
        for (int elem_index = mono->ss_elem_index[ss_id];
             elem_index < (mono->ss_elem_index[ss_id] + mono->ss_num_sides[ss_id]); elem_index++) {
    ...
    if (E->num_elem_vars > 0) {
      ...
      E->elem_var_tab = (int *)smalloc(E->num_elem_vars * E->num_elem_blocks * sizeof(int));
      /*
      ...
      for (ieb = 0; ieb < E->num_elem_blocks; ieb++) {
        ...
        for (i = 0; i < nev; i++) {
          E->elem_var_tab[ieb * nev + i] = mono->elem_var_tab[g_eb_index * nev + i];
        }
      }
    }
    ...
    if (E->num_glob_vars > 0) {
      ...
      E->exoid = ex_open(E->path, E->cmode, &E->comp_wordsize, &E->io_wordsize, &E->version);
      ...
      status =
          ex_put_variable_names(E->exoid, EX_GLOBAL, mono->num_glob_vars, mono->glob_var_names);
      EH(status, "ex_put_variable_names global");

      alloc_exo_gv(mono, 1);

      for (i = 0; i < mono->num_gv_time_indeces; i++) {
        status = ex_get_var(mono->exoid, i + 1, EX_GLOBAL, 1, 1, mono->num_glob_vars, mono->gv[0]);
        EH(status, "ex_get_var global");

        status = ex_put_var(E->exoid, i + 1, EX_GLOBAL, 1, 0, mono->num_glob_vars, mono->gv[0]);
    ...
    if (E->num_elem_vars >
        0) /* Save me baby Jesus ! Imma try to clone the nv section and hope for the best */
    {
  ...
  if (add_decomp_plot_vars) {
    ...
    if (mono->num_nv_time_indeces > 1) {
      EH(-1, "Reconcile new nodal results with multiple time previous results.");
    }
    ...
    if (nnv > 0) {
      mono->node_var_names = (char **)realloc(mono->node_var_names, len * sizeof(char *));
    } else {
    ...
    if (mono->num_ev_time_indeces > 1) {
      EH(-1, "Reconcile new elemvar results with multiple time previous results.");
    }
    ...
    if (nev > 0) {
      mono->elem_var_names = (char **)realloc(mono->elem_var_names, len * sizeof(char *));
      mono->ev[0] = (dbl **)realloc(mono->ev[0], (len * neb) * sizeof(dbl *));
    } else {
      mono->elem_var_names = (char **)smalloc(len * sizeof(char *));
      mono->num_ev_time_indeces = 1;
      mono->ev_time_indeces = (int *)smalloc(mono->num_ev_time_indeces * sizeof(int));
      mono->ev = (dbl ***)smalloc(mono->num_ev_time_indeces * sizeof(dbl **));
    ...
    for (i = nev; i < len; i++) {
      for (j = 0; j < neb; j++) {
        index = j * len + i;
        mono->ev[0][index] = (dbl *)smalloc(mono->eb_num_elems[j] * sizeof(dbl));
      }
    }
    ...
    for (blk_index = 0; blk_index < neb; blk_index++) {
      for (e = 0; e < mono->eb_num_elems[blk_index]; e++, i++) {
        ...
        mono->ev[0][blk_index * len + index_eoa][e] = (double)num_element_procs;

        /*
         * Ownership...
         */

        mono->ev[0][blk_index * len + index_eown][e] = (double)element_owner[i];
      }
    }
  ...
  if (denominator != 0) {
    fprintf(stdout, "Sam's dof weight figure of merit: (b+e)/(i+b) = %g\n",

...
int rd_exo(Exo_DB *x, /* def'd in exo_struct.h */
  ...
  if (x->state == EXODB_STATE_GRND) {
    ...
    if (len > FILENAME_MAX_ACK) {
      sr = sprintf(err_msg, "EXODUS II pathname \"%s\" too long (%d>%d).", fn, len,
                   FILENAME_MAX_ACK);
  ...
  if (err > FILENAME_MAX_ACK) {
    sr = sprintf(err_msg, "EXODUS II pathname \"%s\" too long (%d>%d).", fn, err, FILENAME_MAX_ACK);
    GOMA_EH(GOMA_ERROR, err_msg);
  }
  ...
  x->exoid = ex_open(x->path, x->mode, &(x->comp_wordsize), &(x->io_wordsize), &(x->version));
#ifdef PARALLEL
  ...
  if (task & EXODB_ACTION_RD_INIT) {
    ...
    */
    ...
    status = ex_get_init(x->exoid, x->title, &x->num_dim, &x->num_nodes, &x->num_elems,
  ...
  if (task & EXODB_ACTION_RD_MESH) {
     ...
     * the storage space.
     */
    Element_Blocks = (ELEM_BLK_STRUCT *)alloc_struct_1(ELEM_BLK_STRUCT, x->num_elem_blocks);
    ...
    if (x->num_elem_blocks > 0) {
       ...
       */
      for (i = 0; i < x->num_elem_blocks; i++) {
        status = ex_get_block(x->exoid, EX_ELEM_BLOCK, x->eb_id[i], x->eb_elem_type[i],
                              &x->eb_num_elems[i], &x->eb_num_nodes_per_elem[i], 0, 0,
                              &x->eb_num_attr[i]);
        ...
        if ((x->eb_num_elems[i] * x->eb_num_nodes_per_elem[i]) > 0) {
           ...
           */
          x->eb_conn[i] = (int *)smalloc(x->eb_num_elems[i] * x->eb_num_nodes_per_elem[i] * si);

          status = ex_get_conn(x->exoid, EX_ELEM_BLOCK, x->eb_id[i], x->eb_conn[i], 0, 0);
        ...
        if ((x->eb_num_elems[i] * x->eb_num_attr[i]) > 0) {
          x->eb_attr[i] = (dbl *)smalloc(x->eb_num_elems[i] * x->eb_num_attr[i] * sd);
          status = ex_get_attr(x->exoid, EX_ELEM_BLOCK, x->eb_id[i], x->eb_attr[i]);
    ...
    if (x->num_node_sets > 0) {
      ...
      status = ex_get_concat_sets(x->exoid, EX_NODE_SET, &ns_specs);
    ...
    if (x->num_side_sets > 0) {
      ...
      status = ex_get_concat_sets(x->exoid, EX_SIDE_SET, &ss_specs);
      ...
      for (i = 0; i < x->num_side_sets; i++) {
        ...
        status = ex_get_side_set_node_list(x->exoid, x->ss_id[i], x->ss_node_cnt_list[i],
                                           x->ss_node_list[i]);
        /*
        ...
        for (j = 0; j < x->ss_num_sides[i]; j++) {
          x->ss_node_side_index[i][j + 1] =
              (x->ss_node_side_index[i][j] + x->ss_node_cnt_list[i][j]);
        }
      }
    }
    ...
    if (x->ns_num_props > 0) {
      ...
      for (i = 0; i < x->ns_num_props; i++) {
        status = ex_get_prop_array(x->exoid, EX_NODE_SET, x->ns_prop_name[i], x->ns_prop[i]);
    ...
    if (x->ss_num_props > 0) {
      ...
      for (i = 0; i < x->ss_num_props; i++) {
        status = ex_get_prop_array(x->exoid, EX_SIDE_SET, x->ss_prop_name[i], x->ss_prop[i]);
    ...
    if (x->eb_num_props > 0) {
      ...
      for (i = 0; i < x->eb_num_props; i++) {
        status = ex_get_prop_array(x->exoid, EX_ELEM_BLOCK, x->eb_prop_name[i], x->eb_prop[i]);
  ...
  if (task & EXODB_ACTION_RD_RES0) {
    ...
    if (x->num_glob_vars > 0) {
      ...
      status = ex_get_variable_names(x->exoid, EX_GLOBAL, x->num_glob_vars, x->glob_var_names);
    ...
    if (x->num_elem_vars > 0) {
      ...
      status = ex_get_variable_names(x->exoid, EX_ELEM_BLOCK, x->num_elem_vars, x->elem_var_names);
    ...
    if (x->num_node_vars > 0) {
      ...
      status = ex_get_variable_names(x->exoid, EX_NODAL, x->num_node_vars, x->node_var_names);
    ...
    if (x->num_elem_vars > 0) {
      x->elem_var_tab = (int *)smalloc(x->num_elem_vars * x->num_elem_blocks * si);

      status = ex_get_truth_table(x->exoid, EX_ELEM_BLOCK, x->num_elem_blocks, x->num_elem_vars,
                                  x->elem_var_tab);
  ...
  if (task & EXODB_ACTION_RD_RESN) {
    ...
    if (x->num_nv_time_indeces > 0 && x->num_nv_indeces > 0 && x->num_nodes > 0 &&
        x->num_node_vars > 0) {
      ...
      for (i = 0; i < x->num_nv_time_indeces; i++) {
        ...
        for (j = 0; j < x->num_nv_indeces; j++) {
          ...
          status = ex_get_var(x->exoid, time_index, EX_NODAL, nodal_var_index, 1, x->num_nodes,
                              x->nv[i][j]);
          GOMA_EH(status, "ex_get_nodal_var");
        }
      }
  ...
  if (task & EXODB_ACTION_RD_RESG) {
    ...
    if (x->num_gv_time_indeces > 0 && x->num_glob_vars > 0) {
      ...
      for (i = 0; i < x->num_gv_time_indeces; i++) {
        ...
        status = ex_get_var(x->exoid, time_index, EX_GLOBAL, 1, 1, x->num_glob_vars, x->gv[i]);
  ...
  if (task & EXODB_ACTION_RD_RESE) {
    ...
    if (x->num_ev_time_indeces > 0 && x->num_elem_vars > 0 && x->num_elems > 0) {
 ...
 *
 */
int free_exo(Exo_DB *x) /* pointer to EXODUS II FE db structure */
{
  int i;
  int j;
  ...
  if (x->ns_num_props > 0 && x->ns_prop_name != NULL && x->ns_prop != NULL) {
    for (i = 0; i < x->ns_num_props; i++) {
  ...
  if (x->ss_num_props > 0 && x->ss_prop_name != NULL && x->ss_prop != NULL) {
    for (i = 0; i < x->ss_num_props; i++) {
  ...
  if (x->eb_num_props > 0 && x->eb_prop_name != NULL && x->eb_prop != NULL) {
    for (i = 0; i < x->eb_num_props; i++) {
...
int fence_post(const int val, /* the integer we seek */
               int *array,
               const int length) {
  int index;
...
void alloc_exo_ev(Exo_DB *x, const int num_timeplanes) {
  int i;
  int index;
  int j;
  int k;
...
void alloc_exo_gv(Exo_DB *x, const int num_timeplanes) {
  int i;
...
void alloc_exo_nv(Exo_DB *x, const int num_timeplanes, const int num_nodal_vars) {
  int i;
  int j;

     ...
     
{
  ...
  for (int j = 0; j < ei[pg->imtrx]->dof[PSTAR]; j++)
     {
      d_func[0][PSTAR][j] = 0.0;
       for (int a = 0; a < pd->Num_Dim; a++)
         {
            //d_func[0][PSTAR][j] += fv->snormal[a] * bf[PSTAR]->grad_phi[j][a];
         }
     }
  ...
  for (int a = 0; a < pd->Num_Dim; a++)
     {
       func[0] -= fv->snormal[a] * (fv->v_star[a] + fv_old->v_star[a]) / dt;
     }

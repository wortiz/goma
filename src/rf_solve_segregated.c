        ...
        for (pg->imtrx = 0; pg->imtrx < upd->Total_Num_Matrices; pg->imtrx++) {
          ...
          */
          ...
          if (converged) {
            ...
            //       if (pd_glob[mn]->v[pg->imtrx][var]) {
            //         for (i = 0; i < num_total_nodes; i++) {
            //           int j_shear_rate = Index_Solution(i, SHEAR_RATE, 0, 0, mn, shear_rate_matrix);
            //           int j = Index_Solution(i, var, 0, 0, mn, pg->imtrx);

            //           if (j_shear_rate != -1 && j != -1 && x[shear_rate_matrix][j_shear_rate] > 0) {
            ...
            //   MPI_Allreduce(&limited_values, &global_limited, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);

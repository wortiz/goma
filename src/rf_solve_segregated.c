    ...
    dbl *te_out) /* te_out - return actual end time */
{
  ...
  if (callnum == 1) {
    ...
    for (int imtrx = 0; imtrx < upd->Total_Num_Matrices; imtrx++) {
      ...
      if (pg->matrix_subcycle_fraction[imtrx] < 1) {
        num_sub_time_steps =
            (int)floor(1.0 / pg->matrix_subcycle_fraction[imtrx]);
        sub_delta_t =
            (1.0 / floor(1.0 / (pg->matrix_subcycle_fraction[imtrx])));
      }
      ...
      pg->num_sub_time_steps[imtrx] = num_sub_time_steps;
      if (num_sub_time_steps > 1) {
        timestep_subcycle = 1;
      }
    }
    ...
    if (timestep_subcycle) {
      ...
      for (int imtrx = 0; imtrx < upd->Total_Num_Matrices; imtrx++) {
        if (pg->num_sub_time_steps[imtrx] == 1) {
          continue;
        }
    ...
    for (n = 0; n < MaxTimeSteps; n++) {
      ...
      for (int subcycle = 0; subcycle < upd->SegregatedSubcycles ||
        ...
        for (pg->imtrx = 0; pg->imtrx < upd->Total_Num_Matrices; pg->imtrx++) {
          ...
          if (pg->num_sub_time_steps[pg->imtrx] > 1) {
            ...
            for (int sub_time_step = 0;
                 sub_time_step < pg->num_sub_time_steps[pg->imtrx];
                 sub_time_step++) {
              ...
              if (upd->SegregatedSolve && pg->imtrx == 0) {
                ...
                predict_solution(numProcUnknowns[pg->imtrx],
                                 pg->sub_delta_t[pg->imtrx],
                                 pg->sub_delta_t_old[pg->imtrx],
                                 pg->sub_delta_t_older[pg->imtrx], theta,
                                 pg->sub_step_solutions[pg->imtrx].x,
              ...
              if (ls != NULL && ls->Evolution == LS_EVOLVE_SLAVE &&
                ...
                surf_based_initialization(
                    pg->sub_step_solutions[pg->imtrx].x, NULL, NULL, exo,
                    num_total_nodes, ls->init_surf_list, time1, theta, delta_t);
              }
              ...
              printf("\n--------------------- SOLVING MATRIX %d sub_time_step "
                     ...
                     pg->num_sub_time_steps[pg->imtrx]);
              ...
              err = solve_nonlinear_problem(
                  ...
                  pg->sub_step_solutions[pg->imtrx].x_update, scale[pg->imtrx],
                  &converged, &nprint, tev[pg->imtrx], tev_post[pg->imtrx], gv,
              ...
              // change delta t's as this time step may have changed the current
              // delta t
            ...
            for (i = 0; i < numProcUnknowns[pg->imtrx]; i++) {
              xdot[pg->imtrx][i] = (1.0 + 2.0 * theta) / delta_t * (x[pg->imtrx][i] - x_old[pg->imtrx][i]) -

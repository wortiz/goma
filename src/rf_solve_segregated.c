    ...
    dbl *te_out) /* te_out - return actual end time */
{
  ...
  if (strcmp(Matrix_Format, "epetra") == 0) {
    ...
    EH(err, "Incompatible matrix solver for epetra, epetra supports amesos and "
            "aztecoo solvers.");
    ...
    for (n = 0; n < tran->MaxSteadyStateSteps; n++) {
      ...
      for (pg->imtrx = 0; pg->imtrx < upd->Total_Num_Matrices; pg->imtrx++) {
        ...
        err = solve_nonlinear_problem(
            ...
            is_steady_state, x_AC[pg->imtrx], x_AC_dot[pg->imtrx], time1, NULL,
          ...
          for (iAC = 0; iAC < nAC; iAC++) {
            ...
            /*		  else if(augc[iAC].Type == AC_USERMAT ||
               ...
               Param=%10.6e\n", augc[iAC].MTID, augc[iAC].VOLID, evol_local,
                              ...
                              DPRINTF(stderr, "\tMT[%4d] LSVEL phase[%4d]=%10.6e
      ...
      if (converged) {
        ...
        for (pg->imtrx = 0; pg->imtrx < upd->Total_Num_Matrices; pg->imtrx++) {
          ...
          if (pg->imtrx == 0) {
            ...
            double pres = vector_distance_pres(NumUnknowns[pg->imtrx],
                                               x[pg->imtrx], x_old[pg->imtrx]);
            if (ProcID == 0) {
        ...
        //xdot, 				  xdot_old, tev, tev_post, gv, rd, gvec, gvec_elem, &nprint,
        //delta_t, 				  theta, (double) nprint, NULL, exo, dpi); 	nprint++;
        ...
        if (distance < tran->steady_state_tolerance) {
          ...
          write_solution_segregated(ExoFileOut, resid_vector, x, x_old, xdot,
                                    xdot_old, tev, tev_post, gv, rd, gvec,
                                    gvec_elem, &nprint, delta_t, theta, time1,
                                    NULL, exo, dpi);
   ...
   ********************************************************************************/
    ...
    for (pg->imtrx = 0; pg->imtrx < upd->Total_Num_Matrices; pg->imtrx++) {
      ...
      if (upd->ep[pg->imtrx][FILL] > -1 &&
        ...
        if (ls != NULL || pfd != NULL) {
          ...
          /*
           * More initialization needed. Have to set those field variables that
           * initially are indexed by level set function.  For example, species
    ...
    if (Write_Initial_Solution) {
      write_solution_segregated(
          ExoFileOut, resid_vector, x, x_old, xdot, xdot_old, tev, tev_post, gv,
          rd, gvec, gvec_elem, &nprint, delta_t, theta, time1, NULL, exo, dpi);
      nprint++;
    }
    ...
    for (n = 0; n < MaxTimeSteps; n++) {
      ...
      for (int subcycle = 0; subcycle < upd->SegregatedSubcycles ||
        ...
        for (pg->imtrx = 0; pg->imtrx < upd->Total_Num_Matrices; pg->imtrx++) {
          ...
          if (subcycle == 0) {
            if (upd->SegregatedSolve && pg->imtrx == 0) {
              ...
              predict_solution(numProcUnknowns[pg->imtrx], delta_t, delta_t_old,
                               delta_t_older, theta, x[pg->imtrx],
          ...
          if (upd->ep[pg->imtrx][R_EIKONAL] > -1) {
            ...
            dcopy1(numProcUnknowns[pg->imtrx], x[Fill_Matrix],
                   x_old[pg->imtrx]);
            dcopy1(numProcUnknowns[pg->imtrx], x[Fill_Matrix],
                   x_older[pg->imtrx]);
            dcopy1(numProcUnknowns[pg->imtrx], x[Fill_Matrix],
            ...
            double cfl = cfl_eikonal(
                x[pg->imtrx], x_old[pg->imtrx], x_older[pg->imtrx],
                xdot[pg->imtrx], xdot_old[pg->imtrx], resid_vector[pg->imtrx],
                ams[pg->imtrx]->proc_config, exo);

            DPRINTF(stdout, "\nEIKONAL CFL %g\n", cfl);

            double eikonal_delta_t = 0.2*cfl;

            double eikonal_delta_t_old = eikonal_delta_t;

            double eikonal_delta_t_older = eikonal_delta_t;

            double eikonal_theta = 0.0;
            for (int step = 0; step < 10; step++) {
              DPRINTF(stdout, "Solving eikonal step %d with dt %g\n", step,
                      eikonal_delta_t);
              predict_solution(numProcUnknowns[pg->imtrx], eikonal_delta_t, eikonal_delta_t_old,
                               eikonal_delta_t_older, eikonal_theta, x[pg->imtrx],
                               x_old[pg->imtrx], x_older[pg->imtrx],
                               x_oldest[pg->imtrx], xdot[pg->imtrx],
                               xdot_old[pg->imtrx], xdot_older[pg->imtrx]);
              exchange_dof(cx[pg->imtrx], dpi, x[pg->imtrx], pg->imtrx);
              ...
              err = solve_nonlinear_problem(
                  ams[pg->imtrx], x[pg->imtrx], eikonal_delta_t, eikonal_theta,
              ...
              if (converged) {
                double eikonal_distance = vector_distance(
                    numProcUnknowns[pg->imtrx], x[pg->imtrx], x_old[pg->imtrx]);

                DPRINTF(stdout, "Eikonal distance %g\n", eikonal_distance);
                double F_distance = vector_distance(
                    numProcUnknowns[pg->imtrx], x[pg->imtrx], x[Fill_Matrix]);
                ...
                eikonal_delta_t_older = eikonal_delta_t_old;
                eikonal_delta_t_old = eikonal_delta_t;
                if (step > 3) {
                  eikonal_delta_t *= 1.25;
                }
            ...
            err = solve_nonlinear_problem(
                ...
                resid_vector[pg->imtrx], x_update[pg->imtrx], scale[pg->imtrx],
                &converged, &nprint, tev[pg->imtrx], tev_post[pg->imtrx], gv,
                rd[pg->imtrx], NULL, NULL, gvec[pg->imtrx],
                gvec_elem[pg->imtrx], time1, exo, dpi, cx[pg->imtrx], 0,
                &time_step_reform, 0, x_AC[pg->imtrx], x_AC_dot[pg->imtrx],
                time1, NULL, NULL, NULL, NULL, dg_neighbor_data);
          }
          ...
          //                {
          //                  int j = Index_Solution(i, var, 0, 0 , -1,
          //                  pg->imtrx);
          ...
          //            MPI_Allreduce(&floored_values, &global_floored, 1,
			...
			DPRINTF(stderr, "\tAC[%4d] DF[%4d]=% 10.6e\n", iAC, 5, augc[iAC].DataFlt[5]);
                ...
                /*	      else if (augc[iAC].Type == AC_USERMAT ||
                   ...
                   augc[iAC].BCID, augc[iAC].DFID, x_AC[iAC]);
                              ...
                              {
                              DPRINTF(stderr, "\tNodeSet[%4d]_Pos = %10.6e F_bal
                   = %10.6e VC[%4d] Param=%10.6e\n", augc[iAC].MTID,
      ...
      // reset renorm subcycle after we have completed the appropriate number of
      // subcycles
        ...
        if (time1 >= (ROUND_TO_ONE * TimeMax)) {
          ...
          if (Anneal_Mesh) {
            ...
            for (pg->imtrx = 0; pg->imtrx < upd->Total_Num_Matrices;
              ...
              err =
                  anneal_mesh(x[pg->imtrx], tev[pg->imtrx], tev_post[pg->imtrx],

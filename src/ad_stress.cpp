...
int ad_film_height_stress_source(int mode,
                                 ADType S[DIM][DIM],
  ...
  for (int ii = 0; ii < 3; ii++) {
    for (int jj = 0; jj < 3; jj++) {
      source_term[ii][jj] = -mup * gamma[ii][jj];
    }
  }
...
int ad_sqrt_conf_source(int mode, ADType b[DIM][DIM], ADType source_term[DIM][DIM]) {
  ...
  if (sdim == 2) {
    ...
    GOMA_EH(GOMA_ERROR, "Unknown sdim = %d for SQRT conformation tensor", sdim);
  }
  ...
  case PTT: {
    ...
    if (vn->ptt_type == PTT_LINEAR) {
      Z = 1 + eps * (trace - (double)sdim);
    } else if (vn->ptt_type == PTT_EXPONENTIAL) {
      const double exp_max = 700;
      ADType inner = eps * (trace - (double)sdim);
      if ((inner > exp_max) || (inner < -exp_max)) {
        GOMA_WH_MANY(GOMA_ERROR, "Exponential overflow in PTT_EXPONENTIAL");
        return GOMA_ERROR;
      }
      Z = exp(eps * (trace - (double)sdim));
    } else {
      GOMA_EH(GOMA_ERROR, "Unrecognized PTT Form %d", vn->ptt_type);
    }
...
extern "C" int ad_assemble_film_height_stress(dbl tt, /* parameter to vary time integration from
  ...
  ADType S[DIM][DIM];     /* stress tensor */
  ADType S_dot[DIM][DIM]; /* stress tensor from last time step */
  ...
  ADType S_dot_g[DIM][DIM];
  ADType S_dot_gt[DIM][DIM];
  ADType g_dot_S[DIM][DIM];
  ADType gt_dot_S[DIM][DIM];
  ...
  /* advective terms are precalculated */
  ADType v_dot_del_S[DIM][DIM];
  ADType x_dot_del_S[DIM][DIM];
  ...
  for (mode = 0; mode < vn->modes; mode++) {
    ...
    ad_load_modal_pointers(mode, tt, dt, S, S_dot, grad_S);
    ...
    for (int ii = 0; ii < 3; ii++) {
      for (int jj = 0; jj < 3; jj++) {
        ...
        x_dot_del_S[ii][jj] = 0.;
        for (q = 0; q < WIM; q++) {
          v_dot_del_S[ii][jj] += v[q] * grad_S[q][ii][jj];
          x_dot_del_S[ii][jj] += x_dot[q] * grad_S[q][ii][jj];
        }
      }
    }
    ...
    (void)ad_tensor_dot(S, g, S_dot_g, 3);
    (void)ad_tensor_dot(g, S, g_dot_S, 3);
    (void)ad_tensor_dot(S, gt, S_dot_gt, 3);
    (void)ad_tensor_dot(gt, S, gt_dot_S, 3);

    ADType source_term[DIM][DIM];
    goma_error err = ad_film_height_stress_source(mode, S, grad_v, source_term);
    if (err) {
      return 1;
    }
            ...
            for (i = 0; i < ei[pg->imtrx]->dof[eqn]; i++) {
              ...
              if (pd->e[pg->imtrx][eqn] & T_ADVECTION) {
                if (DOUBLE_NONZERO(lambda)) {
                  ...
                  advection += v_dot_del_S[ii][jj] - x_dot_del_S[ii][jj];
                  if (ucwt != 0.)
                    advection -= ucwt * (gt_dot_S[ii][jj] + S_dot_g[ii][jj]);
                  if (lcwt != 0.)
...
ad_assemble_film_height_sqrt_conf_stress(dbl tt, /* parameter to vary time integration from
  ...
  for (mode = 0; mode < vn->modes; mode++) {
    ...
    (void)ad_tensor_dot(b, g, b_dot_g, 3);

...
void load_enthalpy(double saturation, double pressure)
 ...
 ************************************************************************/
{
  ...
  if (saturation != 0.0) {
    pmv->enthalpy[0] = h_water_compressed_EOS(Pl, T, &dh_dP, &dh_dT, d_dh_dP, d_dh_dT);
    ...
    if (pd->TimeIntegration == TRANSIENT) {
      pmv_old->enthalpy[0] =
          h_water_compressed_EOS(Pl_old, T_old, &dh_dP, &dh_dT, d_dh_dP, d_dh_dT);
  ...
  if (saturation < 1.0) {
    ...
    if (Pg >= Pvsat) {
      ...
      pmv->enthalpy[1] = h_water_superheat_EOS(Pvsat, T, &dh_dP, &dh_dT, d_dh_dP, d_dh_dT);

      /* fill only the non-zero terms */
      pmv->d_enthalpy[1][POR_TEMP] = dh_dT + dh_dP * dPvsat_dT;
      pmv->d_d_enthalpy[1][POR_TEMP][POR_TEMP] =
          d_dh_dT[1] + d_dh_dT[0] * dPvsat_dT + d_dh_dP[1] * dPvsat_dT + dh_dP * d_dPvsat_dT;
    } else {

      /* Use Pg as pressure; note h is f(Pg,T) here! */

      pmv->enthalpy[1] = h_water_superheat_EOS(Pg, T, &dh_dP, &dh_dT, d_dh_dP, d_dh_dT);
      /* fill only the non-zero terms */
    ...
    if (pd->TimeIntegration == TRANSIENT) {
      ...
      if (Pg_old >= Pvsat) {
        ...
        pmv_old->enthalpy[1] =
            h_water_superheat_EOS(Pvsat, T_old, &dh_dP, &dh_dT, d_dh_dP, d_dh_dT);
        /* fill only the non-zero terms */
        ...
        pmv_old->enthalpy[1] =
            h_water_superheat_EOS(Pg_old, T_old, &dh_dP, &dh_dT, d_dh_dP, d_dh_dT);
        /* fill only the non-zero terms */
...
double calc_rho_gas(double Pg,
  ...
  *rho_wg_sat =
      rho_sat_water_vap_EOS(*Pvsat, T, &drho_wg_dPv, &drho_wg_dT, d_drho_wg_dPv, d_drho_wg_dT);
  ...
  *d_drho_wgsat_dT = d_drho_wg_dT[1] + d_drho_wg_dT[0] * (*dPvsat_dT) +
  ...
  rho_air = rho_air_EOS(Pa, T, *dPvsat_dT, d_dPvsat_dT, &drho_air_dP, &drho_air_dT, d_drho_air_dP,
                        d_drho_air_dT);
...
double calc_Ywg(double rho_wg_sat,
                double drho_wgsat_dT,
                double d_drho_wgsat_dT,
                double rho_gas,
                double drho_gas_dPg,
  ...
  *dYwg_dT = drho_wgsat_dT / rho_gas - (Ywg / rho_gas) * drho_gas_dT;
  ...
  d_dYwg_dT[0] =
      -drho_wgsat_dT / rho_gas / rho_gas * drho_gas_dPg - (*dYwg_dPg) / rho_gas * drho_gas_dT +
      Ywg / rho_gas / rho_gas * drho_gas_dPg * drho_gas_dT - Ywg / rho_gas * d_drho_gas_dT[0];
  d_dYwg_dT[1] = d_drho_wgsat_dT / rho_gas - drho_wgsat_dT / rho_gas / rho_gas * drho_gas_dT -
...
double rho_sat_water_vap_EOS(double P_sat,
                             double T,
...

{
  ...
  static double Y[] = {0.13299942e-04,  0.10000000e+01, -0.10000000e+01, -0.56746973e-02,
                       -0.32791354e-06, 0.21636240e-08, -0.38485869e-11, 0.40896880e-01,
                       0.27696827e-02,  -0.94741649e-04};
  static double Z[] = {0.12789230e+00,  -0.28996744e+00, 0.26873883e-02,  0.33783903e-04,
                       0.55690966e-02,  0.72603809e-05,  -0.44323127e-07, 0.49878874e-03,
                       -0.13186635e-04, 0.72041771e-06};
  ...
  *drho_wgsat_dT = dfdT / g - f / g / g * dgdT;
  ...
  d_drho_wgsat_dP[0] = (df_d_dP[0] / g - 2.0 * dfdP / gsq * dgdP + 2.0 * f / gcb * dgdP * dgdP -
                        f / gsq * dg_d_dP[0]) /
                       1.0e12;
  d_drho_wgsat_dP[1] = (df_d_dP[1] / g - dfdP * dgdT / gsq - dfdT / gsq * dgdP +
                        2.0 * f / gcb * dgdT * dgdP - f / gsq * dg_d_dP[1]) /
                       1.0e6;
  d_drho_wgsat_dT[0] = (df_d_dT[0] / g - dfdT / gsq * dgdP - dfdP / gsq * dgdT +
                        2.0 * f / gcb * dgdP * dgdT - f / gsq * dg_d_dT[0]) /
                       1.0e6;
  d_drho_wgsat_dT[1] =
      df_d_dT[1] / g - 2.0 * dfdT / gsq * dgdT + 2.0 * f / gcb * dgdT * dgdT - f / gsq * dg_d_dT[1];
...
double P_water_sat_EOS(double T, double *dPvsat_dT, double *d_dPvsat_dT)
...

{
  ...
  double gsq, gcb;
  static int order = 4;
  static double Y[] = {0.71725602e-03, 0.22607516e-04, 0.26178556e-05, -0.10516335e-07,
                       0.63167028e-09};
  static double Z[] = {0.10000000e+01, -0.22460012e-02, 0.30234492e-05, -0.32466525e-09, 0.0};
  ...
  *d_dPvsat_dT =
      (df_d_dT / g - 2.0 * dfdT * dgdT / gsq + 2.0 * f / gcb * dgdT * dgdT - f / gsq * dg_d_dT) *
      1.0e6;

  return (f / g * 1.e6);
}

double eval_pv_poly(int order, double coef[], double T)
...
/*****************************************************************************/
/*****************************************************************************/
double eval_pv_poly_dT(int order, double coef[], double T, double *df_d_dT)
...
#ifdef NOT_USED
double rho_sat_water_liq_EOS(double P_sat,
                             double T,
                             double *drhol_dP,
                             double *drhol_dT,
                             double drhol_d_dP[],
...

{
  ...
  static double Y[] = {0.10000000e+01, -0.50430220e-01, 0.12147449e-02, -0.29566543e-04,
                       0.11719555e-01, -0.10272834e-03, 0.16483547e-06, 0.74802254e-03,
                       0.17552861e-05, -0.16978281e-05};
  static double Z[] = {0.10020170e-02, -0.52711077e-04, 0.14548166e-05, -0.36472636e-07,
                       0.11718816e-04, -0.93182060e-07, 0.12768238e-09, 0.72200359e-06,
                       0.18887078e-08, -0.14167944e-08};
  ...
  *drhol_dT = *drhol_dT;
  ...
  drhol_d_dT[0] = (df_d_dT[0] / g - dfdT / gsq * dgdP - dfdP / gsq * dgdT +
                  ...
                  1.0e6;
  drhol_d_dT[1] =
      df_d_dT[1] / g + 2.0 * dfdT / gsq * dgdT + 2.0 * f / gcb * dgdT - f / gsq * dg_d_dT[1];
...
double h_air_EOS(double T, double *dha_dT, double *d_dha_dT)
...

{
  ...
  static double Y[] = {1003.7, 0.025656, 4.5457e-4, -2.7107e-7};
  T = T + 273.15; /* T in K  */
...
double h_water_compressed_EOS(
    double P, double T, double *dhl_dP, double *dhl_dT, double d_dhl_dP[2], double d_dhl_dT[2])
...

{
  ...
  static double Y[] = {-0.28892688e-04, 0.10155128e-02,  0.38182267e-04, 0.29406408e-06,
                       0.42068671e-02,  -0.26722745e-04, 0.39965615e-07, 0.14983417e-03,
                       0.11199162e-05,  -0.44963038e-06};
  static double Z[] = {0.10000000e+01,  0.38028489e-01, 0.32800006e-03, 0.38164755e-07,
                       -0.62817403e-02, 0.87410801e-05, 0.18991534e-08, -0.11452490e-03,
                       -0.11341777e-06, 0.19903338e-08};
...
double h_water_superheat_EOS(
    double P, double T, double *dhwg_dP, double *dhwg_dT, double d_dhwg_dP[2], double d_dhwg_dT[2])
...

{
  ...
  static double Y[] = {0.49023415e+00,  -0.10000000e+01, 0.24474474e-01, 0.23476073e-03,
                       0.86459576e-02,  0.38256791e-04,  0.19190905e-07, 0.16237610e-02,
                       -0.74126396e-04, -0.78086106e-06};
  static double Z[] = {0.19602927e+00,  -0.35954866e+00, 0.54884993e-02,  0.58496026e-04,
                       0.33114850e-02,  0.12829588e-04,  -0.20053974e-08, 0.78784157e-03,
                       -0.18512345e-04, -0.52896691e-06};
...
double eval_poly(double coef[], double P, double T)
...
double eval_poly_dP(double coef[], double P, double T, double d_dP_poly[])
...
double eval_poly_dT(double coef[], double P, double T, double d_dT_poly[])

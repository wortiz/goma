...
static int int_intersect(int *,     /* a - first integer list	        (in) */
...
void build_elem_node(Exo_DB *exo) {
  int ebi;
  int elem;
  int i;
  int j;
...
void build_node_node(Exo_DB *exo) {
  int curr_list_size;
  int dude;
  int e;
  int elem;
  ...
  exo->node_node_list = (int *)realloc(exo->node_node_list, total_list_size * sizeof(int));
...
void build_node_elem(Exo_DB *exo) {
  int e;
  int i;
  int index;
  int length;
  ...
  for (e = 0; e < exo->num_elems; e++) {
    for (n = exo->elem_node_pntr[e]; n < exo->elem_node_pntr[e + 1]; n++) {
      ...
      while (exo->node_elem_list[index] != -1 && index < exo->node_elem_pntr[node + 1]) {
        index++;
      }

      if (index >= exo->node_elem_pntr[node + 1]) {
        sr = sprintf(err_msg, "No free spot elem [%d], node [%d].", e, node);
...
void build_elem_elem(Exo_DB *exo) {
  int ce;
  int count;
  int e;
  int ebi;
  ...
  if (!exo->elem_node_conn_exists || !exo->node_elem_conn_exists) {
    GOMA_EH(GOMA_ERROR, "Build elem->node before node->elem.");
    return;
  }
  /*
    ...
    for (e = 0; e < exo->eb_num_elems[ebi]; e++, elem++) {
      ...
      for (face = 0; face < num_elem_sides; face++) {
        ...
        for (n = 0; n < num_nodes; n++) {
          ...
          for (ce = exo->node_elem_pntr[node]; ce < exo->node_elem_pntr[node + 1]; ce++) {
            ...
            */
            ...
            int shell_on_shell = 0;
            int flippy_flop = 0;
            int nbr_ebid;
            int nbr_num_elem_sides;
            ...
            if (exo->eb_id[ebi] < 100 && exo->eb_id[nbr_ebid] >= 100)
              flippy_flop = 1;
            if (exo->eb_id[ebi] >= 100 && exo->eb_id[nbr_ebid] < 100)
              flippy_flop = 1;

            if ((nbr_ebid != ebi) && (strstr(exo->eb_elem_type[nbr_ebid], "SHELL")) &&
                (strstr(exo->eb_elem_type[ebi], "SHELL")) && flippy_flop)
              shell_on_shell = 1;
          ...
          len_intr = int_intersect(prev_set, curr_set, len_prev, len_curr, ip, ic);
        ...
        if (len_intr == 2) {
          ...
          if (prev_set[0] == elem) {
            ...
            if (prev_set[1] != elem) {
              sr = sprintf(err_msg, "2 elems ( %d %d ) 1 should be %d!", prev_set[0], prev_set[1],
                           elem);
              GOMA_EH(GOMA_ERROR, err_msg);
            }
          }
         ...
         */
          ...
          sr = sprintf(err_msg, "Elem %d, face %d should self contain!", elem, face);
          GOMA_EH(GOMA_ERROR, err_msg);
        } else if (len_intr == 1 && prev_set[0] != elem) {
          sr = sprintf(err_msg, "Elem %d, face %d only connects with elem %d ?", elem, face,
                       prev_set[0]);
          GOMA_EH(GOMA_ERROR, err_msg);
        } else {
          sr = sprintf(err_msg, "Unknown elem-elem connection elem %d, face %d, len_intr=%d", elem,
                       face, len_intr);
          GOMA_WH(-1, err_msg);
        }
...
int build_side_node_list(int elem, int face, Exo_DB *exo, int *snl) {
  int element_type;
  int i;
  int nodes_this_side;
  int shape;
  ...
  for (i = 0; i < nodes_this_side; i++) {
    snl[i] = exo->elem_node_list[exo->elem_node_pntr[elem] + local_nodeces[i]];
  }
...
static int get_num_faces(char *elem_type) {
  int val = -1;
  ...
  {
    val = 4;
  } else if (strstr(elem_type, "TRUSS") != NULL || strstr(elem_type, "BAR") != NULL ||
...
int get_exterior_faces(int elem, int *ex_faces, const Exo_DB *exo, const Dpi *dpi) {
  int index;
  int face;
  int num_ext_faces = 0;
  int *elem_elem_list;
  ...
  for (index = exo->elem_elem_pntr[elem], face = 0; index < exo->elem_elem_pntr[elem + 1];
       index++, face++) {

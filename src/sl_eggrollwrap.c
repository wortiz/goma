                 ...
                 Dpi *dpi)     /* ptr to distributed processing info */
{
  int i, j, ic, nj, nnz_j, first_linear_solver_call, Factor_Flag, matr_form, error, rcflag, action,
      ...
      push_mode, init_shft, recycle;
  int step = 0;
  dbl stol, ivector, dwork[20];
  dbl *ev_e, *ev_i, *ev_r, *ev_x, *v1, *v2, *mat, **evect, **schur;
  ...
  if (recycle != 0)
    GOMA_EH(GOMA_ERROR, "Eigen recycle currently doesn't work, turn it off.");
  ...
  do {
    ic++;
    /* printf("ic = %d\n", ic); fflush(stdout); */
    gevp_solver_rc(nj, mm, max_itr, stol, filter, &ev_n, &ev_r[0], &ev_i[0], &ev_e[0], &ev_x[0],
                   &lead, ev_jac, nev_want, &nev_found, schur, evect, recycle, ivector, 0, &rcflag,
                   &action, &dwork[0], &v1[0], &v2[0]);
    ...
    case 3: /* inv(J-sM) */
      ...
      */
      ...
      UMF_system_id = SL_UMF(UMF_system_id, &first_linear_solver_call, &Factor_Flag, &matr_form,
                             &nj, &nnz_j, &ija[0], &ija[0], &mat[0], &v1[0], &v2[0]);
      first_linear_solver_call = 0;
      break;
    case 4: /* v2 = inv(J-sM)*M*v1 */
      Factor_Flag = 3;
      if (first_linear_solver_call)
        GOMA_EH(GOMA_ERROR, "Tried to transform eigenvectors before a solve!");
      gevp_transformation(UMF_system_id, first_linear_solver_call, Factor_Flag, matr_form, 1, nj,
                          nnz_j, &ija[0], &jac[0], &mas[0], &mat[0],
                          /*			  soln_tech, */
  ...
  UMF_system_id = SL_UMF(UMF_system_id, &first_linear_solver_call, &Factor_Flag, &matr_form, &nj,
                         &nnz_j, ija, ija, mat, &v1[0], &v2[0]);
  ...
  if (push_mode > 0) {
    ...
    for (i = 0; i < push_mode; i++) {
      ...
      if (LSA_3D_of_2D_wave_number == -1.0)
        sprintf(ExoFileOut, "LSA_%d_of_%d_%s", i + 1, push_mode, save_ExoFileOut);
      else
        sprintf(ExoFileOut, "LSA_%d_of_%d_wn=%g_%s", i + 1, push_mode, LSA_3D_of_2D_wave_number,
                save_ExoFileOut);
      ...
      for (j = 0; j < tnv; j++) {
        extract_nodal_vec(&evect[i][0], rd->nvtype[j], rd->nvkind[j], rd->nvmatID[j], gvec, exo,
                          FALSE, time_value);
        wr_nodal_result_exo(exo, ExoFileOut, gvec, j + 1, 1, time_value);
      }
       ...
       */
      if (tnv_post > 0) {
        post_process_nodal(&evect[i][0], NULL, x_old, xdot, xdot_old, resid_vector, 1, &time_value,
                           delta_t, 0.0, NULL, exo, dpi, rd, ExoFileOut, 0);
      }
      printf(" recorded.\n");
    }
  }

...
int dof_lnode_var_type(const int n,
                       const int Element_Type,
                       const int proc_node_num,
                       const int var_type,
...
int num_varType_at_node(const int inode, const int varType)
...
int get_nv_ndofs(NODAL_VARS_STRUCT *nv, const int varType)
...
int get_nv_ndofs_modMF(NODAL_VARS_STRUCT *nv, const int varType)
...
int get_nv_index_varType(NODAL_VARS_STRUCT *nv,
                         const int varType,
                         const int mn,
...
int get_nodal_unknown_offset(NODAL_VARS_STRUCT *nv,
                             const int varType,
                             const int mn,
                             const int subVarType,
...
int get_nv_offset_idof(NODAL_VARS_STRUCT *nv,
                       const int varType,
                       const int idof,
                       int subVarType,
                       VARIABLE_DESCRIPTION_STRUCT **vd_ptr)
...
void get_nv_vd_from_offset(NODAL_VARS_STRUCT *nv,
                           int offset,
                           VARIABLE_DESCRIPTION_STRUCT **vd_ptr,
                           int *idof)
...
int nodal_vars_comparison(NODAL_VARS_STRUCT *nv1, NODAL_VARS_STRUCT *nv2)
 ...
 *******************************************************************/
{
  ...
  if (nv2 == NULL)
    return FALSE;
  if (nv1->Num_Var_Desc != nv2->Num_Var_Desc)
    return FALSE;
  if (nv1->Num_Unknowns != nv2->Num_Unknowns)
    return FALSE;
...
int MatchAdd_Node_Vars_List(NODAL_VARS_STRUCT *newNV, NODAL_VARS_STRUCT **matchNV_hdl)
 ...
 ********************************************************************/
{
   ...
   */
  realloc_ptr_1((void ***)&Nodal_Vars_List, Nodal_Vars_List_Length + 1, Nodal_Vars_List_Length);
...
void add_var_to_nv_struct(VARIABLE_DESCRIPTION_STRUCT *vd, NODAL_VARS_STRUCT *nv)
 ...
 *****************************************************************/
{
  ...
  if (nv == NULL)
    GOMA_EH(GOMA_ERROR, "add_var_to_nv_struct error");
  if (vd == NULL)
    GOMA_EH(GOMA_ERROR, "add_var_to_nv_struct error");
   ...
   */
  realloc_ptr_1((void ***)&(nv->Var_Desc_List), nv->Num_Var_Desc + 1, nv->Num_Var_Desc);
   ...
   */
  realloc_int_1(&nv->Nodal_Offset, nv->Num_Var_Desc + 1, nv->Num_Var_Desc);
...
static void rearrange_mat_increasing(int var_type,
                                     int subvarIndex,
                                     NODAL_VARS_STRUCT *nv_old,
                                     NODAL_VARS_STRUCT *nv_new,
                                     int *rec_used)
 ...
 *****************************************************************/
{
  ...
  if (var_type == MASS_FRACTION) {
    num = nv_old->Num_Var_Desc_Per_Type[var_type] / upd->Max_Num_Species_Eqn;
  }
  ...
  for (j = 0; j < num; j++) {
    ...
    for (i = 0; i < nv_old->Num_Var_Desc; i++) {
      if (!rec_used[i]) {
        ...
        if ((var_type == vd->Variable_Type) && (subvarIndex == (int)vd->Subvar_Index) &&
            (mn_min > vd->MatID)) {
          var_rec = i;
          mn_min = vd->MatID;
        }
     ...
     */
    add_var_to_nv_struct(nv_old->Var_Desc_List[var_rec], nv_new);
  }
}
/************************************************************************/
/************************************************************************/
...
NODAL_VARS_STRUCT *copy_and_rearrange_nv(NODAL_VARS_STRUCT *nv_old)
 ...
 *****************************************************************/
{
  ...
  for (var_type = V_FIRST; var_type < V_LAST; var_type++) {
    if (var_type == MASS_FRACTION) {
      for (subvarIndex = 0; subvarIndex < upd->Max_Num_Species_Eqn; subvarIndex++) {
        rearrange_mat_increasing(var_type, subvarIndex, nv_old, nv_new, rec_used);
      }
    } else {
...
void nodal_vars_free(NODAL_VARS_STRUCT *nv)
...
void nodal_vars_destroy(NODAL_VARS_STRUCT **nv_hdl)
...
int find_var_type_index_in_nv(const int var_type, NODAL_VARS_STRUCT *nv)
...
int find_vd_index_in_nv(const int var_type, const int matID, NODAL_VARS_STRUCT *nv)
...
void pack_nv(struct nv_packed *nvp, NODAL_VARS_STRUCT *nv)
...
NODAL_VARS_STRUCT *unpack_nv(struct nv_packed *nvp, const int imtrx)
 ...
 *************************************************************************/
{
  ...
  for (i = 0; i < numVD; i++) {
     ...
     */
    var_ptr = find_or_create_vd((int)vdp->Variable_Type, (int)vdp->Ndof, (int)vdp->MatID,
                                (int)vdp->Subvar_Index, imtrx);

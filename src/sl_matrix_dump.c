...

{
  if (value < 0 || value > 255) {
    fprintf(stderr, "matrix_dump ERROR: byte conversion used, but value = %d: Proc = %d\n", value,
            ProcID);
    exit(-1);
  }
...

{
  extern int errno;
  if (result) {
    fprintf(stderr, "matrix_dump ERROR: fclose failed Proc = %d: %s\n", ProcID, strerror(errno));
    exit(-1);
  }
}
...
{
  if (!result) {
    fprintf(stderr, "matrix_dump ERROR: xdr write failed Proc = %d\n", ProcID);
    exit(-1);
  }
}
/*****************************************************************************/
...
static int
get_columnNodeList_msr(const double *a, const int *ija, const int ieqn, UMI_LIST_STRUCT *ls_ptr) {
...
void matrix_dump_msr(struct Aztec_Linear_Solver_System *ams, Exo_DB *exo, Dpi *dpi, double *x)
 ...
 *****************************************************************************/
{
  ...
  if (ams->Number_Jac_Dump >= 0) {
    if (ams->Number_Jac_Dump < index)
      return;
  } else {
    if ((index + ams->Number_Jac_Dump) != 0)
      return;
  }
  ...
  do {
    ...
    if (gNodeGlobalMin == gNodeLocalNext) {
      ...
      while ((gNodeGlobalMin == gNodeLocalNext) && (gNodeGlobalMin < gNum_Nodes)) {
        ...
        CHECK_XDR(
            xdr_vector(xdrs, (char *)flt_coord, (u_int)3, sizeof(float), (xdrproc_t)xdr_float));
         ...
         */
        for (i = 0; i < num_unknowns_node; i++) {
          vdT = Index_Solution_Inv(ieqn, &inode, &i_Var_Desc, &i_offset, &idof, pg->imtrx);
          nodeT = Nodes[inode];
          /*
        ...
        for (icol = 0; icol < num_cols; icol++) {
          ...
          CHECK_XDR(
              xdr_vector(xdrs, (char *)a_blk, (u_int)itmp, sizeof(double), (xdrproc_t)xdr_double));
...
void matrix_dump_vbr(struct Aztec_Linear_Solver_System *ams, Exo_DB *exo, Dpi *dpi, double *x)
    ...
    if (gNodeGlobalMin == gNodeLocalNext) {
      ...
      while ((gNodeGlobalMin == gNodeLocalNext) && (gNodeGlobalMin < gNum_Nodes)) {
        ...
        for (icol = 0; icol < num_cols; icol++) {
          ...
          itmp = m1 * n1;
          CHECK_XDR(
              xdr_vector(xdrs, (char *)a_tmp, (u_int)itmp, sizeof(double), (xdrproc_t)xdr_double));

...
/* Function Prototypes */
static void find_exit_wound(
    const int, particle_t *, const dbl[3], const dbl[3], const dbl[3], const int, const int);
...
static void
output_a_particle(particle_t *const, const dbl, const dbl, const int, const int, const int);
 ...
 */
void initialize_particles(const Exo_DB *exo,
                          dbl *const x,
                          dbl *const x_old,
                          dbl *const xdot,
                          dbl *const xdot_old,
  ...
  if ((Particle_Model == SWIMMER_EXPLICIT || Particle_Model == SWIMMER_IMPLICIT) &&
    ...
    GOMA_EH(GOMA_ERROR, "You probably need to fix something first...");
  ...
  if (pd_glob[0]->e[pg->imtrx][R_MESH1])
    GOMA_EH(GOMA_ERROR, "Cannot couple particles and deformable mesh, yet.");
  if (mdim == 2) {
    if (static_exo->eb_elem_itype[0] != BIQUAD_QUAD &&
        static_exo->eb_elem_itype[0] != BILINEAR_QUAD)
      GOMA_EH(GOMA_ERROR, "Can only have 9-node and 4-node quadrilateral elements in 2D.");
    sides_per_element = 4;
  } else /* static_exo->num_dim == mdim == 3 */
  {
    if (static_exo->eb_elem_itype[0] != TRIQUAD_HEX &&
        static_exo->eb_elem_itype[0] != TRILINEAR_HEX)
      GOMA_EH(GOMA_ERROR, "Can only handly 27-node and 8-node hex elements in 3D.");
    sides_per_element = 6;
  }
  ...
  backup_element_particle_list_head =
      (particle_t **)malloc(static_exo->num_elems * sizeof(particle_t *));
  particles_to_send = NULL;

  DPRINTF(stdout, "  elements: ");
  fflush(stdout);
  ...
  element_particle_info =
      (element_particle_info_t *)malloc(static_exo->num_elems * sizeof(element_particle_info_t));
  for (i = 0; i < static_exo->num_elems; i++) {
    element_particle_info[i].PBC_side_id = (int *)malloc(sides_per_element * sizeof(int));
#ifdef PARALLEL
    element_particle_info[i].owner_local_element_id =
        (int *)calloc((unsigned)sides_per_element, sizeof(int));
    element_particle_info[i].num_ghost_target_elems =
        0; /* number of times each particle needs to be ghosted. */
  ...
  if (Particle_Full_Output_Stride) {
    if (!(pa_full_fp = fopen(construct_filename(Particle_Full_Output_Filename), "w")))
      GOMA_EH(GOMA_ERROR, "Could not open Particle_Full_Output_Filename for zeroing.");
    DPRINTF(stdout, "%s, ", Particle_Full_Output_Filename);
    fflush(stdout);
  } else
    pa_full_fp = NULL;
  ...
  if (Particle_Number_Sample_Types) {
    ...
    for (i = 0; i < Particle_Number_Sample_Types; i++) {
      if (!(pa_fp[i] = fopen(construct_filename(Particle_Filename_Template[i]), "w")))
        GOMA_EH(GOMA_ERROR, "Could not open a particle file for zeroing.");
      DPRINTF(stdout, "%s, ", Particle_Filename_Template[i]);
      fflush(stdout);
    ...
    pa_fp = NULL;
  if (Particle_Output_Format == TECPLOT && Particle_Number > 0)
    ...
    if (Particle_Creation_Domain == BRICK && Particle_Move_Domain == BRICK) {
      for (i = 0; i < 3; i++)
        if (Particle_Move_Domain_Reals[2 * i] > Particle_Creation_Domain_Reals[2 * i] ||
          ...
          GOMA_EH(GOMA_ERROR, "Mismatch in BRICK bounds for Creation/Move.");
    }
  }
  ...
  if (Particle_Number > 0) {
    ...
    if (ProcID == 0) {
      while (creation_count < Particle_Number) {
        DPRINTF(stdout, "    %d=", Particle_Number - creation_count);
        fflush(stdout);
        ...
        for (proc_index = 0; proc_index < Num_Proc; proc_index++) {
          number_to_create[proc_index] = (int)floor(proc_volume[proc_index] / total_volume *
                                                    (Particle_Number - creation_count));
        ...
        for (i = 0; i < left_over; i++) {
          ...
          proc_index = 0;
          while (proc_index < Num_Proc && r > proc_volume[proc_index]) {
            r -= proc_volume[proc_index];
            proc_index++;
          }
          if (proc_index == Num_Proc)
            GOMA_EH(GOMA_ERROR, "proc_index == Num_Proc");
          number_to_create[proc_index]++;
        }

        for (i = 0; i < Num_Proc - 1; i++)
          DPRINTF(stdout, "%d+", number_to_create[i]);
        DPRINTF(stdout, "%d", number_to_create[Num_Proc - 1]);
        fflush(stdout);
        ...
        MPI_Barrier(MPI_COMM_WORLD);
        MPI_Reduce(local_number_created, number_created, Num_Proc, MPI_INT, MPI_SUM, 0,
                   MPI_COMM_WORLD);
        ...
        DPRINTF(stdout, "%d", number_created[Num_Proc - 1]);
        fflush(stderr);
    ...
    DPRINTF(stdout, "    total R = %d, ", rejection_count);
    fflush(stdout);
#else
    ...
    DPRINTF(stdout, "total R=%d, ", creation_count);
    fflush(stdout);
#endif
  }
  get_time(time_of_day);
...
static void find_exit_wound(const int elem_id,
                            particle_t *p,
    ...
    if (num_crossed == 2) {
      ...
      if (fabs(xi_tmp[1]) > 1.0) /* Not this intersection!  Do exit_id2... */
      {
        ...
        if (fabs(xi_tmp[0]) > 1.0)
          dump20(EXIT, p,
                 "  x_start = (%g,%g,%g), x_end = (%g,%g,%g)\n  bdry_crossed = (%d,%d,%d), "
                 "exit_ids = (%d,%d,%d)\n  xi = (%g,%g,%g), xi_tmp = (%g,%g,%g)\n  stack_count = "
                 "%d\nOh no!  I couldn't find any good intersections!  Bye-bye!\n",
                 x_start[0], x_start[1], x_start[2], x_end[0], x_end[1], x_end[2], bdry_crossed[0],
                 bdry_crossed[1], bdry_crossed[2], exit_id1, exit_id2, exit_id3, xi[0], xi[1],
                 xi[2], xi_tmp[0], xi_tmp[1], xi_tmp[2], stack_count);
          ...
          if (fabs(xi_tmp[0]) > 1.0 || fabs(xi_tmp[1]) > 1.0) {
            if (tolerance_level < 2) {
              ...
              dump20(NO_EXIT, p,
                     ...
                     bdry_crossed[0], bdry_crossed[1], bdry_crossed[2], exit_id1, exit_id2,
  ...
  if (PBC_id == -1) /* must be another local element on the other side! */
  {
    ...
    if (p->owning_elem_id == -1) {
      fprintf(stderr, "\nelem_id = %d, global_elem_id = %d\n", elem_id,
              DPI_ptr->elem_index_global[elem_id]);
    ...
    if (get_element_xi_newton(p->owning_elem_id, x_end, xi_tmp) == -1)
      dump10(EXIT, p,
             "  exit_ids = (%d,%d,%d)\n  bdry_crossed = (%d,%d,%d)\n  xi_tmp = (%g,%g,%g)\nOh no!  "
             "I couldn't find element coordinates!  Bye-bye!\n",
             exit_id1, exit_id2, exit_id3, bdry_crossed[0], bdry_crossed[1], bdry_crossed[2],
             xi_tmp[0], xi_tmp[1], xi_tmp[2]);
 ...
 * components.
 */
int compute_particles(const Exo_DB *exo,         /* mesh */
                      ...
                      const int n)               /* time step number */
{
  ...
  dbl local_total_accum_ust, local_particle_accum_ust, local_output_accum_ust,
      local_communication_accum_ust;
        ...
        if (i == ProcID) /* I'm the sender! */
        {
          ...
          while (p) {
            ...
            */
            ...
            mpi_retval = MPI_Send(&send_p, sizeof(particle_t), MPI_BYTE, p->owning_proc_id, 0,
                                  MPI_COMM_WORLD);
          ...
          for (j = 0; j < Num_Proc; j++)
            if (i == j)
              ...
              */
              mpi_retval =
                  MPI_Send(&termination_p, sizeof(particle_t), MPI_BYTE, j, 0, MPI_COMM_WORLD);
              if (mpi_retval != MPI_SUCCESS) {
          ...
          while (recv_p.state != TERMINATION_MARKER) {
            mpi_retval =
                MPI_Recv(&recv_p, sizeof(particle_t), MPI_BYTE, i, 0, MPI_COMM_WORLD, &mpi_status);
            if (mpi_retval != MPI_SUCCESS) {
            ...
            /*
              ...
              MPI_SOURCE/TAG/ERROR = %d/%d/%d, MPI_Count = %d.\n", ProcID, i, recv_p.state,
              mpi_status.MPI_SOURCE, mpi_status.MPI_TAG, mpi_status.MPI_ERROR, mpi_byte_count);
              fflush(stderr);
            */
  ...
  MPI_Reduce(&local_max_particle_iterations, &max_particle_iterations, 1, MPI_INT, MPI_MAX, 0,
             MPI_COMM_WORLD);
  MPI_Reduce(&local_max_newton_iterations, &max_newton_iterations, 1, MPI_INT, MPI_MAX, 0,
             MPI_COMM_WORLD);
  MPI_Reduce(&local_particle_transfers, &particle_transfers, 1, MPI_INT, MPI_SUM, 0,
             MPI_COMM_WORLD);
  MPI_Reduce(&local_particle_accum_ust, &particle_accum_ust, 1, MPI_DOUBLE, MPI_MAX, 0,
             MPI_COMM_WORLD);
  ...
  MPI_Reduce(&local_communication_accum_ust, &communication_accum_ust, 1, MPI_DOUBLE, MPI_MAX, 0,
             MPI_COMM_WORLD);
  ...
  if (Particle_Model == INERTIAL_TRACER_IMPLICIT || Particle_Model == SWIMMER_IMPLICIT ||
      Particle_Model == TRACER_IMPLICIT || Particle_Model == CHARGED_TRACER_IMPLICIT ||
 ...
 * internally).
 */
static dbl fill_element_volumes(const int e_begin, const int e_end) {
  int i, ip, ip_total;
 ...
 */
static int position_particle_uniformly(const int elem_id, particle_t *p) {
  ...
  while (!done) {
    ...
    counter++;
    if (counter == 100 && !done)
      GOMA_EH(GOMA_ERROR, "Local element rejection sampling counter reached 100.");
  }
  return counter;
}
 ...
 * whatever...
 */
static char *construct_filename(const char *template) {
 ...
 * be fast. */
static void get_boundary_xi_newton(const dbl *const coeff,
                                   const int elem_id,
                                   dbl *xi_tmp,
  ...
  if (mdim == 2) {
    ...
    iter = 0;
    while (fabs(f) >= error_tolerance * f_scale && iter < 100) {
      /*
    ...
    */
    ...
    if (get_element_xi_newton(elem_id, pt, pt_xi) == -1) {
      fprintf(stderr, "Bad gexn(), pt = (%.10g,%.10g,%.10g), pt_xi = (%.10g,%.10g,%.10g)\n", pt[0],
              pt[1], pt[2], pt_xi[0], pt_xi[1], pt_xi[2]);
    }
    /*
    ...
    */
    ...
    iter = 0;
    while (fabs(f) >= error_tolerance * resid_scale && iter < 100) {
      /*
  ...
  if (iter == 100 && fabs(f) > error_tolerance) {
    fprintf(stderr,
            "last dfdxi = %g, t = %g, f = %g, resid_scale = %g, resid_initial = %g, xi_norm = %g, "
            "pt = (%g,%g,%g), pt_xi = (%g,%g,%g)\n",
            dfdxi1, t, f, resid_scale, resid_initial, xi_norm, pt[0], pt[1], pt[2], pt_xi[0],
            pt_xi[1], pt_xi[2]);
    GOMA_EH(GOMA_ERROR, "Exceeded 100 Newton iterations in get_boundary_xi_newton()");
  }
 ...
 * last fields filled.
 */
static particle_t *obtain_particle_space(const int elem_id) {
  particle_t *p;
  char s[128];
  ...
  if (!element_particle_list_head[elem_id]) {
    ...
    if (!p)
      GOMA_EH(GOMA_ERROR, "Could not allocate space for particle.");
 ...
 */
static particle_t *create_a_particle(particle_t *seed_p, const int elem_id) {
  particle_t *p, *p_last_keep, *p_next_keep;
  int i;

  if (DPI_ptr->elem_owner[elem_id] != ProcID && seed_p->state != GHOST)
    dump4(EXIT, seed_p,
          "Proc %d tried to create a particle on element %d(%d), which it doesn't own.\n", ProcID,
...
static int rejection_sample_a_particle(void) {
  ...
  if (num_element_samples == -1)
    ...
    if (Particle_Creation_Domain == BRICK) {
      for (i = 0; i < pdim; i++)
        if (p.x[i] < Particle_Creation_Domain_Reals[2 * i] ||
            p.x[i] > Particle_Creation_Domain_Reals[2 * i + 1])
          rejection = 1;
    }
  }
...
static void initialize_and_create_new_particle(particle_t *p, const int elem_id) {
...
static void create_element_particle_info_maps(void) {
  ...
  for (proc_id = 0; proc_id < Num_Proc; proc_id++) {
    DPRINTF(stderr, "%d, ", proc_id);
    fflush(stderr);
    if (proc_id == ProcID) {
      if (output) {
        fp = fopen("map_creation.txt", proc_id ? "a" : "w");
        if (!fp)
          GOMA_EH(GOMA_ERROR, "Could not open map_creation.txt");
      }
      ...
      for (elem_id = 0; elem_id < static_exo->num_elems; elem_id++) {
         ...
         * on unmatched surfaces. */
          ...
          for (side_id = 0; side_id < sides_per_element; side_id++) {
            neighbor_elem_id =
                static_exo->elem_elem_list[static_exo->elem_elem_pntr[elem_id] + side_id];
            if (element_particle_info[elem_id].PBC_side_id[side_id] == -1 &&
              ...
              if (output) {
                fprintf(fp, "Proc %d Elem %4d(%3d) Side %d not linked nodes =", ProcID,
                        global_elem_id, elem_id, side_id);
      ...
      while (msg[0] != -2) {
        ...
        if (msg[0] == -1) {
          ...
          if (elem_id > -1 && DPI_ptr->elem_owner[elem_id] == ProcID) {
            ...
            element_particle_info[elem_id].ghost_proc = (int *)realloc(
                element_particle_info[elem_id].ghost_proc, (ghost_id + 1) * sizeof(int));
            element_particle_info[elem_id].ghost_local_elem_id = (int *)realloc(
                element_particle_info[elem_id].ghost_local_elem_id, (ghost_id + 1) * sizeof(int));
          ...
          for (elem_id = 0; elem_id < static_exo->num_elems && !match_found; elem_id++) {
            ...
            for (side_id = 0; side_id < sides_per_element && !match_found; side_id++) {
              ...
              if (output && 0) {
                fprintf(fp2, "\tComparing to %d %d %d %d\n", node_compares[0], node_compares[1],
                        node_compares[2], node_compares[3]);
                fflush(fp2);
              }
 ...
 * determined earlier. */
static void handle_surface_interaction(particle_t *p, dbl *x, dbl *xi, int PBC_id) {
  int i;
  PBC_t *PBC;
  FILE *fp;
 ...
 * CYLINDRICAL, too).
 */
static void generate_source_particles(const dbl tt, /* parameter to vary time integration */
                                      ...
                                      const dbl creation_time) /* time when particle was created. */
{
  ...
  for (i = 0; i < Particle_Number_PBCs; i++) {
    PBC = &(PBCs[i]);
    if (PBC->type == PBC_SOURCE || PBC->type == PBC_FREESTREAM_SOURCE) {
      SS_id = PBC->SS_id;
        ...
        if (SS_id == static_exo->ss_id[j]) {
          for (k = 0; k < static_exo->ss_num_sides[j]; k++) {
            ...
            if (pd->CoordinateSystem != CARTESIAN && pd->CoordinateSystem != CYLINDRICAL &&
              ...
              GOMA_EH(GOMA_ERROR,
                      "Cannot apply volumetric source particles on this coordinate system.");

            load_element_information(elem_id);
            num_nodes_on_side =
                static_exo->ss_node_side_index[j][k + 1] - static_exo->ss_node_side_index[j][k];
            local_ss_node_list =
                &(static_exo->ss_node_list[j][static_exo->ss_node_side_index[j][k]]);
            for (m = 0; m < num_nodes_on_side; m++) {
              if ((local_elem_node_id[m] =
                       in_list(local_ss_node_list[m], 0, nodes_per_element,
                               &(static_exo->elem_node_list[ei[pg->imtrx]->iconnect_ptr]))) == -1)
                GOMA_EH(GOMA_ERROR, "Bad id_local_elem_coord lookup");
            }
            ...
            */
            ...
            for (ip_id = 0; ip_id < ip_total; ip_id++) {
              ...
              find_surf_st(ip_id, ei[pg->imtrx]->ielem_type, side_id + 1, pd->Num_Dim, xi, &s, &t,
                           &u);
              ...
              surface_determinant_and_normal(
                  elem_id, ei[pg->imtrx]->iconnect_ptr, ei[pg->imtrx]->num_local_nodes,
                  ei[pg->imtrx]->ielem_dim - 1, side_id + 1, num_nodes_on_side, local_elem_node_id);
              for (m = 0; m < mdim; m++)
                inflow_volume +=
                    -(fv->v[m] * fv->snormal[m]) * wt * fv->sdet; /* Normal is outward pointing. */
            }
            inflow_volume *= dt;
            ...
            if (expected_particles < 0.0) {
              fprintf(stderr,
                      "# expected particles = %g on elem %d side %d with inflow volume = %g\n",
                      expected_particles, elem_id, side_id, inflow_volume);
              GOMA_EH(GOMA_ERROR, "Negative number of expected particles.");
            }
 ...
 * passed particle.*/
static void zero_a_particle(particle_t *p) {
  p->state = ACTIVE;
  p->theta = M_PIE;
  p->theta_old = M_PIE;
...
static void initialize_surface_interactions(void) {
  ...
  for (i = 0; i < Particle_Number_PBCs; i++) {
    ...
    case PBC_IMPERMEABLE:
       ...
       * and catch it later. */
      GOMA_EH(GOMA_ERROR, "CGM not supported, IMPERMEABLE PBC.");
      /*
...
static void load_element_information(const int elem_id) {
  int err;
  int i;

  if (last_element_loaded == elem_id)
    return;
  err = load_elem_dofptr(elem_id, static_exo, static_x, static_x_old, static_xdot, static_xdot_old,
                         0);
  GOMA_EH(err, "Error from load_elem_dof_ptr()");
  ...
  for (i = 0; i < Num_Basis_Functions; i++)
    if (pd_glob[ei[pg->imtrx]->mn]->i[pg->imtrx][VELOCITY1] == bfd[i]->interpolation)
      velo_interp = i;
  if (velo_interp == -1)
    GOMA_EH(GOMA_ERROR, "Could not find velocity interpolation function.");
 ...
 * node_coord[][]. */
static void load_element_node_coordinates(const int elem_id) {
  int i, j, k;
...
static void fill_hex_side_indices(const int side_id, int indices[4]) {
  switch (side_id) {
  case 0:
    indices[0] = 0;
    indices[1] = 1;
  ...
  case 5:
    ...
    break;
  default:
    GOMA_EH(GOMA_ERROR, "Bad side_id.");
    break;
  }
}
 ...
 */
static void select_random_side_location(const int elem_id, const int side_id, dbl *x, dbl *xi) {
  int i, j, indices[4];
...
static dbl get_side_area(const int elem_id, const int side_id) {
  int i, indices[4];
  ...
  load_element_node_coordinates(elem_id);
  if (mdim == 2)
    return my_distance(&(node_coord[side_id][0]), &(node_coord[(side_id + 1) % 4][0]), mdim);
  else {
    fill_hex_side_indices(side_id, indices);
    /* Get diagonals... */
    for (i = 0; i < 3; i++) {
 ...
 * load... */
static void load_field_variables_at_xi(const int elem_id, dbl *const xi) {
  int err;
  int i, xis_equal;
  ...
  GOMA_EH(err, "Error from load_basis_functions()");

  err = beer_belly();
  GOMA_EH(err, "Error from beer_belly()");

  err = load_fv();
  GOMA_EH(err, "Error from load_fv()");
  if (TimeIntegration == STEADY) {
  ...
  err = load_bf_grad();
  GOMA_EH(err, "Error from load_bf_grad()");

  err = load_fv_grads();
  GOMA_EH(err, "Error from load_fv_grads()");
 ...
 */
static int get_element_xi_newton(const int elem_id, const dbl *const x, dbl *xi) {
  ...
  fprintf(stderr, "x = (%g,%g,%g), xi = (%g,%g,%g), fv->x = (%g,%g,%g), resid = %g (%g,%g,%g)\n",
          x[0], x[1], x[2], xi[0], xi[1], xi[2], fv->x[0], fv->x[1], fv->x[2], resid_norm, resid[0],
resid[1], resid[2]); fflush(stderr);
  */

  iter = 0;
  while (resid_norm > termination_residual && iter < 100) {
    ...
    iter++;
    /*
    fprintf(stderr, "  GEXN update = (%g,%g,%g), xi = (%g,%g,%g), resid = (%g,%g,%g), resid norm =
    %g\n", update[0], update[1], update[2], xi[0], xi[1], xi[2], resid[0], resid[1], resid[2],
    resid_norm); fprintf(stderr, "  GEXN fv->x = (%g,%g,%g)\n", fv->x[0], fv->x[1], fv->x[2]);
...
*/
  ...
  fv->x[2], xi[0], xi[1], xi[2]); fprintf(stderr, "  GEXN terminated with resid = %g, steps =
  %d\n\n", resid_norm, iter);
  */
 ...
 * basis.
 */
static dbl get_element_minimum_side_length(const int elem_id) {
  int edge_id;
  dbl len = 1.0e+10;
 ...
 */
static void
output_TECPLOT_zone_info(const dbl end_time, const int goma_time_step, const int force) {
  ...
  */
  ...
  if ((TimeIntegration == TRANSIENT && Particle_Output_Stride > 0 &&
       (!((goma_time_step + 1) % Particle_Output_Stride))) ||
 ...
 */
static void output_a_particle(particle_t *const p,
  ...
  if (Particle_Output_Stride > 0) {
    if ((!((goma_time_step + 1) % Particle_Output_Stride) || force_output) && last_step) {
    ...
    for (output_n = start_n; output_n < end_n; output_n++) {
      if (particle_time == 0.0) {
        ...
        time_fraction = (output_time - last_particle_time) / (particle_time - last_particle_time);
      }
      for (i = 0; i < pdim; i++)
      ...
      if (output_sample_number != -1) {
        ...
        for (i = 0; i < Particle_Number_Output_Variables[output_sample_number]; i++) {
          real_data_time =
              (1.0 - time_fraction) * p->real_data_old[i] + time_fraction * p->real_data[i];
  ...
  if (Particle_Full_Output_Stride > 0) {
    ...
    last_goma_step =
        ((TimeIntegration == STEADY) && (goma_time_step == Particle_Max_Time_Steps - 1)) ||
        ((TimeIntegration == TRANSIENT) &&
         ((goma_time_step == tran->MaxTimeSteps - 1) || (particle_time == tran->TimeMax)));

    /*
    fprintf(stderr, "Proc%d: In oap/full, strided_output = %d, last_step = %d, last_goma_step = %d,
    force_output=%d\n", ProcID, strided_output, last_step, last_goma_step, force_output);
    */

    if ((strided_output && last_step) || (last_goma_step && last_step) || force_output) {
      for (i = 0; i < pdim; i++)
...
static void store_particle_data(particle_t *const p) {
  char *output_var;
...
static dbl compute_particle_dt(particle_t *const p, const dbl max_dt) {
  ...
  case INERTIAL_TRACER_EXPLICIT:
    GOMA_EH(GOMA_ERROR, "INERTIAL_TRACER_EXPLICIT offline -- need a better mover.");
    break;
  ...
  case INERTIAL_TRACER_IMPLICIT:
    ...
    /* Get the acceleration values and from that adjust timestep if necessary.  We assume that the
       fluid velocity doesn't change appreciably over the timestep for CFL purposes. */
    for (i = 0; i < pdim; i++)
      forces[i] = stokes_force_coeff * (fv->v[i] - p->v[i]) - fv->grad_P[i] * volume;
    ...
    /*       max_vel_diff =  0.0; */
    /*       for(i = 0; i < pdim; i++) */
    /* 	max_vel_diff = MAX(max_vel_diff, MAX(fabs(fv->v[i] - p->v[i]), fabs(fv_old->v[i] -
     * p->v[i]))); */
    /*       if(max_vel_diff > 0.0) */
    /* 	tmp_dt = sqrt(Particle_CFL_Maximum * min_side_length / (stokes_force_coeff / mass *
     * max_vel_diff)); */
    /*       else */
    /* 	tmp_dt = 1.0e+10; */
    /*       dt = MIN(dt, tmp_dt); */
    ...
    /* 	    tmp_dt = sqrt(Particle_CFL_Maximum * min_side_length / (gravity_force_coeff *
     * relative_mass)); */
    /* 	  else */
    /* 	    tmp_dt = 1.0e+10; */
    /* 	} */
  ...
  case CHARGED_TRACER_EXPLICIT:
    GOMA_EH(GOMA_ERROR, "CHARGED_TRACER_EXPLICIT offline -- need a better mover.");
    break;

  case CHARGED_TRACER_IMPLICIT:
    /* These need to match the same values used in move_particle(). */
    mass =
        4.0 / 3.0 * M_PIE * Particle_Density * Particle_Radius * Particle_Radius * Particle_Radius;
    stokes_force_coeff = 6.0 * M_PIE * mp->viscosity * Particle_Radius;
  ...
  case DIELECTROPHORETIC_TRACER_IMPLICIT:
    ...
    dielectrophoretic_force_coeff = 2.0 * M_PIE * Particle_Radius * Particle_Radius *
    ...
    */
    ...
    for (i = 0; i < pdim; i++) {
      ...
      /*
      fprintf(stderr, "\ti=%d sfc/m=%g sfc/m(%g)=%g\n", i, stokes_force_coeff/mass, fv->v[i] -
      p->v[i], stokes_force_coeff/mass*(fv->v[i]-p->v[i])); fprintf(stderr, "\ti=%d dfc/m=%g
      dfc/m(2)(%g) = dfc/m(2)(%g)(%g)=%g\n", i, dielectrophoretic_force_coeff/mass, fv->Enorm *
      fv->grad_Enorm[i], fv->Enorm, fv->grad_Enorm[i], dielectrophoretic_force_coeff/mass * 2.0 *
      fv->Enorm * fv->grad_Enorm[i]);
      */
      ...
      stokes_force_coeff/mass*(fv->v[i]-p->v[i]), dielectrophoretic_force_coeff/mass * 2.0 *
      fv->Enorm * fv->grad_Enorm[i]);
      */
    }
    if (max_force > 0.0) {
...
static int move_particle(particle_t *const p,
                         dbl *max_particle_dt,
  ...
  case SWIMMER_EXPLICIT:
    ...
    if (Particle_Model == SWIMMER_EXPLICIT) {
      ...
      weiner_speed = drand_truncated_normal(Particle_Model_Data[2], Particle_Model_Data[1] *
                                                                        Particle_Model_Data[2] *
                                                                        sqrt(particle_dt));

      /*
      fprintf(stderr, "Proc%d: beta = %g, mass = %g\n", ProcID, beta, mass);
      fprintf(stderr, "Proc%d: torque = %g\n", ProcID, 980.0 * Particle_Radius * 0.05 * mass *
      sin(p->theta_old) / beta);
      */

      if (mdim == 2) {
        /* For 2D, only one angle, theta.  theta = pi is up. */
        p->theta_old = p->theta;
        p->theta += particle_dt * (980.0 * Particle_Radius * 0.05 * mass * sin(p->theta_old) /
                                   beta) /* gravity-induced torque */
    ...
    } else if (Particle_Model_Data[0] != 0.0)
      GOMA_EH(GOMA_ERROR, "Set up random components!");
  ...
  case CHARGED_TRACER_EXPLICIT: /* This is *always* INERTIAL!! */
    ...
    for (i = 0; i < pdim; i++)
      current_grad_V[i] =
          (1.0 - time_fraction) * (my_fv_old->grad_V[i]) + time_fraction * (fv->grad_V[i]);
    ...
    for (i = 0; i < pd->Num_Dim; i++)
      total_velocity[i] =
          p->v[i] + particle_dt * (stokes_force_coeff / mass * (current_v[i] - p->v[i]) +
  ...
  case CHARGED_TRACER_IMPLICIT:
    ...
    for (i = 0; i < pdim; i++) {
      ...
      b[pdim + i] = p->v[i] - p->v_old[i] -
                    particle_dt * (stokes_force_coeff / mass * (current_v[i] - p->v[i]) +
                                   coulombic_force_coeff / mass * current_grad_V[i]);
    }
    ...
    */
    ...
    update_norm = 1.0e+10;
    newton_iterations = 0;
    while (b_norm > 1.0e-8 && update_norm > 1.0e-8 && newton_iterations < 100) {
    ...
    if (newton_iterations == 100 && b_norm > 1.0e-8 && update_norm > 1.0e-8) {
      fprintf(stderr, "\nmove_particle() says:\n");
  ...
  case DIELECTROPHORETIC_TRACER_IMPLICIT:
    ...
    for (i = 0; i < pdim; i++)
      current_grad_Enorm[i] =
          (1.0 - time_fraction) * (my_fv_old->grad_Enorm[i]) + time_fraction * (fv->grad_Enorm[i]);
    ...
    for (i = 0; i < pdim; i++) {
      b[i] = p->x[i] - p->x_old[i] - particle_dt * p->v[i];
      b[pdim + i] = mass * (p->v[i] - p->v_old[i]) -
                    particle_dt * (stokes_force_coeff * (current_v[i] - p->v[i]) +
                                   dielectrophoretic_force_coeff * 2.0 * current_Enorm *
                                       current_grad_Enorm[i]);
    ...
    while (b_norm > 1.0e-8 && update_norm > 1.0e-8 && newton_iterations < 100) {
      ...
      */
      ...
      for (i = 0; i < pdim; i++) {
        ...
        for (j = 0; j < pdim; j++)
          J[pdim + i][j] = -particle_dt * (stokes_force_coeff * fv->grad_v[j][i] +
                                           dielectrophoretic_force_coeff * 2.0 *
                                               current_grad_Enorm[j] * current_grad_Enorm[i]);
      }
      /*
      ...
      */
      ...
      fill_my_fv_old();
      for (i = 0; i < pdim; i++)
        current_grad_Enorm[i] = (1.0 - time_fraction) * (my_fv_old->grad_Enorm[i]) +
                                time_fraction * (fv->grad_Enorm[i]);
      ...
      for (i = 0; i < pdim; i++) {
        ...
        /*
        fprintf(stderr, "b[%d] = %g * (%g) - %g * ( %g * (%g) + %g * %g * %g\n",
                i, mass, p->v[i]-p->v_old[i], particle_dt, stokes_force_coeff, current_v[i] -
        p->v[i], dielectrophoretic_force_coeff*2.0, current_Enorm, current_grad_Enorm[i]);
        */
      }
      b_norm = nnorm(2 * pdim, b);
    ...
    if (xi_max_np1 > xi_target) {
      ...
      */
      ...
      while (((fabs(xi_max_np1 - xi_target) > xi_tolerance && /* not near the target */
              ...
              (b_norm >= 1.0e-8 && update_norm > 1.0e-8)) &&
        ...
        */
        ...
        xi_max_np1 = MAX(fabs(p->xi[0]), MAX(fabs(p->xi[1]), fabs(p->xi[2])));
        /*
        fprintf(stderr, "XI_MAX new solution |xi_max_np1-xi_target| = %.16g <=
  ...
  case INERTIAL_TRACER_IMPLICIT:
    ...
    */
    ...
    /* Forces are Stokes, gravity, and pressure gradient.  The latter two are often combined (for
       vertical flow) as "buoyancy".
    ...
    */
    ...
    for (i = 0; i < pdim; i++) {
      ...
      b[pdim + i] = p->v[i] - p->v_old[i] -
                    particle_dt / mass *
                        (stokes_force_coeff * (current_v[i] - p->v[i]) + -fv->grad_P[i] * volume);
    }
    ...
    if (newton_iterations == 100 && b_norm > 1.0e-8 && update_norm > 1.0e-8) {
      ...
      fprintf(stderr, "resid = (%g,%g,%g,%g,%g,%g), resid_norm = %g\n", b[0], b[1], b[2], b[3],
              b[4], b[5], b_norm);
      fprintf(stderr, "p->v[i] -  p->v_old[i] - particle_dt/mass * (stokes * (current_v[i] - "
                      "p->v[i]) - fv->grad_P[i] * volume) =\n");
      fprintf(stderr, "  i=0: %g -  %g - %g / %g * (%g * (%g - %g) - %g * %g)\n", p->v[0],
              p->v_old[0], particle_dt, mass, stokes_force_coeff, current_v[0], p->v[0],
              fv->grad_P[0], volume);
      fprintf(stderr, "  i=1: %g -  %g - %g / %g * (%g * (%g - %g) - %g * %g)\n", p->v[1],
              p->v_old[1], particle_dt, mass, stokes_force_coeff, current_v[1], p->v[1],
              fv->grad_P[1], volume);
      fprintf(stderr, "  i=2: %g -  %g - %g / %g * (%g * (%g - %g) - %g * %g)\n", p->v[2],
              p->v_old[2], particle_dt, mass, stokes_force_coeff, current_v[2], p->v[2],
              fv->grad_P[2], volume);
      ...
      fprintf(stderr, "fv->v = (%g,%g,%g), fv_old->v = (%g,%g,%g)\n", fv->v[0], fv->v[1], fv->v[2],
              fv_old->v[0], fv_old->v[1], fv_old->v[2]);
      fprintf(stderr, "particle_dt = %g\n", particle_dt);
      dump1(EXIT, p, "Failed to converge in 100 Newton iterations.");
    }
    return_value = newton_iterations;
 ...
 * my_dim. */
static dbl my_distance(const dbl *x, const dbl *y, int my_dim) {
  int i;
  dbl tmp, tmp2;
...
static void enforce_impermeable_PBCs(particle_t *const p, dbl *total_velocity, dbl particle_dt) {}
...
static dbl drand_truncated_normal(const dbl mean, const dbl stddev) {
  dbl x = 3.1;
 ...
 */
static void dump_fcn(
    const char *const file_name, const int line_number, const int exit_too, particle_t *p, ...) {
  va_list args;
  ...
  switch (p->state) {
  case ACTIVE:
    strcpy(state_string, "ACTIVE");
    break;
  case DEAD:
    strcpy(state_string, "DEAD");
    break;
  case PROC_TRANSFER:
    strcpy(state_string, "PROC_TRANSFER");
    break;
  case TERMINATION_MARKER:
    strcpy(state_string, "TERMINATION_MARKER");
    break;
  case GHOST:
    strcpy(state_string, "GHOST");
    break;
  default:
  ...
  fprintf(stderr, "  p->x = (%g,%g,%g), p->x_old = (%g,%g,%g)\n", p->x[0], p->x[1], p->x[2],
          p->x_old[0], p->x_old[1], p->x_old[2]);
  fprintf(stderr, "  p->xi = (%g,%g,%g), p->xi_old = (%g,%g,%g)\n", p->xi[0], p->xi[1], p->xi[2],
          p->xi_old[0], p->xi_old[1], p->xi_old[2]);
  fprintf(stderr, "  p->v = (%g,%g,%g), p->v_old = (%g,%g,%g)\n", p->v[0], p->v[1], p->v[2],
          p->v_old[0], p->v_old[1], p->v_old[2]);
 ...
 */
static void advance_a_particle(particle_t *p,
  ...
  while (!done) {
    ...
    newton_iterations =
        move_particle(p, &particle_dt, global_start_time, global_end_time - global_start_time);
    p->time += particle_dt;
    ...
    */
    ...
    if (p->time == global_end_time || p->state != ACTIVE)
      done = 1;
...
static void add_to_send_list(particle_t *p) {
 ...
 * be allocated, too. */
static void add_to_do_list(particle_t *p) {
  particle_t *p_recv;

  if (!particles_to_do) {
    particles_to_do = (particle_t *)malloc(sizeof(particle_t));
    if (!particles_to_do)
      GOMA_EH(GOMA_ERROR, "Could not malloc particle space.");
...
static void couple_to_continuum(void) {
  ...
  int *num_particles_in_elem, *num_ghost_particles_to_send, *num_ghost_particles_to_recv,
      *num_ghost_particles_copied;
  particle_t *p_tmp, **ghost_particles_to_send, *ghost_particles_to_recv;
  MPI_Status mpi_status;

  use_ghost_particles = (Particle_Model == SWIMMER_EXPLICIT || Particle_Model == SWIMMER_IMPLICIT);
  ...
  if (use_ghost_particles) {
    ...
    for (i = 0; i < static_exo->num_elems; i++)
      for (j = 0; j < element_particle_info[i].num_ghost_target_elems; j++)
        num_ghost_particles_to_send[element_particle_info[i].ghost_proc[j]] +=
            num_particles_in_elem[i];
    for (i = 0; i < Num_Proc; i++)
    ...
    for (i = 0; i < Num_Proc; i++)
      if (num_ghost_particles_to_send[i])
        ghost_particles_to_send[i] =
            (particle_t *)calloc((unsigned)num_ghost_particles_to_send[i], sizeof(particle_t));
      else
      ...
      if (element_particle_info[i].num_ghost_target_elems)
        for (p = element_particle_list_head[i]; p; p = p->next)
          for (j = 0; j < element_particle_info[i].num_ghost_target_elems; j++) {
            ...
            memcpy(&ghost_particles_to_send[recipient_proc]
                                           [num_ghost_particles_copied[recipient_proc]],
                   p, sizeof(particle_t));
            ghost_particles_to_send[recipient_proc][num_ghost_particles_copied[recipient_proc]]
                .owning_elem_id = recipient_local_elem_id;
            num_ghost_particles_copied[recipient_proc]++;
          }
      ...
      if (i == ProcID) {
        ...
        for (j = 0; j < Num_Proc; j++)
          if (i != j && num_ghost_particles_to_send[j])
            MPI_Send(&ghost_particles_to_send[j][0],
                     num_ghost_particles_to_send[j] * sizeof(particle_t), MPI_BYTE, j, 0,
                     MPI_COMM_WORLD);
        ...
        if (num_ghost_particles_to_recv[ProcID]) {
          ghost_particles_to_recv = (particle_t *)calloc(
              (unsigned)num_ghost_particles_to_recv[ProcID], sizeof(particle_t));
          temp_ust = ust();
          MPI_Recv(ghost_particles_to_recv,
                   num_ghost_particles_to_recv[ProcID] * sizeof(particle_t), MPI_BYTE, i, 0,
                   MPI_COMM_WORLD, &mpi_status);
          communication_accum_ust += MAX(ust() - temp_ust, 0.0);
          for (j = 0; j < num_ghost_particles_to_recv[ProcID]; j++) {
            ghost_particles_to_recv[j].state = GHOST;
            p = create_a_particle(&ghost_particles_to_recv[j],
                                  ghost_particles_to_recv[j].owning_elem_id);
  ...
  MPI_Reduce(&local_num_particle_ghosts, &total_num_particle_ghosts, 1, MPI_INT, MPI_SUM, 0,
             MPI_COMM_WORLD);
#endif
  ...
  if (Particle_Model == SWIMMER_EXPLICIT || Particle_Model == SWIMMER_IMPLICIT) {
    ...
    if (mp->DensityModel != CONSTANT)
      GOMA_EH(GOMA_ERROR,
              "Sorry, only handling CONSTANT density models right now.  Check back later.");
    source_mass = 4.0 / 3.0 * M_PIE * (Particle_Density - mp->density) * Particle_Radius *
                  Particle_Radius * Particle_Radius * Particle_Ratio;
  }
    ...
    for (p = element_particle_list_head[i]; p; p = p->next) {
      if (Particle_Model == SWIMMER_EXPLICIT || Particle_Model == SWIMMER_IMPLICIT) {
        ...
        for (j = 0; j < ei[pg->imtrx]->dof[source_eqn]; j++) {
          element_particle_info[p->owning_elem_id].source_term[j] +=
              source_mass * bf[source_eqn]->phi[j];
    ...
    if (proc_id == ProcID) {
      ...
      for (elem_id = 0; elem_id < static_exo->num_elems; elem_id++) {
        if (DPI_ptr->elem_owner[elem_id] != ProcID) {
          ...
          fprintf(fp, "Proc %2d Elem %4d(%3d)\n", ProcID, DPI_ptr->elem_index_global[elem_id],
                  elem_id);
...
static void load_restart_file(void) {
  ...
  if (!(fp = fopen(construct_filename(Particle_Restart_Filename), "r"))) {
    ...
    GOMA_EH(GOMA_ERROR, "Could not open restart file");
  }
  zero_a_particle(&p);
  while (!feof(fp)) {
    char *fgetserr;
    ...
    if (get_element_xi_newton(p.owning_elem_id, p.x, p.xi) == -1)
      GOMA_EH(GOMA_ERROR, "Could not place particle from restart.");
    load_field_variables_at_xi(p.owning_elem_id, p.xi);
    for (i = 0; i < pdim; i++)
      if (fabs(fv->x[i] - p.x[i]) > 1.0e-6)
        GOMA_EH(GOMA_ERROR, "Did not match particle location on restart.");

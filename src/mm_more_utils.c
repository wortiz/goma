...
int goal_post_nodal(const int var) {
  int post_flag = FALSE;
  int mat;
  int value;
  ...
  for (mat = 0; mat < upd->Num_Mat; mat++) {
    post_flag |=
        (pd_glob[mat]->i[pg->imtrx][var] == I_Q1 || pd_glob[mat]->i[pg->imtrx][var] == I_Q2 ||
         pd_glob[mat]->i[pg->imtrx][var] == I_Q1_G || pd_glob[mat]->i[pg->imtrx][var] == I_Q2_G ||
         pd_glob[mat]->i[pg->imtrx][var] == I_Q1_GP || pd_glob[mat]->i[pg->imtrx][var] == I_Q2_GP ||
         pd_glob[mat]->i[pg->imtrx][var] == I_Q1_GN || pd_glob[mat]->i[pg->imtrx][var] == I_Q2_GN ||
         pd_glob[mat]->i[pg->imtrx][var] == I_Q1_XV || pd_glob[mat]->i[pg->imtrx][var] == I_Q2_XV ||
         pd_glob[mat]->i[pg->imtrx][var] == I_Q1_XG || pd_glob[mat]->i[pg->imtrx][var] == I_Q2_XG ||
         pd_glob[mat]->i[pg->imtrx][var] == I_Q1_HV || pd_glob[mat]->i[pg->imtrx][var] == I_Q1_HG ||
         pd_glob[mat]->i[pg->imtrx][var] == I_Q1_HVG ||
         pd_glob[mat]->i[pg->imtrx][var] == I_Q2_HV || pd_glob[mat]->i[pg->imtrx][var] == I_Q2_HG ||
...
int goal_post_elem(const int var) {
  int post_flag = FALSE;
  int mat;
  int value;
  ...
  if (post_flag) {
    if (Num_Var_In_Type[pg->imtrx][var] > 1) {
      fprintf(stderr, "%s: Too many components in variable type for element variable\n", yo);
      exit(-1);
    }
    value = Num_Var_In_Type[pg->imtrx][var];
  } else {
...
int set_nv_tkud(RESULTS_DESCRIPTION_STRUCT *r, /* of just results for Exodus II */
                const int i,                   /* index */
 ...
 ******************************************************************/
{
  int status = 0;
  if (i > MAX_NNV) {
    GOMA_EH(GOMA_ERROR, "too many nodal post-process variables for exodus");
  }
  r->nvtype[i] = v;
  r->nvkind[i] = k;
...
int set_ev_tkud(RESULTS_DESCRIPTION_STRUCT *r, /* elem result of Exodus II       */
                const int i,                   /* index */
                const int v,                   /* variable index */
                const char *name,              /* name (short)*/
                const char *unit,              /* units (unused) */
                                                  ...
                                                  derivative or not */
{
  int status = 0;
  if (i > MAX_NEV) {
    GOMA_EH(GOMA_ERROR, "too many element post-process variables for exodus");
  }
  r->evtype[i] = v;
...
int load_global_var_info(struct Results_Description *r, /* global results Exodus */
                         const int i,                   /* index */
                         const char *name)              /* name (short)*/
{
  int status;
  ...
  if (r->ngv > MAX_NGV)
    GOMA_EH(GOMA_ERROR, "too many global post-process varibles for exodus.  Change in rf_solve.c "
                        "and in names in load_global_var_info");
...
void sum_total_stress(double sol_vec[], int var_no, int k, double nodal_vec[], Exo_DB *exo)
  ...
  for (eb_index = 0; eb_index < exo->num_elem_blocks; eb_index++) {
    ...
    for (ielem = e_start; ielem < e_end; ielem++) {
      ...
      if (pd->v[pg->imtrx][var_no] &&
          (pd->i[pg->imtrx][var_no] == I_Q1 || pd->i[pg->imtrx][var_no] == I_Q1_G ||
           pd->i[pg->imtrx][var_no] == I_Q1_GP || pd->i[pg->imtrx][var_no] == I_Q1_GN ||
           pd->i[pg->imtrx][var_no] == I_Q1_XV || pd->i[pg->imtrx][var_no] == I_Q1_XG ||
           pd->i[pg->imtrx][var_no] == I_Q1_HV || pd->i[pg->imtrx][var_no] == I_Q1_HG ||
           pd->i[pg->imtrx][var_no] == I_Q1_HVG || pd->i[pg->imtrx][var_no] == I_SP) &&
        ...
        for (i = 4; i < 8; i++) {
          ...
          if (Index_Solution(I, var_no, ktype, 0, mn, pg->imtrx) == -1) {
            ...
            nodal_vec[I] = 0.5 * (sol_vec[Index_Solution(Ileft, var_no, ktype, 0, mn, pg->imtrx)] +
                                  sol_vec[Index_Solution(Iright, var_no, ktype, 0, mn, pg->imtrx)]);
            var = var_no + 24;
            for (mode = 1; mode < vn->modes; mode++) {
              nodal_vec[I] += 0.5 * (sol_vec[Index_Solution(Ileft, var, ktype, 0, mn, pg->imtrx)] +
          ...
          if (ielem_type == BIQUAD_QUAD) {
            ...
            if (Index_Solution(I, var_no, ktype, 0, mn, pg->imtrx) == -1) {
              for (ileft = 0; ileft < 4; ileft++) {
                Ileft = Proc_Elem_Connect[iconnect_ptr + ileft];
                nodal_vec[I] +=
                    0.25 * sol_vec[Index_Solution(Ileft, var_no, ktype, 0, mn, pg->imtrx)];
                var = var_no + 24;
                for (mode = 1; mode < vn->modes; mode++) {
                  nodal_vec[I] +=
                      0.25 * sol_vec[Index_Solution(Ileft, var, ktype, 0, mn, pg->imtrx)];
                  var += 6;
                }
              }
            }
...
void extract_nodal_vec(double sol_vec[],
                       int var_no,
                       int ktype,
                       int matIndex,
                       double nodal_vec[],
...
void extract_nodal_eb_vec(double sol_vec[],
                          int var_no,
                          int ktype,
                          int matIndex,
                          int eb_index,
 ...
 *********************************************************************************/
{
  ...
  for (ielem = e_start; ielem < e_end; ielem++) {
    ...
    midside = 0;
    if (((pd->i[pg->imtrx][var_no] == I_Q1) || (pd->i[pg->imtrx][var_no] == I_Q1_G) ||
         (pd->i[pg->imtrx][var_no] == I_Q1_GP) || (pd->i[pg->imtrx][var_no] == I_Q1_GN) ||
         (pd->i[pg->imtrx][var_no] == I_Q1_XV) || (pd->i[pg->imtrx][var_no] == I_Q1_XG) ||
         (pd->i[pg->imtrx][var_no] == I_Q1_HV) || (pd->i[pg->imtrx][var_no] == I_Q1_HG) ||
...
void anneal_map(const int dim, const double X_old[], const double displacement[], double X_new[]) {
  int p;
  double factor = 1.0;
...
int get_new_coord(double *new_coord[DIM], double *x, const Exo_DB *exo) {
  int p, i, j;
  int dim = exo->num_dim;
  ...
  if (displacement_somewhere == FALSE)
    return (FALSE);
...
void elements_attached_to_NS(int *element_list, int NS_ID, Exo_DB *exo) {
  int index;
  int num_nodes;
  int *ns_node_list;
  int i, j, I, e;
  ...
  if ((index = in_list(NS_ID, 0, exo->ns_node_len, exo->ns_id)) == -1) {
    GOMA_EH(GOMA_ERROR, "Node set ID not found\n");
  }

...
void chemkin_not_linked(char *errString)
...
int ck_decide_vol_chem(CPC_VOLDOMAIN_STRUCT *volD)
 ...
 *************************************************************************/
{
   ...
   */
  if ((volD->NVolPhase > 1) || (volD->NSurPhase > 0))
...
int chemkin_mat_prop_init(MATRL_PROP_STRUCT *mat_ptr, int mn, PROBLEM_DESCRIPTION_STRUCT *pd_ptr)
 ...
 *************************************************************************/
{
  ...
  if (!chemkinDatabaseRead) {
    ...
    retn = cpc_initialize_1p(TRUE, ProcID, Num_Proc, initGasTransport, initLQCHEMKIN, TRUE,
                             DomainMappingFile);
    if (retn != CPC_SUCCESS) {
   ...
   */
  mat_ptr->Species_Names = cpc_alloc_VecFixedStrings(mat_ptr->Num_Species, CPC_MAX_NAME_LEN_P1);
   ...
   */
  cpc_iset(mat_ptr->MolecularWeightModel, mat_ptr->Num_Species, CHEMKIN_MODEL);
   ...
   *  types to NO_MODEL.
   */
  cpc_iset(mat_ptr->VaporPressureModel, mat_ptr->Num_Species, NO_MODEL);
  cpc_iset(mat_ptr->LatentHeatFusionModel, mat_ptr->Num_Species, NO_MODEL);
  cpc_iset(mat_ptr->LatentHeatVapModel, mat_ptr->Num_Species, NO_MODEL);
...
void chemkin_initialize_mp(void)
 ...
 *************************************************************************/
{
  ...
  pl = 0;
#endif
  retn = cpc_initialize_mp(pl, Num_Proc, infoproc, ProcID, initGasTransport, initLQCHEMKIN,
                           initCKExtractGuts);
  if (retn != CPC_SUCCESS)
    exit(-1);
}

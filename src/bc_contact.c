...
int apply_contact_bc(double x[],              /* Solution vector for the current processor    */
 ...
 ****************************************************************************/
{
  ...
  for (ip = 0; ip < ip_total; ip++) {
    ...
    if (Do_Overlap && ls->CrossMeshQuadPoints > 0) {
      find_subsurf_st_wt(ip, ls->CrossMeshQuadPoints, ielem_type, id_side, pd->Num_Dim, xi, &wt);
    } else {
    ...
    if (TimeIntegration != STEADY && pd->e[pg->imtrx][MESH_DISPLACEMENT1]) {
      for (icount = 0; icount < ielem_dim; icount++) {
        x_dot[icount] = (1 + 2. * theta) * (fv->x[icount] - fv_old->x[icount]) / delta_t -
      ...
      if (BC_Types[bc_input_id].desc->method == bc_application) {
        ...
        case SOLID_LAGRANGE_MULT_BC:
          ...
          for (aa = 0; aa < ielem_dim; aa++) {
            ...
            if (ac_lm) {
              if (apply_AC)
                lm_fluid[aa] = augc[ioffset + aa].lm_value;
              dof_l[aa] = 1;
              phi_l[aa][0] = 1.0;
          ...
          Lagrange_mult_equation(func, d_func, theta, delta_t, h_elem_avg, lm_fluid, fluid_velocity,
        ...
        case SOLID_FLUID_CONTACT_BC:
          ...
          /*before we do the rest of this evaluation, restore world since
            invert_isoparameteric_map changed this */
          setup_shop_at_point(ielem, xi, exo);
          contact_fn_dot_T(func, d_func, delta_t, lagrange_mult, apply_AC, dof_l, phi_l);
        ...
        case LAGRANGE_NO_SLIP_BC:
          ...
          if (pass == 1)
            ioffset = first_overlap_ac(ielem, elem_side_bc->id_side);
          else if (pass == 2)
            ioffset = oAC;
          ...
          /*before we do the rest of this evaluation, restore world since
            invert_isoparameteric_map changed this */
          setup_shop_at_point(ielem, xi, exo);
          solid_kinematic_bc(func, d_func, delta_t, theta, fluid_velocity, apply_AC, dof_q, phi_q,
                             x_dot);
          break;
        ...
        for (i = 0; i < num_dofs; i++) {
          ...
          for (p = 0; p < BC_Types[bc_input_id].desc->vector; p++) {
            ...
            if (index_eq >= 0 || (((BC_Types[bc_input_id].BC_Name == LAGRANGE_NO_SLIP_BC) ||
                                   (BC_Types[bc_input_id].BC_Name == SOLID_LAGRANGE_MULT_BC)) &&
              ...
              if (ac_lm && pass == 2 && p < 2) {
                ...
                case SOLID_FLUID_CONTACT_BC:
                  ...
                  nu = index_eq;
                  if (nu < 0)
                    GOMA_EH(GOMA_ERROR, "Bad variable index");
                  bAC[iAC][nu] += jac;
...
int jump_down_to_fluid(const Exo_DB *exo, /* Ptr to Exodus database */
 ...
 */
{
  ...
  for (eb_index = 0; eb_index < exo->num_elem_blocks; eb_index++) {
    ...
    if (!ifound && exo->eb_id[eb_index] == BC_Types[bcid].BC_Data_Int[1]) {
      ifound = 1;
  ...
  fluid_mesh_elem_id = find_id_elem(fv->x[0], fv->x[1], fv->x[2], x, exo, e_start, e_end);
  ...
  for (j = 0; j < DIM; j++)
    xi_2[j] = 0.0;
  invert_isoparametric_map(&fluid_mesh_elem_id, &coordinates[0], xi_2, exo, x, &velo_interp);
...
void contact_fn_dot_T(double func[DIM],
                      double d_func[DIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
                      double dt,
...
void Lagrange_mult_equation(double func[DIM],
                            double d_func[DIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
                            double tt,
                            double dt,
                            double h_elem_avg,
...
void solid_kinematic_bc(double func[DIM],
                        double d_func[DIM][MAX_VARIABLE_TYPES + MAX_CONC][MDE],
                        double dt,
                        double tt,
                        double fluid_velocity[DIM],
...
/*****************************************************************************/
int apply_embedded_bc(int ielem,    /* element number */
 ...
 **************************************************************************/
{
  ...
  if (ls->SubElemIntegration && !ls->Ignore_F_deps) {
    ...
    Subgrid_Int.ip_total = get_subelement_integration_pts(&Subgrid_Int.s, &Subgrid_Int.wt,
                                                          &Subgrid_Int.ip_sign, fminus, -1, 0);
      ...
      if (oAC != 0) {
        ...
        if (!ls->Ignore_F_deps) {
          ...
          if (pd->e[pg->imtrx][R_PRESSURE] && !ls->AdaptIntegration) {
            err = assemble_continuity_path_dependence(time_value, theta, dt, pg_data);
            GOMA_EH(err, "assemble_continuity_path_dependence");
          }
          if (pd->e[pg->imtrx][R_ENERGY] && !ls->AdaptIntegration) {
            assemble_energy_path_dependence(time_value, theta, dt, pg_data);
          }
...
/*******************************************************************************/
int assemble_embedded_bc(int ielem,    /* element number */
                         double x[],   /* Solution vector for the current processor    */
                         double dt,    /* current time step size                       */
                         double theta, /* parameter (0 to 1) to vary time integration
                                        *  ( implicit - 0 to explicit - 1)     */
  ...
  while (bcref) {
    ...
    case LS_LATENT_HEAT_BC:
      assemble_ls_latent_heat_source(bc->BC_Data_Float[0], bc->BC_Data_Float[1], dt, theta,
                                     time_value, bcref->bc_input_id, BC_Types);
      break;
    case LS_YFLUX_BC:
      assemble_ls_yflux_source(bc->BC_Data_Int[0], bc->BC_Data_Float[0], bc->BC_Data_Float[1], dt,
                               theta, time_value, bcref->bc_input_id, BC_Types);
      break;
    case LS_CONT_T_BC:
      ...
      if ((ls->Evolution == LS_EVOLVE_SLAVE) && (ls->init_surf_list->start->type == LS_SURF_SS)) {
        ...
        for (a = 0; a < ei[pg->imtrx]->ielem_dim; a++) {
          if (pd->TimeIntegration != STEADY && pd->v[pg->imtrx][MESH_DISPLACEMENT1 + a]) {
            x_dot[a] =
                (1. + 2. * theta) * (fv->x[a] - fv_old->x[a]) / dt - 2. * theta * fv_dot->x[a];
          } else {
            x_dot[a] = 0.;
          }
...
int apply_embedded_colloc_bc(int ielem,    /* element number */
                             double x[],   /* Solution vector for the current processor    */
                             double dt,    /* current time step size                       */
                             double theta, /* parameter (0 to 1) to vary time integration
                                            *  ( implicit - 0 to explicit - 1)     */
  ...
  if ((ls == NULL) || (ls->embedded_bc == NULL) || (xfem == NULL) || (xfem->elem_state == 0))
    return (0);
  ...
  while (bcref) {
    ...
    case LS_T_BC: {
      ...
      if (pd->i[pg->imtrx][eqn] == I_Q1_G || pd->i[pg->imtrx][eqn] == I_Q2_G ||
          ...
          pd->i[pg->imtrx][eqn] == I_Q1_XV || pd->i[pg->imtrx][eqn] == I_Q2_XV) {
        break; /* nothing needs to be done here */
      } else {
        GOMA_EH(GOMA_ERROR,
                "LS_UVW expects XFEM interpolation Q1_XV, Q2_XV, Q1_XG, Q2_XG, Q1_G, or Q2_G\n");
      }
      ...
      for (i = 0; i < ei[pg->imtrx]->dof[eqn]; i += 2) {
        ...
        if (TRUE) {
          ...
          if ((is_neg && bc->BC_ID == 1) || (!is_neg && bc->BC_ID == -1)) {
            continue;
          }
          idof_to = i;
          idof_from = i + 1;
          ...
          if (af->Assemble_Jacobian) {
            for (var = V_FIRST; var < V_LAST; var++) {
              ...
              if (pvar != -1 && (Inter_Mask[pg->imtrx][eqn][var])) {
                for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
                  lec->J[LEC_J_INDEX(peqn, pvar, idof_to, j)] +=
                      bc->BC_ID * lec->J[LEC_J_INDEX(peqn, pvar, idof_from, j)];
                }
              }
            }
          }
    ...
    case LS_CONT_FLUX_BC: {
      ...
      if (pd->i[pg->imtrx][eqn] == I_P0_G || pd->i[pg->imtrx][eqn] == I_P1_G ||
          ...
          if (af->Assemble_Jacobian) {
            for (var = V_FIRST; var < V_LAST; var++) {
              ...
              if (pvar != -1 && (Inter_Mask[pg->imtrx][eqn][var])) {
                for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
                  lec->J[LEC_J_INDEX(peqn, pvar, idof_to, j)] +=
                      lec->J[LEC_J_INDEX(peqn, pvar, idof_from, j)];
                }
              }
            }
          }
    ...
    case LS_CONT_TRACTION_BC: {
      for (a = 0; a < ei[pg->imtrx]->ielem_dim; a++) {
        ...
        if (pd->i[pg->imtrx][eqn] == I_P0_G || pd->i[pg->imtrx][eqn] == I_P1_G ||
          ...
          GOMA_EH(GOMA_ERROR,
                  "LS_CONT_TRACTION_BC expects XFEM interpolation P0_G, P1_G, Q1_G, or Q2_G\n");
        }
...
void find_subsurf_st_wt(const int ip,
                        const int ip_total,
  ...
  case SHELL:
  case TRISHELL:
    GOMA_EH(GOMA_ERROR, "subsurf integration for overset grids and ls subelement integration not "
                        "supported for SHELLS");
    break;
  ...
  if (ip_total == linear_ip_total) {
    ...
    *wt = Gq_surf_weight(ip, quad_type);
    ...
    if (num_dim != 2)
      GOMA_EH(GOMA_ERROR, "Cross Mesh sub-integration only supported in 2-D\n");
...
void find_segment_s_wt(const int iquad, /* current GQ index */
...
double dof_distance(int var_type, int lvdof) {
...
void gnn_distance(const int I,
  ...
  *F = x[ie];
  if (Fold != NULL)
    *Fold = xold[ie];
  if (Fprev != NULL)
    *Fprev = x[ie] + damp_factor * var_damp[idv[pg->imtrx][ie][0]] * delta_x[ie];
}
...
int first_overlap_ac(int ielem, int side)
/*
...
int lookup_active_dof(int var, int j, int eptr)
/*
 ...
 */
{
  ...
  */
  ...
  if (nn->DBC[pg->imtrx] != NULL) {
    ...
    if (pd->i[pg->imtrx][var] == I_P0_G || pd->i[pg->imtrx][var] == I_P1_G ||
        pd->i[pg->imtrx][var] == I_P0_GP || pd->i[pg->imtrx][var] == I_P1_GP ||
        pd->i[pg->imtrx][var] == I_P0_GN || pd->i[pg->imtrx][var] == I_P1_GN ||
        pd->i[pg->imtrx][var] == I_P0_XV || pd->i[pg->imtrx][var] == I_P1_XV ||
        pd->i[pg->imtrx][var] == I_P1_XG || pd->i[pg->imtrx][var] == I_Q1_G ||
        pd->i[pg->imtrx][var] == I_Q2_G || pd->i[pg->imtrx][var] == I_Q1_GP ||
        pd->i[pg->imtrx][var] == I_Q2_GP || pd->i[pg->imtrx][var] == I_Q1_GN ||
        pd->i[pg->imtrx][var] == I_Q2_GN || pd->i[pg->imtrx][var] == I_Q1_XV ||
        pd->i[pg->imtrx][var] == I_Q2_XV || pd->i[pg->imtrx][var] == I_Q1_XG ||
        pd->i[pg->imtrx][var] == I_Q2_XG) {
      if (j % 2 == 0 && nn->DBC[pg->imtrx][noffset] >= 0)
        nu = -2;
    } else {
      if (nn->DBC[pg->imtrx][noffset] >= 0)
        nu = -2;
    }
  }
...
void setup_shop_at_point(int ielem, double *xi, const Exo_DB *exo)
/*
 ...
 */
{
  ...
  if (ielem != ei[pg->imtrx]->ielem) {
    err = load_elem_dofptr(ielem, exo, x_static, x_old_static, xdot_static, xdot_old_static, 0);
...
double fv_at_point(double *xi, int var) {
  int i;
  double *esp, val, phi_i;

  for (i = 0, val = 0.; i < ei[pg->imtrx]->dof[var]; i++) {
    phi_i = newshape(xi, ei[pg->imtrx]->ielem_type, PSI, ei[pg->imtrx]->dof_list[var][i],
...
double fv_old_at_point(double *xi, int var) {
  int i;
  double *esp, val, phi_i;
...
double fv_and_phi_at_point(double *xi, int var, double *phi, int sign) {
  int i;
  double *esp, val;
  int old_sign;

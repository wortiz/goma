...
static void associate_bc_to_matrix(void);

static void set_bc_equation(void) {
  int ibc;
    ...
    if (set_eqn < 0 || set_eqn >= V_LAST) {
      if (eqn >= V_FIRST && eqn < V_LAST) {
        ...
        case SOLID_POSITION3:
          ...
          break;
        default:
          GOMA_EH(GOMA_ERROR, "Error in linking BC to equation");
          break;
        }
      }
    }
 ...
 **********************************************************************/
{
  ...
  lec->J = (dbl *)smalloc(MAX_LOCAL_VAR_DESC * MAX_LOCAL_VAR_DESC * lec->max_dof * lec->max_dof *
                          sizeof(dbl));
  lec->J_stress_neighbor =
      (dbl *)smalloc(4 * lec->max_dof * MAX_LOCAL_VAR_DESC * lec->max_dof * sizeof(dbl));
  pd = pd_glob[0]; // issues if not set currently
  return 0;
}
/************************************************************************/

int free_problem(Exo_DB *exo, /* ptr to the finite element mesh database */
                 Dpi *dpi)    /* distributed processing information */
                              /*
...
static void
check_discontinuous_interp_type(PROBLEM_DESCRIPTION_STRUCT *curr_pd, int var_type, int imtrx)
 ...
 ********************************************************************/
{
  ...
  if (v_ptr[var_type] & V_MATSPECIFIC) {
    ...
    case I_SP:
      ...
      GOMA_EH(GOMA_ERROR, "incompatible interp type");
      break;
    case I_G0:
    case I_G1:
    case I_P0:
    ...
    default:
      ...
      GOMA_EH(GOMA_ERROR, "unknown interpolation type");
    }
  }
}
/************************************************************************/
...
static void
turn_on_discontinuous(PROBLEM_DESCRIPTION_STRUCT *curr_pd, const int var_type, const int imtrx)
...
int coordinate_discontinuous_variables(Exo_DB *exo, Dpi *dpi)
 ...
 *******************************************************************/
{
  ...
  for (ibc = 0; ibc < Num_BC; ibc++) {
    if (BC_Types[ibc].desc->i_apply == CROSS_PHASE_DISCONTINUOUS) {
       ...
       */
      if (eqn_type == R_MOMENTUM1 || eqn_type == R_MOMENTUM2 || eqn_type == R_MOMENTUM3 ||
          eqn_type == R_MOM_NORMAL || eqn_type == R_MOM_TANG1 || eqn_type == R_MOM_TANG2) {
       ...
       */
      if (eqn_type == R_MASS || (eqn_type >= R_SPECIES_UNK_0 && eqn_type <= R_SPECIES_UNK_LAST)) {
        eqn_type = R_SPECIES_UNK_0;
      }

      for (ss_index = 0; ss_index < exo->num_side_sets; ss_index++) {
        if (BC_Types[ibc].BC_ID == exo->ss_id[ss_index]) {
          for (side_index = 0; side_index < exo->ss_num_sides[ss_index]; side_index++) {
              ...
              for (imat = 0; imat < num_mat; imat++) {
                ...
                for (imtrx = 0; imtrx < upd->Total_Num_Matrices; imtrx++) {
                  if (eqn_type == R_MOMENTUM1) {
                    ...
                    for (var_type = R_SPECIES_UNK_0; var_type < R_SPECIES_UNK_LAST; var_type++) {
...
static int find_next_max(int *bin_list, int *flag_list, int num)
...
static int assign_matrl_2(struct Boundary_Condition *bc_ptr, int matrl) {
  if (bc_ptr->BC_matrl_index_1 == matrl || bc_ptr->BC_matrl_index_2 == matrl) {
    return 0;
  }
...
/************************************************************************/
/************************************************************************/
static void bc_matrl_index_print(struct Boundary_Condition *bc_ptr,
                                 int *bin_matrl,
                                 int *bin_matrl_elem,
                                 int min_node_matrl,
                                 int max_node_matrl,
 ...
 *******************************************************************/
{
  ...
  if (max_node_matrl >= 0) {
    ...
    printf("max materials at a node (%d) = %d,", max_node_matrl, matrlLP->Length);
    printf("  their id's = ");
    ...
    matrlLP = &(node_ptr->Mat_List);
    printf("min materials at a node (%d) = %d,", min_node_matrl, matrlLP->Length);
    printf("  their id's = ");
    for (i = 0; i < matrlLP->Length; i++)
      printf("%d ", matrlLP->List[i]);
    printf("\n");
 ...
 ********************************************************************/
{
  ...
  for (ibc = 0; ibc < Num_BC; ibc++) {
     ...
     */
    if (!strcmp(bc_ptr->Set_Type, "LS") || !strcmp(bc_ptr->Set_Type, "PF")) {
      bc_ptr->Set_Index = 0;
    }
    /*
    ...
    if (ss_index == -1) {
      DPRINTF(stderr,
              "On P_%d, bc_set_index: ERROR Can't find ns or ss id %d for"
              " boundary condition %d with type %s \n",
              ProcID, bc_ptr->BC_ID, ibc, bc_ptr->Set_Type);
      GOMA_EH(GOMA_ERROR, "Error in specification of boundary condition");
    }
  }
}
/************************************************************************/
...
void bc_internal_boundary(Exo_DB *exo)
...
void bc_matrl_index(Exo_DB *exo)
 ...
 *******************************************************************/
{
  ...
  for (ibc = 0; ibc < Num_BC; ibc++) {
    ...
    if (i_apply_meth == CROSS_PHASE_DISCONTINUOUS || i_apply_meth == CROSS_PHASE) {
      num_matrl_needed = 2;
              ...
              if (!node_flag_1ss[node_num]) {
                ...
                for (i = 0; i < matrlLP->Length; i++) {
                  ...
                  if (matrlLP->List[i] < 0) {
                    ...
                    GOMA_EH(GOMA_ERROR, "logic error in ignoring an element block");
                  }
#endif
                 ...
                 */
                if (matrlLP->Length == 1)
                  node_matrl_1 = node_num;
                if (matrlLP->Length == 2)
                  node_matrl_2 = node_num;
                if (matrlLP->Length == 3)
                  node_matrl_3 = node_num;
                if (matrlLP->Length == 4)
                  node_matrl_4 = node_num;
              }
            }
          }
        }
    ...
    if (found && n_flagged_nodes > 0) {
      ...
      if (success < 0) {
        printf("%s P_%d: problem in assigning first matrl index in ibc %d, %d:\n", yo, ProcID, ibc,
               matrl_first);
        bc_matrl_index_print(bc_ptr, bin_matrl, bin_matrl_elem, min_node_matrl, max_node_matrl,
    ...
    MPI_Bcast(&(bc_ptr->BC_matrl_index_1), 1, MPI_INT, k, MPI_COMM_WORLD);
    MPI_Bcast(&(bc_ptr->BC_matrl_index_2), 1, MPI_INT, k, MPI_COMM_WORLD);
    MPI_Bcast(&(bc_ptr->BC_matrl_index_3), 1, MPI_INT, k, MPI_COMM_WORLD);
    MPI_Bcast(&(bc_ptr->BC_matrl_index_4), 1, MPI_INT, k, MPI_COMM_WORLD);
 ...
 ********************************************************************/
{
  ...
  for (ibc = 0; ibc < Num_BC; ibc++) {
    ...
    case POROUS_GAS_BC:
      if (!bc_ptr->Internal_Boundary) {
        fprintf(stderr, "Boundary condition %s can't be put on external domain bc\n",
                bc_desc->name1);
        GOMA_EH(GOMA_ERROR, "Boundary Condition incompatibility");
      }
      bc_ptr->DV_Indexing_Type = DVI_VSIG;
      break;
    ...
    case POROUS_FLUX_BC:
      if (bc_ptr->Internal_Boundary) {
        fprintf(stderr,
                " WARNING: Boundary condition %s can't be put on internal domain bc and expect "
                "mass conservation\n",
                bc_desc->name1);
        fprintf(stderr, "\t No attempt at matching fluxes on both sides of interfaces is made\n");
      }
      break;
...
void setup_external_nodal_matrls(Exo_DB *exo, Dpi *dpi, Comm_Ex *cx)
 ...
 ************************************************************************/
{
  ...
  for (p = 0; p < dpi->num_neighbors; p++) {
    ...
    np_ptr->send_message_buf = (void *)(mesg_send + ptr_node_send[p] * max_Matrl);
    np_ptr->send_message_length = sizeof(int) * cx[p].num_nodes_send * max_Matrl;
    np_ptr->recv_message_buf = (void *)(mesg_recv + ptr_node_recv[p] * max_Matrl);
    np_ptr->recv_message_length = sizeof(int) * cx[p].num_nodes_recv * max_Matrl;
    np_ptr++;
  }

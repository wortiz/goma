...
static int set_nodal_Dirichlet_BC(int inode,
                                  int ibc,
...
static void setup_Elem_BC(struct elem_side_bc_struct **elem_side_bc,
...
static int same_side(int[], int[], int);

static void print_Elem_Surf_BC(int ielem, struct elem_side_bc_struct *elem_side_bc);
static void elem_side_matrl_list(struct elem_side_bc_struct *);
static void vcrr_determination(struct elem_side_bc_struct *, struct Boundary_Condition *, int, int);

/*****************************************************************************/
/*****************************************************************************/
/*****************************************************************************/

void find_and_set_Dirichlet(double x[],    /* solution vector at this processor */
                            ...
                            Dpi *dpi)      /* ptr to distrib processing FE database */
                                            ...
                                            */
{
  int side_index, I, mn, ielem, e_start, e_end, eb_index, ielem_type, num_local_nodes, matID,
      elem_block_index, offset;
  int *been_there = NULL;
  ...
  for (ibc = 0; ibc < Num_BC; ibc++) {
    ...
    if (!strcmp(BC_Types[ibc].Set_Type, "NS")) {
       ...
       *  boundary condition
       */
      if (BC_Types[ibc].desc->method == DIRICHLET || BC_Types[ibc].BC_Name == SURFTANG_BC ||
          BC_Types[ibc].BC_Name == SURFTANG_SCALAR_BC || BC_Types[ibc].BC_Name == CAP_ENDFORCE_BC ||
          BC_Types[ibc].BC_Name == CAP_ENDFORCE_SCALAR_BC || BC_Types[ibc].BC_Name == CA_BC ||
          BC_Types[ibc].BC_Name == CA_MOMENTUM_BC || BC_Types[ibc].BC_Name == CA_OR_FIX_BC ||
          BC_Types[ibc].BC_Name == MOVING_CA_BC || BC_Types[ibc].BC_Name == VELO_THETA_TPL_BC ||
          BC_Types[ibc].BC_Name == VELO_THETA_HOFFMAN_BC ||
          BC_Types[ibc].BC_Name == VELO_THETA_COX_BC ||
          BC_Types[ibc].BC_Name == VELO_THETA_SHIK_BC || BC_Types[ibc].BC_Name == SH_SLOPE_X_BC ||
          BC_Types[ibc].BC_Name == SH_SLOPE_Y_BC || BC_Types[ibc].BC_Name == SHEET_ENDSLOPE_BC) {
        ...
        for (ins = 0; ins < exo->num_node_sets; ins++) {
          ...
          if (exo->ns_id[ins] == BC_Types[ibc].BC_ID) {
            ...
            for (i = 0; i < exo->ns_num_nodes[ins]; i++) {
               ...
               */
              error_cond = (set_nodal_Dirichlet_BC(inode, ibc, &BC_Types[ibc], x, xdot, matID) ||
                            error_cond);
        ...
        for (ins = 0; ins < exo->num_side_sets; ins++) {
          ...
          if (Proc_SS_Ids[ins] == BC_Types[ibc].BC_ID) {
            ...
            for (side_index = 0; side_index < exo->ss_num_sides[ins]; side_index++) {
              ...
              if (BC_Types[ibc].BC_EBID_Apply != -1) {
                if (Element_Blocks[elem_block_index].Elem_Blk_Id != BC_Types[ibc].BC_EBID_Apply) {
                  continue;
                }
              }
              /*
          ...
          for (ielem = e_start; ielem < e_end; ielem++) {
            ...
            for (i = 0; i < num_local_nodes; i++) {
              ...
              if (!been_there[I]) {
                 ...
                 * for the current node, inode, and boundary condition
                 */
                error_cond |= set_nodal_Dirichlet_BC(inode, ibc, &BC_Types[ibc], x, xdot, mn);
              }
            }
          } /*  for (ielem = e_start; ielem < e_end; ielem++) */
        }   /* for (eb_index = 0; eb_index < exo->num_elem_blocks; eb_index++)  */
  ...
  if (PRESSURE_DATUM == 1) {
    if (pressure_datum_element < 0 || pressure_datum_element >= dpi->num_elems_global) {
      log_err("Specified pressure datum element %d out of range.", pressure_datum_element);
    }

    local_elem = in_list(pressure_datum_element, 0, dpi->num_elems, dpi->elem_index_global);
  ...
  if (apply_pressure_datum) {
    ...
    for (n = 0; n < num_nodes; n++) {
      ...
      if (Dolphin[pg->imtrx][inode][PRESSURE] > 0 && datum_set) {
        ...
        log_msg("        pressure datum element (local)  = %d", pressure_datum_element);
        log_msg("        pressure datum element (global) = %d",
                dpi->elem_index_global[pressure_datum_element]);
        log_msg("        pressure datum node (local)     = %d", inode);
        log_msg("        pressure datum node (global)    = %d", dpi->node_index_global[inode]);
        log_msg("        pressure datum dof (local)      = %d", ie);
        log_msg("        pressure datum value            = %g", pressure_datum_value);
      }
    }
  }
  return;
 ...
 ********************************************************************/
{
   ...
   */
  if (boundary_condition->desc->method == DIRICHLET && boundary_condition->BC_Name != FIX_BC &&
      boundary_condition->BC_Name != PERIODIC_BC) {

    eqn = boundary_condition->desc->equation;
    if (eqn > V_LAST) {
      GOMA_EH(GOMA_ERROR, "Bad eqn");
    }
    if (eqn != R_MASS) {
      ...
      if (offset >= 0) {
        ...
        if (boundary_condition->BC_Name == DX_USER_NODE_BC ||
          ...
          if (boundary_condition->BC_Name == F_DIODE_BC) {
            GOMA_EH(GOMA_ERROR, "F_DIODE_BC has special needs. You need to comment this line out "
                                "and follow instructions");
            /*real hack here for diode level-set.  Let liquid out, but not back in.
              if you want to use this comment out the EH above and go to bc_dirich.c and uncomment
              the if{} protection clearly marked for this BC */
            if (x[ieqn] <= 0.0) {
      ...
      if (w >= (upd->Max_Num_Species)) {
        GOMA_EH(GOMA_ERROR, "Species number on BC Y card exceeds number of species available");
      }
      offset = get_nodal_unknown_offset(nv, eqn, matID, w, &vd);
      if (offset < 0) {
        GOMA_WH(-1, "Y BC applied to material without species equation");
      } else {
        if (node->DBC[pg->imtrx] == NULL) {
          node->DBC[pg->imtrx] = alloc_short_1(nv->Num_Unknowns, -1);
        }
    ...
    boundary_condition->BC_Data_Float[7] = boundary_condition->BC_Data_Float[4] - (Coor[0][inode]);
    if (boundary_condition->BC_Data_Int[0]) {
      xdot[ieqn] = 0.;
    }
    ieqn = Index_Solution(inode, MESH_DISPLACEMENT2, 0, ndof, matID, pg->imtrx);
    boundary_condition->BC_Data_Float[8] = boundary_condition->BC_Data_Float[5] - (Coor[1][inode]);
    if (boundary_condition->BC_Data_Int[0]) {
      xdot[ieqn] = 0.;
    }
...
void alloc_First_Elem_BC(struct elem_side_bc_struct ****First_Elem_Side_BC_Array,
 ...
 ***********************************************************************/
{
  ...
  *First_Elem_Edge_BC_Array = malloc(sz_edge * (size_t)upd->Total_Num_Matrices);
  ...
  for (imtrx = 0; imtrx < upd->Total_Num_Matrices; imtrx++) {
    (*First_Elem_Side_BC_Array)[imtrx] =
        (struct elem_side_bc_struct **)alloc_ptr_1(num_internal_elems);
    (*First_Elem_Edge_BC_Array)[imtrx] =
        (struct elem_edge_bc_struct **)alloc_ptr_1(num_internal_elems);
  }
  return;
}
/**************************************************************************************************************************/
 ...
 */
{
  ...
  if (ebindex_target == -1) {
    GOMA_EH(GOMA_ERROR, "searchEBForNode Error: eb_id_target not found");
  }
...
void set_up_Surf_BC(struct elem_side_bc_struct **First_Elem_Side_BC_Array[], Exo_DB *exo, Dpi *dpi)
 ...
 *****************************************************************/
{
                ...
                if (BC_Types[ibc].BC_Name == SH_GAMMA1_DERIV_SYMM_BC ||
                  ...
                  if (BC_Types[ibc].matrix >= 0) {
                    setup_Elem_BC(&First_Elem_Side_BC_Array[BC_Types[ibc].matrix][ielem],
                                  &BC_Types[ibc], ibc, num_nodes_on_side, ielem, local_node_list,
                                  exo);
                  }
                }
              } /* END for (i = 0; i < Proc_NS_Count[ins]; i++)           */
              ...
              */
              ...
              if (num_nodes_on_side < 1) {
                fprintf(stderr,
                        ...
                        ProcID, iss, exo->ss_id[iss], i, ielem + 1, side, num_nodes_on_side);
              }
              local_node_list = &(exo->ss_node_list[iss][exo->ss_node_side_index[iss][i]]);
#ifdef DEBUG_SS
              ...
              if (BC_Types[ibc].matrix >= 0) {
                setup_Elem_BC(&First_Elem_Side_BC_Array[BC_Types[ibc].matrix][ielem],
                              &BC_Types[ibc], ibc, num_nodes_on_side, ielem, local_node_list, exo);
              }
  ...
  for (ibc = 0; ibc < Num_BC; ibc++) {
    ...
    if (bc->BC_Name == SDC_STEFANFLOW_BC) {
      ...
      for (ibc2 = 0; ibc2 < Num_BC; ibc2++) {
        bc2 = BC_Types + ibc2;
        if ((bc2->BC_ID == ibc_id) && (bc2->BC_Name == VL_EQUIL_PRXN_BC)) {
          bc->BC_Data_Int[1]++;
  ...
  DPRINTF(stdout, "\n");
  for (ibc = 0; ibc < Num_BC; ibc++) {
    if (BC_Types[ibc].BC_Name == KIN_LEAK_BC || BC_Types[ibc].BC_Name == KIN_CHEM_BC ||
        BC_Types[ibc].BC_Name == KIN_ELECTRODEPOSITION_BC ||   /*  RSL 5/27/02  */
        BC_Types[ibc].BC_Name == VNORM_ELECTRODEPOSITION_BC || /*  RSL 5/30/02  */
        BC_Types[ibc].BC_Name == QRAD_BC || BC_Types[ibc].BC_Name == QCONV_BC ||
        BC_Types[ibc].BC_Name == VNORM_LEAK_BC || BC_Types[ibc].BC_Name == LATENT_HEAT_BC ||
        BC_Types[ibc].BC_Name == LS_EIK_KIN_LEAK_BC ||
        BC_Types[ibc].BC_Name == LS_EXTV_KIN_LEAK_BC || BC_Types[ibc].BC_Name == LS_EXTV_LATENT_BC)

    {
      ...
      for (ibc2 = 0; ibc2 < Num_BC; ibc2++) {
        ...
        if ((BC_Types[ibc2].BC_ID == ibc_id) &&
            ((BC_Types[ibc2].BC_Name == YFLUX_BC) || (BC_Types[ibc2].BC_Name == LS_YFLUX_BC) ||
             (BC_Types[ibc2].BC_Name == LS_LATENT_HEAT_BC) ||
             (BC_Types[ibc2].BC_Name == YFLUX_BV_BC) || (BC_Types[ibc2].BC_Name == YFLUX_HOR_BC) ||
  ...
  for (ibc = 0; ibc < Num_BC; ibc++) {
    if (BC_Types[ibc].BC_Name == VELO_TANGENT_BC || BC_Types[ibc].BC_Name == VELO_TANGENT_USER_BC ||
        BC_Types[ibc].BC_Name == VELO_SLIP_BC || BC_Types[ibc].BC_Name == VELO_SLIP_ROT_BC ||
        BC_Types[ibc].BC_Name == VELO_SLIP_FILL_BC ||
        BC_Types[ibc].BC_Name == VELO_SLIP_ROT_FILL_BC || BC_Types[ibc].BC_Name == AIR_FILM_BC ||
        BC_Types[ibc].BC_Name == AIR_FILM_ROT_BC || BC_Types[ibc].BC_Name == VELO_SLIP_FLUID_BC ||
  ...
  for (ibc = 0; ibc < Num_BC; ibc++) {
...

    {
      ...
      if (BC_Types[ibc].BC_Data_Int[0] == -1) {
        sr = sprintf(err_msg, "%s: Q_VELO_SLIP specified without corresponding VELO_SLIP condition",
                     yo);
...
void free_Surf_BC(struct elem_side_bc_struct **First_Elem_Side_BC_Array[], Exo_DB *exo) {
  int e_start, e_end;
  int ielem;
  int imtrx;
...
void free_Edge_BC(struct elem_edge_bc_struct **First_Elem_Edge_BC_Array[], Exo_DB *exo, Dpi *dpi) {
  int e_start, e_end;
  int ielem;
  int imtrx;
...
void setup_Point_BC(struct elem_side_bc_struct **First_Elem_Side_BC_Array[],
                    Exo_DB *exo,
                    Dpi *dpi) {
  char err_msg[MAX_CHAR_IN_INPUT];
  // int i;
  ...
  for (ibc = 0; ibc < Num_BC; ibc++) {
    ...
    if (!strcmp(BC_Types[ibc].Set_Type, "NS")) {
      ...
      if (BC_Types[ibc].desc->method == WEAK_INT_SURF ||
        ...
        is_ns_g = in_list(ns_id, 0, dpi->num_node_sets_global, dpi->ns_id_global);
        ...
        if (!found_ns_global) {
          sr = sprintf(err_msg, "NS_ID %d for BC (%d) not found", ns_id, ibc + 1);
          GOMA_EH(GOMA_ERROR, err_msg);
        }
        ...
        if (found_ns_local) {
          ...
          ielem = exo->node_elem_list[exo->node_elem_pntr[exo->ns_node_list[ins]]];

          setup_Elem_BC(&First_Elem_Side_BC_Array[BC_Types[ibc].matrix][ielem], &BC_Types[ibc], ibc,
                        num_nodes_on_side, ielem, node_list, exo);
        }
      }
    }
  }
...
void set_up_Edge_BC(struct elem_edge_bc_struct **First_Elem_Edge_BC_Array[],
                    Exo_DB *exo,
                    Dpi *dpi) {
  ...
  for (ibc = 0; ibc < Num_BC; ibc++) {
    ...
    if (!strcmp(BC_Types[ibc].Set_Type, "SS")) {
      ...
      if (BC_Types[ibc].desc->method == COLLOCATE_EDGE ||
...
#ifdef PARALLEL
        iss1g = in_list(ss_id1, 0, dpi->num_side_sets_global, dpi->ss_id_global);
        ...
        {
          sr = sprintf(err_msg, "Primary SS_ID %d for BC (%d) not found", ss_id1, ibc + 1);
          GOMA_EH(GOMA_ERROR, err_msg);
        }
...
#ifdef PARALLEL
        iss2g = in_list(ss_id2, 0, dpi->num_side_sets_global, dpi->ss_id_global);
        ...
        {
          sr = sprintf(err_msg, "Secondary SS_ID %d for BC (%d) not found", ss_id2, ibc + 1);
          GOMA_EH(GOMA_ERROR, err_msg);
        }
        ...
        if (found_ss_primary_local && found_ss_secondary_local) {
          for (i = 0; i < exo->ss_num_sides[iss1]; i++) {
            ...
            num_nodes_on_side =
                (exo->ss_node_side_index[iss1][i + 1] - exo->ss_node_side_index[iss1][i]);
            ...
            if (j != -1) {
              ...
              num_nodes_on_side2 =
                  (exo->ss_node_side_index[iss2][j + 1] - exo->ss_node_side_index[iss2][j]);
              ...
              for (k = 0; k < num_nodes_on_side; k++) {
                ...
                l = in_list(k_node, 0, num_nodes_on_side2, &(exo->ss_node_list[iss2][k2_start]));
              ...
              if (node_ctr == 0)
                GOMA_EH(GOMA_ERROR, "No nodes on edge");
              ...
              if (num_nodes_on_edge != -1) {
                ...
                if (num_nodes_on_edge != node_ctr) {
                  GOMA_EH(GOMA_ERROR, "Number of edge nodes varies w/ element!");
                }
              ...
              if (BC_Types[ibc].matrix >= 0) {
                this_edge_bc = setup_Elem_Edge_BC(
                    &First_Elem_Edge_BC_Array[BC_Types[ibc].matrix][ielem], &BC_Types[ibc], ibc,
                    num_nodes_on_edge, ipin, FALSE, ielem, node_list, exo);

                setup_Elem_BC(&(this_edge_bc->elem_side_bc_1), &BC_Types[ibc], ibc,
                              num_nodes_on_side, ielem,
                              &(exo->ss_node_list[iss1][exo->ss_node_side_index[iss1][i]]), exo);

                setup_Elem_BC(&(this_edge_bc->elem_side_bc_2), &BC_Types[ibc], ibc,
                              num_nodes_on_side, ielem,
              ...
              for (l = exo->elem_elem_pntr[ielem], found = FALSE;
                   found == FALSE && l < exo->elem_elem_pntr[ielem + 1]; l++) {
                ...
                if (j != -1) {
                  ...
                  for (k = exo->ss_node_side_index[iss1][i];
                       k < exo->ss_node_side_index[iss1][i + 1]; k++) {
              ...
              if (found == TRUE) {
                ...
                if (BC_Types[ibc].matrix >= 0) {
                  this_edge_bc = setup_Elem_Edge_BC(
                      &First_Elem_Edge_BC_Array[BC_Types[ibc].matrix][ielem], &BC_Types[ibc], ibc,
                      num_nodes_on_edge, ipin, TRUE, ielem, node_list, exo);
 ...
 *****************************************************************/
{
  ...
  for (ibc = 0; ibc < Num_BC; ibc++) {
    ...
    if (!strcmp(BC_Types[ibc].Set_Type, "LS")) {
      ...
      if (ls == NULL) {
        GOMA_EH(GOMA_ERROR,
                "Attempt to apply bc on LS but level set is not active, did you intend PF?\n");
      }
      ...
      if (pfd == NULL) {
        GOMA_EH(GOMA_ERROR, "Attempt to apply bc on PF but phase functions are not active.\n");
      }

      if (pf >= pfd->num_phase_funcs) {
        GOMA_EH(GOMA_ERROR, "Attempt to apply bc to invalid phase function.\n");
      }
  ...
  /*   if (!found_capillary) { */
  /*     GOMA_WH(-1,"No LS_CAPILLARY condition specified on the level set surface.\n"); */
  /*     GOMA_WH(-1,"To add surface tension add 'BC = LS_CAPILLARY LS 0' in the input file.\n"); */
  /*   } */
}
/*****************************************************************************/
/*****************************************************************************/
...
int find_bc_unk_offset(struct Boundary_Condition *bc,
                       int curr_mat,
                       int inode,
                       int p,
                       int *retn_matIndex,
 ...
 *********************************************************/
{
  ...
  if (!pd_curr->e[pg->imtrx][ieqn]) {
     ...
     */
       ...
       */
      if (in_list(matIndex, 0, node->Mat_List.Length, node->Mat_List.List) == -1) {
        GOMA_EH(GOMA_ERROR, "TROUBLE");
      }
    }
  }
  ...
  if (ndofs_ieqn > 1) {
     ...
     */
    if (bc_name == CONT_TANG_VEL_BC || bc_name == CONT_NORM_VEL_BC ||
        bc_name == DISCONTINUOUS_VELO_BC || bc_name == VL_EQUIL_BC || bc_name == VL_POLY_BC ||
     ...
     */
    else if (bc_name == KINEMATIC_SPECIES_BC || bc_name == CAPILLARY_BC ||
             bc_name == CAP_REPULSE_BC || bc_name == CAP_REPULSE_ROLL_BC ||
             bc_name == CAP_REPULSE_USER_BC || bc_name == CAP_REPULSE_TABLE_BC ||
             bc_name == CAPILLARY_TABLE_BC || bc_name == CAP_RECOIL_PRESS_BC) {
  ...
  *retn_matIndex = matIndex;
  ...
  if (ieqn == MASS_FRACTION) {
    offset = get_nodal_unknown_offset(nv, ieqn, matIndex, bc->species_eq, vd_ptr);
  } else {
 ...
 *********************************************************************/
{
  ...
  if (side == NULL) {
    ...
    side->ielem = ielem;
    *elem_side_bc = side;
    side->id_side = find_id_side_BC(ielem, num_nodes_on_side, local_ss_node_list, ibc,
    ...
    if (same_side(local_ss_node_list, side->local_node_id, num_nodes_on_side)) {
      ...
      if (side->Num_BC > MAX_BC_PER_SIDE) {
        sprintf(err_msg, "MAX_BC_PER_SIDE(%d) exceeded - elem [%d], BC %d", MAX_BC_PER_SIDE, ielem,
                ibc);
        GOMA_EH(GOMA_ERROR, err_msg);
      }
      vcrr_determination(side, bc_type, ielem, ibc);
       ...
       * recursive loop
       */
      setup_Elem_BC(&(side->next_side_bc), bc_type, ibc, num_nodes_on_side, ielem,
                    local_ss_node_list, exo);
    }
  }
}
/*****************************************************************************/
...
static struct elem_edge_bc_struct *setup_Elem_Edge_BC(struct elem_edge_bc_struct **elem_edge_bc,
                                                      struct Boundary_Condition *bc_type,
                                                      int ibc,
                                                      int num_nodes_on_edge,
                                                      int ipin,
  ...
  if (*elem_edge_bc == NULL) {
    ...
    *elem_edge_bc = alloc_struct_1(struct elem_edge_bc_struct, 1);
    ...
    this_edge_bc =
        setup_Elem_Edge_BC(&((*elem_edge_bc)->next_edge_bc), bc_type, ibc, num_nodes_on_edge, ipin,
                           shared, ielem, local_edge_node_list, exo);
  }
...
static int same_side(int ilist1[], int ilist2[], int list_length)
...
void initialize_Boundary_Condition(struct Boundary_Condition *bc_ptr)
...
{
  if (bc_ptr == NULL) {
    GOMA_EH(GOMA_ERROR, "initialize_Boundary_Condition FATAL ERROR");
  }
...
int find_id_side(const int ielem,                /* element index number */
                 ...
                 const Exo_DB *exo)              /* ptr to FE db         (in) */
{
  ...
  for (i = 0; i < num_nodes_on_side; i++) {
    if ((id_local_elem_coord[i] = in_list(local_ss_node_list[i], 0, num_local_nodes,
                                          &exo->elem_node_list[iconnect_ptr])) == -1) {
      sr = sprintf(err_msg, "%s: lost SS node [%d] = %d  %d  %d", yo, i, local_ss_node_list[i],
                   num_nodes_on_side, ielem);
      GOMA_EH(GOMA_ERROR, err_msg);
    }
  }
  ...
  sr = sprintf(err_msg, "%s: problem for elem (%d), dimension %d, %d nodes on side.", yo, ielem + 1,
               ielem_dim, num_nodes_on_side);
...
int find_id_side_BC(const int ielem,                /* element index number */
                    const int num_nodes_on_side,    /* guess what?          (in) */
                    const int local_ss_node_list[], /* nodes, ordered	(in) */
                    ...
                    const Exo_DB *exo)              /* ptr to FE db         (in) */
{
  ...
  sideid = find_id_side(ielem, num_nodes_on_side, local_ss_node_list, id_local_elem_coord, exo);
  ...
  if (ielem_type == LINEAR_TET || ielem_type == QUADRATIC_TET) {
    ...
    if (nodes[0] && nodes[1] && nodes[3]) {
      ...
      GOMA_EH(GOMA_ERROR, "Unknown tet layout");
    }
  }
...
int find_id_side_SS(const int ielem,   /* ielem - element index number     (in) */
...
void print_setup_Surf_BC(struct elem_side_bc_struct *First_Elem_Side_BC_Array[]) {
  int ielem;

  if (ProcID == 0) {
    (void)printf("\t\t PRINTOUT OF SURFACE BOUNDARY INTEGRAL SET-UP\n");
  }
...
static void print_Elem_Surf_BC(int ielem, struct elem_side_bc_struct *elem_side_bc) {
  ...
  while (elem_side_bc->BC_input_id[ibc] != -1) {
    (void)printf("   %5d       %5d        %5d         %9d     %5d        |    ", ielem,
                 (int)elem_side_bc->id_side, (int)elem_side_bc->num_nodes_on_side,
                 elem_side_bc->BC_applied, (int)elem_side_bc->BC_input_id[ibc]);
...
struct BC_descriptions *alloc_BC_description(struct BC_descriptions *old_ptr)
 ...
 ********************************************************************/
{
  ...
  (void)memcpy((void *)tmp_ptr, (void *)old_ptr, sizeof(struct BC_descriptions));
  return tmp_ptr;
}
/*************************************************************************/
/*************************************************************************/
...
char *rot_eq_type2string(const int eq_type) {
...
char *rotopology_type2string(const int type) {
...
static void elem_side_matrl_list(struct elem_side_bc_struct *side)
  ...
  for (i = 0; i < num_nodes; i++) {
    ...
    for (j = 0; j < mlist->Length; j++) {
      ...
      if (mlist->List[j] < 0) {
        ...
        GOMA_EH(GOMA_ERROR, "logic error with ignored element block");
      }
#endif
...
static void vcrr_determination(struct elem_side_bc_struct *side,
  ...
  if (bc->DV_Indexing_Type == DVI_DVVSIG) {
    if (side->Num_MatID > 1) {
      ...
      if (mn_low != mn) {
        ...
        if (nf) {
          GOMA_EH(GOMA_ERROR, "Problems with material mappings");
        }
        /*
        ...
        for (i = 0; i < side->num_nodes_on_side; i++) {
          ...
          for (p = 0; p < bc_desc->vector; p++) {
            index_eq = bc_eqn_index(lnn, node_num, ibc, mn, p, &eqn, &matID_apply, &vd);
            if (index_eq >= 0) {
...
void set_up_BC_connectivity(void)
 ...
 ************************************************************/
{
  ...
  for (ibc = 0; ibc < Num_BC; ibc++) {
    ...
    case SDC_KIN_SF_BC:
      ...
      if (!ifound) {
        GOMA_EH(GOMA_ERROR, "set_up_BC_connectivity failure");
      }
      break;
    default:
      break;

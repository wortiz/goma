...
EXTERN void
goma_wh_all_proc(const int error_flag, const char *const file, const int line, const char *format, ...);
...
#define GOMA_WH_ALL_PROC(IERR, FORMAT, ...)                              \
  do {                                                          \
    static bool print = true;                                   \
    if (print) {                                                \
      goma_wh_all_proc(IERR, __FILE__, __LINE__, FORMAT, ##__VA_ARGS__); \
      print = false;                                            \
    }                                                           \
  } while (0)
